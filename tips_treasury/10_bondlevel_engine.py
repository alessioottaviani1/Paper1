...
tra0 = pd.read_excel(BBG/"Treasury.xlsx", sheet_name="Bonds")
tra0["Maturity"]=pd.to_datetime(tra0["Maturity"]); tra0["ISIN"]=tra0["ISIN"].astype(str).str.strip()
nmat_map = tra0.dropna(subset=["Maturity"]).drop_duplicates("ISIN").set_index("ISIN")["Maturity"].to_dict()
...
def basis_for(tips, tsy, w0, w1):
    ...
    tau_raw = ((tmat[tips]-idx).days/365.25).values
    tau = tau_raw.clip(1,30)
    ...
    taun = (((nmat_map[tsy]-idx).days/365.25).values.clip(1,30)) if tsy in nmat_map else tau
    adj = np.array([np.interp(a,gk[np.isfinite(r)],r[np.isfinite(r)])-np.interp(b,gk[np.isfinite(r)],r[np.isfinite(r)])
                    if np.isfinite(r).sum()>1 else 0.0 for a,b,r in zip(tau,taun,Gm)])
    return pd.DataFrame({"date":idx,"tips":tips,"tsy":tsy,"tau":tau_raw,
                         "lam":(yt+s-(yn+adj))*100.0,"lam_gsw":(yt+s-yg)*100.0})
...
P(f"    n_ours/fll_N mediano: {float((rep['n_ours']/rep['N']).median()):.2f} | corr su coppie N>=300: {rep[rep.N>=300]['ours'].corr(rep[rep.N>=300]['fll']):.3f} ({int((rep.N>=300).sum())} coppie)")
...
sh  = out[(out.tau>=1.0)&(out.tau<2.5)].set_index("date").groupby(level=0)["lam"].median()
