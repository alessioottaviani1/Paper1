...
def X_of(l0,Hm,B):
    ...
    X=np.empty(len(zH))
    if Hm>=T_MAT_M: X[alive]=l0*AF(10.0)
    else:
        Hy=Hm/12.0; X[alive]=l0*AF(Hy)+np.exp(-Y0*Hy)*(-D_DUR*zH[alive])
    X[~alive]=l0*AF(tau[~alive])-B*np.exp(-Y0*tau[~alive])
    return X
