...
def _compute_weights_and_grid(X_prep, y_c, lambda2_grid, n_lambda1,
    ...
    # OLS gives every factor a finite weight; the sparse Stage-1 EN estimate
    # would set wⱼ = T for zeroed factors and ban them from Stage 2 + bootstrap.
    beta_init = compute_initial_beta(X_prep, y_c)
    weights = (np.abs(beta_init) + 1.0 / T) ** (-gamma)
