...
def run_aen_full(y, X, factor_names, lambda2_grid, n_lambda1,
    ...
    # Adaptive weights — OLS initial estimator (p < T — cf. 02/05)
    epsilon = 1.0 / T
    beta_init = compute_initial_beta(X_s, y_c)
...
def sensitivity_gamma(strategy_name, strategy_path):
    ...
    # OLS initial estimator for the weights (p < T — cf. 02/05); β̂(enet)
    # above is kept only for the informational nonzero count.
    ...
    for gamma_val in GAMMA_VALUES:
        ...
        epsilon = 1.0 / T
        weights = (np.abs(beta_init) + epsilon) ** (-gamma_val)
