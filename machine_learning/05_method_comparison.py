...
def run_comparison_for_strategy(strategy_name, strategy_path):
    ...
    # ── METHOD 5: AEN with stability selection (load from 04) ──────────
    # Use the SAME stability-selected factors as the main results table
    # (ols_stable_results.json from Step 04), not the raw Step-02 AEN,
    # so that the "AEN" row is consistent with the Post-Selection OLS table.
    print(f"\n   [5/6] AEN + stability selection (loaded from Step 04)...")
    with open(strategy_dir / "ols_stable_results.json", 'r') as f:
        aen_res = json.load(f)
    sel_aen = aen_res['stable_factors']
