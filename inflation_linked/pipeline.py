...
def _euro_nss_params(mkt: str, ref_nom: pd.DataFrame, ytm: pd.DataFrame,
                     dates: pd.DatetimeIndex, px_file: str | None = None,
                     out_name: str | None = None) -> pd.DataFrame:
...
def _euro_real_nss_params(mkt: str, ref_link: pd.DataFrame, ytm_link: pd.DataFrame,
                          dates: pd.DatetimeIndex) -> pd.DataFrame:
    """Curva REALE zero fittata dai linker (BTP\u20aci): stesso motore GSW-sui-prezzi del
    nominale. I linker euro quotano in termini REALI (clean reale; il CI entra solo nel
    regolamento), quindi cashflow = cedole reali all'indietro dalla scadenza (CPN,
    CPN_FREQ dell'anagrafica linker), dirty reale = clean + rateo ACT/ACT ICMA, sconto
    dal regolamento: identico al nominale, cambia solo l'universo e l'output.
    Output: nss_real_{mkt}.parquet. BEI_{mkt} = NSS nominale - NSS reale."""
    return _euro_nss_params(mkt, ref_link, ytm_link, dates,
                            px_file=f"px_mid_{mkt}",
                            out_name=f"nss_real_{mkt}")
