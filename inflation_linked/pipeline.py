...
MARKET_FLOOR = {"IT": True, "FR": True, "FR_CPI": True, "DE": True, "ES": True,
                "US": True, "UK": False}


# ------------------------------------------------------------------ curva zero nominale
def _euro_nss_params(mkt: str, ref_nom: pd.DataFrame, ytm: pd.DataFrame,
                     dates: pd.DatetimeIndex, min_bonds_frac: float = 0.5) -> pd.DataFrame:
    ...
    # Date con troppo pochi titoli. La NSS ha SEI parametri: su 9 bond interpola invece
    # di fittare, e l'rmse basso lo conferma invece di smentirlo. Succede nei giorni di
    # chiusura parziale: il 6 aprile 2012 (Venerdi Santo) il fit italiano gira su 9
    # titoli contro i 44-87 abituali, passa min_bonds_nss=8, e produce le tre
    # osservazioni peggiori dell'intero campione (-1917, -1000, -329bp sullo stesso
    # giorno). La soglia e' RELATIVA alla norma del mercato, non assoluta: IT ha 44-87
    # titoli, FR 44-47, ES 46-54. Le curve scartate diventano NaN e si propagano come
    # dato mancante nelle basi, invece di produrre numeri inventati.
    if min_bonds_frac and "n_bonds" in old.columns:
        soglia = max(min_bonds_frac * old["n_bonds"].median(), 12)
        thin = old["n_bonds"] < soglia
        if thin.any():
            print(f"  {mkt}: {int(thin.sum())} date con meno di {soglia:.0f} titoli nel fit "
                  f"(mediana {old['n_bonds'].median():.0f}) -> curva scartata")
            old.loc[thin, ["b0", "b1", "b2", "b3", "t1", "t2"]] = np.nan
...
def build_market(mkt: str, methods=("nearest", "c_exact"),
                 ...
                 exclude_tail: bool = False, min_ttm_days: int = 90,
                 ytm_convention: str = "annual",
                 pub_lag_days: int | None = None, price_field: str = "px_mid",
                 years_history: int = 10, season_weight: float = 1.0,
                 dates: pd.DatetimeIndex | None = None,
                 date_from: str | None = None, save: bool = True) -> dict:
    t0 = time.time()
    m = MARKETS[mkt]
    if floor is None:
    ...
    cpn_cal = dict({"IT": _hol.Italy, "FR": _hol.France, "FR_CPI": _hol.France,
                    "DE": _hol.Germany, "ES": _hol.Spain,
                    "UK": _hol.UK, "US": _hol.US}[mkt]())
    ...
    # date_from: taglia la storia a monte. Serve per il gate di regressione, che vive sul
    # solo campione di btpei_basis.xlsx (2015-2026): girare dal 2003 costa il doppio del
    # tempo e non aggiunge nulla al confronto. Per i pannelli finali lasciare None.
    if date_from is not None:
        n0 = len(dates)
        dates = dates[dates >= pd.Timestamp(date_from)]
        print(f"  {mkt}: date_from={date_from} -> {len(dates)}/{n0} date")
    zero = None
    if "c_exact" in methods:
        zero = build_zero_panel(nom_mkt, ref_n, ytm, dates)

    cols = list(bonds)
    # basis_nearest / basis_cexact: differenze di tassi ANNUALI, invariate perche' sono
    # l'oggetto del vincolo di regressione. Le _cc e basis_zspread sono le misure per il
    # paper: vedi basis.zspread per il perche' (scaling per (1+y) e profilo di cassa).
    panels = {k: pd.DataFrame(np.nan, index=dates, columns=cols)
              for k in ["basis_nearest", "basis_cexact",
                        "basis_nearest_cc", "basis_cexact_cc", "basis_zspread",
    ...
    for k, asof_ts in enumerate(dates):
        ...
        for isin, b in bonds.items():
            ...
            if exclude_tail and asof > b.maturity - pd.Timedelta(days=365).to_pytimedelta():
                continue
            # Vita residua minima, stessa convenzione GSW che _euro_nss_params applica
            # gia' al fit della curva ("esclusi bond con vita residua <3 mesi"). Senza
            # questo si misurano spread di bond che la curva stessa non guarda, e su un
            # orizzonte che collassa: a un mese dalla scadenza 20 centesimi di prezzo si
            # annualizzano in centinaia di bp. E' l'origine degli estremi a -3149bp (IT,
            # giorni prima del 15 settembre), -638bp (ES, ultimo mese) e +2478bp (DE,
            # giorni prima del 15 aprile), e della stessa zona da cui usciva l'-inf.
            if (b.maturity - asof).days < min_ttm_days:
                continue
            try:
                ...
                panels["basis_nearest_cc"].at[asof_ts, isin] = row["basis_nearest_cc"]
                ...
                if "basis_c_exact" in row:
                    ...
                    panels["basis_cexact_cc"].at[asof_ts, isin] = row["basis_c_exact_cc"]
                    panels["basis_zspread"].at[asof_ts, isin] = row["basis_zspread"]
                if with_real:
