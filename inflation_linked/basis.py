...
def match_nominals(nom_ref: pd.DataFrame, target_maturity: date, asof: date) -> dict:
    ...
    exact = df[df["mat"] == tm]
    out = {"lower": None, "upper": None, "nearest": None, "w_lower": np.nan}
    if len(exact):
        e = exact.sort_values("AMT_OUTSTANDING", ascending=False).index[0]
        out.update(lower=e, upper=e, nearest=e, w_lower=1.0, dist_nearest=0)
        return out
    if len(lower):
    ...
    if dl is not None and du is not None:
        out["w_lower"] = du / (dl + du)
        out["nearest"], out["dist_nearest"] = (out["lower"], dl) if dl <= du else (out["upper"], du)
    elif dl is not None:
        out["w_lower"], out["nearest"], out["dist_nearest"] = 1.0, out["lower"], dl
    elif du is not None:
        out["w_lower"], out["nearest"], out["dist_nearest"] = 0.0, out["upper"], du
    return out
...
def nominal_leg(match: dict, ytm_row: pd.Series, mkt: str, mode: str,
                ytm_convention: str = "local") -> float:
    """YTM della gamba nominale in percento. mode: 'interp' (originale) o 'nearest' (FLL)."""
    def _y(isin):
        v = ytm_row.get(isin, np.nan)
        if pd.isna(v):
            return np.nan
    ...
    if mode == "nearest":
        if match["nearest"] is None or match.get("dist_nearest", 1e9) > MAX_MISMATCH_DAYS:
            return np.nan
        return _y(match["nearest"])
    yl = _y(match["lower"]) if match["lower"] else np.nan
    yu = _y(match["upper"]) if match["upper"] else np.nan
    w = match["w_lower"]
    if pd.isna(yl) and pd.isna(yu):
        return np.nan
    if pd.isna(yl):
        return yu
    if pd.isna(yu):
...
def bond_basis_row(bond: LinkerBond, asof: date, engine, price_clean: float,
                   ...
                   floor: bool = True, ytm_convention: str = "local") -> dict:
    """Tutte le misure per (bond, data). Ritorna dict di output in bp/percento."""
    ...
    for mode in ("interp", "nearest"):
        y_nom = nominal_leg(match, ytm_row, bond.mkt, mode, ytm_convention)
        out[f"basis_{mode}"] = (y_tot - y_nom) * 100 if pd.notna(y_nom) else np.nan
        out[f"ynom_{mode}"] = y_nom
    out["mismatch_days"] = match.get("dist_nearest", np.nan)
...
def regression_check(computed: pd.DataFrame, target_xlsx, tol_bp: float = None) -> pd.DataFrame:
    """Confronta la base calcolata (wide, colonne ISIN, bp) con btpei_basis.xlsx
    (colonne '{ISIN}_Basis'). Riporta max/mean |diff| e i peggiori scostamenti."""
    tol_bp = tol_bp if tol_bp is not None else REGRESSION_TOL_BP
    tgt = pd.read_excel(target_xlsx, index_col=0, parse_dates=True)
    tgt.columns = [str(c).replace("_Basis", "") for c in tgt.columns]
    common_c = [c for c in computed.columns if c in tgt.columns]
    common_i = computed.index.intersection(tgt.index)
    d = (computed.loc[common_i, common_c] - tgt.loc[common_i, common_c]).abs()
    rep = pd.DataFrame({"max_abs_bp": d.max(), "mean_abs_bp": d.mean(),
                        "n_over_tol": (d > tol_bp).sum(), "n_obs": d.notna().sum()})
    print(f"regressione su {len(common_i)} date x {len(common_c)} ISIN | tol {tol_bp} bp")
    print(f"  max |diff| complessivo: {np.nanmax(d.values):.4f} bp | "
          f"celle oltre tol: {int((d > tol_bp).sum().sum())}")
    return rep.sort_values("max_abs_bp", ascending=False)
