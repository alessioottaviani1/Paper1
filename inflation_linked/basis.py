...
SETTLE_DAYS = {"IT": 2, "FR": 2, "FR_CPI": 2, "DE": 2, "ES": 2, "UK": 1, "US": 1}
...
class LinkerBond:
    ...
    def from_ref(cls, isin: str, row: pd.Series) -> "LinkerBond":
        ...
        return cls(isin=isin, mkt=mkt, cpn_rate=float(row["CPN"]),
                   cpn_freq=int(float(row["CPN_FREQ"])),
                   start_acc=_d(row["START_ACC_DT"]) or _d(row["ISSUE_DT"]),
    ...
    def cpn_dates(self) -> list:
        ...
        if t == "Normal":
            # ancora del ciclo: FIRST_CPN_DT (data cedola vera Bloomberg). start_acc+step
            # e' equivalente SOLO se start_acc e' la dated date; col fallback ISSUE_DT
            # (US: START_ACC_DT assente) il ciclo si disallinea dalla maturity e il
            # rimborso (ramo cpn_date == maturity) non viene mai aggiunto.
            cur = self.first_cpn or (self.start_acc + relativedelta(months=step))
    ...
    def irr(self, cf: pd.DataFrame, asof: date) -> float:
        ...
        if len(self.cpn_dates) >= 2 and asof >= self.cpn_dates[-2]:
            # Ramo money-market ACT/365. Se il regolamento (T+2) raggiunge o supera
            # l'ultimo flusso il denominatore e' 0 o negativo: numpy restituisce +-inf
            # IN SILENZIO (float64/0) e il filterwarnings in testa al modulo sopprime
            # anche il RuntimeWarning, cosi' il run dichiara 0 errori e l'infinito
            # finisce nel pannello avvelenando medie e deviazioni standard.
            # Li' la misura non esiste: NaN, che si propaga come dato mancante.
            n_days = (cf.index[-1] - cf.index[0]).days
            if n_days <= 0:
                return np.nan
            return (abs(c[-1] / c[0]) - 1) * 365 / n_days * 100
...
def match_nominals(nom_ref: pd.DataFrame, target_maturity: date, asof: date) -> dict:
    ...
    df = nom_ref.copy()
    fs = pd.to_datetime(df["FIRST_SETTLE_DT"]).fillna(pd.to_datetime(df["ISSUE_DT"]))
    df = df[fs < pd.Timestamp(asof)]
...
def synthetic_irr(bond: LinkerBond, cf: pd.DataFrame, asof: date,
    ...
    zr = zero_row.dropna()
    if not len(zr) or not len(taus):
        return np.nan          # curva scartata (troppi pochi titoli nel fit): non e' un
                               # errore, e' un dato mancante -- stessa guardia di zspread
    grid = np.array(sorted(float(c) for c in zr.index))
    vals = zr.reindex(sorted(zr.index)).values.astype(float)
    ...
    # coerenza col ramo speciale dell'IRR osservato (stessa guardia sul denominatore)
    if len(bond.cpn_dates) >= 2 and asof >= bond.cpn_dates[-2]:
        n_days = (cf.index[-1] - cf.index[0]).days
        if n_days <= 0:
            return np.nan
        return (abs(c2[-1] / c2[0]) - 1) * 365 / n_days * 100
    return _solve_irr(c2, tt) * 100


# ------------------------------------------------------------------ metodo Z-spread
def _cc(y):
    """Tasso annualmente composto (percento) -> continuo (percento).
    Gemella di curves.ann_to_cc; replicata qui per non accoppiare basis a curves."""
    return 100.0 * np.log1p(np.asarray(y, float) / 100.0)


def zspread(cf: pd.DataFrame, asof: date, zero_row: pd.Series) -> float:
    """Shift PARALLELO della curva zero nominale (cc) che riprezza i flussi nominali
    sintetici del linker al prezzo dirty OSSERVATO. Percento.

    E' la misura esatta della base, e domina il C-esatto su due dimensioni:
      - LIVELLO DEI TASSI. Il C-esatto e' una differenza di IRR ANNUALI, quindi scala per
        (1+y): a cheapness vera costante di 30bp legge 30.46bp con curva a 0% e 32.16bp
        con curva a 5.5%. Sul campione 2004-2026 (euro da ~4% a negativo e ritorno) quel
        fattore oscilla dentro il campione e si correla meccanicamente col livello dei
        tassi -- cioe' con MOVE, SRVIX e gli indicatori di crisi che finiscono a destra
        nelle regressioni. Lo Z-spread e' piatto a 30.00bp su tutti i livelli.
      - PROFILO DI CASSA. Su quattro linker con cedola reale 0.10-4.00 e index ratio
        1.00-1.45 e stessa cheapness vera, il C-esatto disperde ~0.15bp, lo Z-spread zero
        esatto per costruzione.
    Il C-esatto convertito in cc recupera quasi tutto (residuo ~0.1bp): e' la colonna
    _cc qui sotto. Le tre misure vanno riportate insieme, come per MATCHING.

    Implementazione: stessa base tempo, stessa griglia e stessa interpolazione lineare in
    maturita' di synthetic_irr, sconto AL REGOLAMENTO -- le colonne sono confrontabili
    riga per riga senza riallineamenti.

    NB: nell'ultimo periodo cedolare synthetic_irr passa al rendimento monetario semplice
    ACT/365 (ramo ereditato dall'originale) mentre qui lo sconto resta esponenziale: in
    quella finestra le due misure divergono per costruzione, non per errore. Usare
    exclude_tail=True se la coda disturba.
    """
    t0 = (cf.index[0] - asof).days / 365.25
    taus = np.array([(d - asof).days / 365.25 for d in cf.index[1:]]) - t0
    zr = zero_row.dropna()
    if not len(zr) or not len(taus):
        return np.nan
    grid = np.array(sorted(float(c) for c in zr.index))
    vals = zr.reindex(sorted(zr.index)).values.astype(float)
    z = np.interp(taus, grid, vals) / 100.0
    c_fut = cf["Cashflows"].values[1:].astype(float)
    p_obs = -float(cf["Cashflows"].values[0])        # la prima riga e' -(dirty osservato)
    if not np.isfinite(p_obs) or p_obs <= 0:
        return np.nan

    def _f(s):
        return float(np.sum(c_fut * np.exp(-(z + s) * taus))) - p_obs

    lo, hi = -0.20, 0.20
    try:
        if _f(lo) * _f(hi) > 0:                      # base fuori bracket: allarga una volta
            lo, hi = -1.0, 1.0
            if _f(lo) * _f(hi) > 0:
                return np.nan
        return brentq(_f, lo, hi, xtol=1e-14, rtol=1e-12) * 100.0
...
def bond_basis_row(bond: LinkerBond, asof: date, engine, price_clean: float,
    ...
    out = {"total_ytm": y_tot,
           # --- invariate: sono l'oggetto del vincolo di regressione (REGRESSION_TOL_BP)
           "basis_nearest": (y_tot - y_nom) * 100 if pd.notna(y_nom) else np.nan,
           # --- cc: toglie lo scaling per (1+y) anche alla gamba nearest, che e' una
           #     differenza di tassi annuali esattamente come il C-esatto
           "basis_nearest_cc": (_cc(y_tot) - _cc(y_nom)) * 100 if pd.notna(y_nom) else np.nan,
           "ynom_nearest": y_nom,
           "mismatch_days": match.get("dist_nearest", np.nan)}
    if zero_row is not None:
        y_syn = synthetic_irr(bond, cf, asof, zero_row)
        out["basis_c_exact"] = (y_tot - y_syn) * 100              # invariata (regressione)
        out["basis_c_exact_cc"] = (_cc(y_tot) - _cc(y_syn)) * 100
        out["basis_zspread"] = zspread(cf, asof, zero_row) * 100  # misura esatta
        out["ysyn"] = y_syn
    return out
