...
YTM_COMP = {"IT": 1, "FR": 1, "FR_CPI": 1, "DE": 1, "ES": 1, "UK": 2, "US": 2}
...
def fit_nss_panel(bonds: pd.DataFrame, min_bonds_nss: int = 8,
    ...
    for dt, g in bonds.groupby("date"):
        ...
        if r is None:
            continue
        prev = r.x
        px = list(r.x[:3]) + [0.0 if ns else r.x[3]] + list(r.x[4:])   # b3=0 nei giorni NS:
        # il residuo lo azzera, quindi il b3 grezzo dell'ottimizzatore e' rumore senza
        # gradiente e NON va in cache (il consumer valuterebbe una curva sbagliata).
        fit = nss_yield(g["tau"].values, *px)
        rmse = float(np.sqrt(np.mean((fit - g["y_cc"].values) ** 2))) * 100.0   # bp
        rows.append([dt, *px, n, rmse, "NS" if ns else "NSS"])
    return pd.DataFrame(rows, columns=["date", "b0", "b1", "b2", "b3", "t1", "t2",
                                       "n_bonds", "rmse_bp", "model"]).set_index("date")



# --------------------------------------------------- fit GSW sui PREZZI (curve euro)
def _pack_bonds(cf_taus, cf_amts):
    """Impacchetta i cashflow di tutti i bond in array piatti per lo sconto vettorizzato:
    una sola nss_yield + exp + bincount per valutazione del residuo (vs loop Python)."""
    flat_t = np.concatenate([np.asarray(t, float) for t in cf_taus])
    flat_a = np.concatenate([np.asarray(a, float) for a in cf_amts])
    idx = np.concatenate([np.full(len(t), i) for i, t in enumerate(cf_taus)]).astype(int)
    return flat_t, flat_a, idx, len(cf_taus)


def fit_nss_prices(cf_taus, cf_amts, p_dirty, d_mod, x0=None, ns_only: bool = False,
                   full_search: bool = True):
    """Fit NSS ai minimi quadrati SUI PREZZI, alla Gurkaynak-Sack-Wright (2007, JME):
    si minimizza (P_model - P_obs) / (D_mod * P_obs) -- errore di prezzo pesato per
    l'inversa della duration, la scelta di GSW perche' approssima l'errore di YIELD
    (il residuo e' in unita' di yield: rmse * 1e4 = bp equivalenti).
    full_search=True prova x0 + tutti i CANONICAL_STARTS (primo giorno / retry);
    full_search=False usa solo x0 (warm start dal giorno precedente: la pratica standard
    del fitting giornaliero, i parametri evolvono lentamente)."""
    from scipy.optimize import least_squares
    po = np.asarray(p_dirty, float)
    dm = np.asarray(d_mod, float)
    ft, fa, idx, nb = _pack_bonds(cf_taus, cf_amts)
    wgt = 1.0 / (dm * po)
    ...
    if full_search or not starts:
        starts += CANONICAL_STARTS
    best = None
    for s in starts:
        try:
            r = least_squares(resid, s, bounds=NSS_BOUNDS, method="trf", max_nfev=4000)
...
def _rmse_prices(px, cf_taus, cf_amts, po, dm):
    ft, fa, idx, nb = _pack_bonds(cf_taus, cf_amts)
    z = nss_yield(ft, *px)
    pm = np.bincount(idx, weights=fa * np.exp(-z / 100.0 * ft), minlength=nb)
    return float(np.sqrt(np.mean(((pm - np.asarray(po)) /
                                  (np.asarray(dm) * np.asarray(po))) ** 2))) * 1e4


def fit_nss_prices_panel(day_bonds: dict, min_bonds_nss: int = 8, min_bonds: int = 4,
                         retry_rmse_bp: float = 12.0, progress_every: int = 250
                         ) -> pd.DataFrame:
    """Driver per-data del fit sui prezzi, procedura standard del fitting giornaliero:
    primo giorno multi-start; poi WARM START dai parametri del giorno precedente; se il
    fit warm-started supera retry_rmse_bp si rilancia la ricerca completa (canonical
    starts) e si tiene il migliore. Stampa avanzamento ogni progress_every date.
    b3=0 anche in cache nei giorni NS. Stesse colonne del pannello yield."""
    import time
    rows, prev = [], None
    t0, n_retry = time.time(), 0
    dts = sorted(day_bonds)
    for k, dt in enumerate(dts, 1):
        cf_t, cf_a, po, dm = day_bonds[dt]
        n = len(po)
        if n < min_bonds:
            continue
        ns = n < min_bonds_nss
        r = fit_nss_prices(cf_t, cf_a, po, dm, x0=prev, ns_only=ns,
                           full_search=(prev is None))
        if r is not None and prev is not None:
            px_try = list(r.x[:3]) + [0.0 if ns else r.x[3]] + list(r.x[4:])
            if _rmse_prices(px_try, cf_t, cf_a, po, dm) > retry_rmse_bp:
                n_retry += 1                       # degradato: ricerca completa
                r2 = fit_nss_prices(cf_t, cf_a, po, dm, x0=prev, ns_only=ns,
                                    full_search=True)
        ...
        rmse = _rmse_prices(px, cf_t, cf_a, po, dm)
        rows.append([dt, *px, n, rmse, "NS" if ns else "NSS"])
        if progress_every and k % progress_every == 0:
            print(f"      [fit NSS prezzi] {k}/{len(dts)} date "
                  f"({time.time()-t0:.0f}s, retry {n_retry})")
    if n_retry:
        print(f"      [fit NSS prezzi] ricerche complete extra (fit degradato): {n_retry}")
    return pd.DataFrame(rows, columns=["date", "b0", "b1", "b2", "b3", "t1", "t2",
                                       "n_bonds", "rmse_bp", "model"]).set_index("date")
