...
def enrich_reference(uni: pd.DataFrame, kind: str, save: bool = True,
    ...
    if kind == "linker":
        ...
        # Derivata da config.MARKETS, non cablata: la versione cablata aveva ES mancante
        # (i Bonos ei non entravano mai nel controllo di ribasamento, e ES00000128D4 e'
        # rimasto con BASE_CPI 100.16968 in base vecchia contro il 78-97 degli altri otto,
        # producendo una base mediana di -104bp per tutta la vita del titolo) e mappava FR
        # su FRCPXTOB, mentre l'OAT€i e' indicizzato a CPTFEMU: i 4 titoli francesi
        # venivano ribasati contro l'indice sbagliato, da cui le quattro chiavi
        # incoerenti (0.8346 / 0.8379 / 0.8644 / 0.8654) invece di una sola.
        euro_cpi = {m: mk.cpi for m, mk in MARKETS.items() if mk.ccy == "EUR"}
...
def fetch_cpi(mkts: list[str] | None = None) -> None:
    for cpi in sorted({MARKETS[c].cpi for c in (mkts or MARKETS)}):
        ...
        _update_wide(f"cpi_{cpi}", {f"{cpi} Index": cpi}, "PX_LAST",
                     floor=pd.Timestamp("1980-01-01"))   # storia CPI lunga: serve al carry,
        # alla stagionalita' a inizio campione (years_history=10 da asof 2004 vuole dal 1994)
        # e a poter validare il base_cpi dei gilt old-style emessi negli anni '80-'90.

FETCH_ORDER = ["ES", "IT", "FR_CPI", "DE", "FR", "UK", "US"]   # dal piu' piccolo: quota graduale
