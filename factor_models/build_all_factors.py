...
def cds_sell_protection(spread, rpv01, in_bps=True):
    """Recipe 1 -- monthly excess return of a fully-collateralised
    SELL-protection position (Palhares 2013; He-Kelly-Manela 2017):

        r_t = S_{t-1}/12  -  dS_t * RPV01

    * S_{t-1}/12  : one month of carry earned for selling protection.
    * -dS_t*RPV01 : mark-to-market (you LOSE when spreads widen).
    Fully collateralised => the result is an excess return.

    spread : monthly spread LEVEL series.  rpv01 : risky annuity (yrs), scalar
    or series (from CDSW).  Used for: CDX_IG, ITRX_MAIN, all PB_* dealer
    baskets (pass the equal-weighted average spread), and the legs of the
    two-leg curve/relative-value trades.
    """
    s = _to_decimal(to_monthly_last(spread), in_bps)
    return s.shift(1) / 12.0 - s.diff() * rpv01


def cds_two_leg(spread_long, spread_short, rpv01_long, rpv01_short,
                k=None, in_bps=True):
    """Recipe 1b -- two-leg CDS position: SELL protection on `long` /
    BUY protection on `short`, RPV01-matched.  Built from Recipe 1.

        r_t = leg(long) - k * leg(short),     k = RPV01_long / RPV01_short

    Used for: MAIN_5/3_FLATTENER (long 5Y, short 3Y), SNR-MAIN
    (long SnrFin, short Main), XOVER/MAIN (long XOVER, short Main).
    """
    leg_l = cds_sell_protection(spread_long,  rpv01_long,  in_bps=in_bps)
    leg_s = cds_sell_protection(spread_short, rpv01_short, in_bps=in_bps)
    if k is None:
        k = rpv01_long / rpv01_short
...
FACTOR_PLAN = {
 ...
 "CDX_IG":            ("A-Credit",   "cds_sell",  "5Y IG; RPV01_5Y_IG. Or KEEP official ER index"),
 ...
 "ITRX_MAIN":         ("A-Credit",   "cds_sell",  "5Y Main; RPV01_5Y_IG. Or KEEP official ER index"),
 "MAIN_5/3_FLATTENER":("A-Credit",   "cds_2leg",  "long 5Y / short 3Y Main, RPV01-matched"),
 "PB_EU_CDS_1Y":      ("A-Credit",   "cds_sell",  "EW avg of 6 EU dealers 1Y; RPV01_1Y"),
 "PB_EU_CDS_5Y":      ("A-Credit",   "cds_sell",  "EW avg of 6 EU dealers 5Y; RPV01_5Y_IG"),
 "PB_US_CDS_1Y":      ("A-Credit",   "cds_sell",  "EW avg of 5 US dealers 1Y; RPV01_1Y"),
 "PB_US_CDS_5Y":      ("A-Credit",   "cds_sell",  "EW avg of 5 US dealers 5Y; RPV01_5Y_IG"),
 "RI_EU":             ("A-Credit",   "keep",      "Industrial A/Baa TR excess return"),
 "SNR-MAIN":          ("A-Credit",   "cds_2leg",  "long SnrFin / short Main, RPV01-matched"),
 "XOVER/MAIN":        ("A-Credit",   "cds_2leg",  "long XOVER / short Main, RPV01-matched"),
 # --- B: Liquidity ----------------------------------------------------------
...
def build():
    ...
    # bbg_cds = src.load_bloomberg("cds")
    ...
    # F["CDX_IG"]   = cds_sell_protection(bbg_cds["CDX_IG_5Y"], src.RPV01_5Y_IG)
    # F["ITRX_MAIN"]= cds_sell_protection(bbg_cds["ITRX_MAIN_5Y"], src.RPV01_5Y_IG)
    ...
    # pb_eu_5y = bbg_cds[[...6 EU dealer 5Y cols...]].mean(axis=1)            # TODO cols
    # F["PB_EU_CDS_5Y"]= cds_sell_protection(pb_eu_5y, src.RPV01_5Y_IG)
    # F["MAIN_5/3_FLATTENER"]= cds_two_leg(bbg_cds["ITRX_5Y"], bbg_cds["ITRX_3Y"],
    #                                      src.RPV01_5Y_IG, rpv01_short=...)   # TODO 3Y RPV01
    # F["SNR-MAIN"]  = cds_two_leg(bbg_cds["SNRFIN_5Y"], bbg_cds["ITRX_5Y"],
    #                              src.RPV01_5Y_SF, src.RPV01_5Y_IG)
    # F["XOVER/MAIN"]= cds_two_leg(bbg_cds["XOVER_5Y"], bbg_cds["ITRX_5Y"],
    #                              src.RPV01_5Y_XO, src.RPV01_5Y_IG)
