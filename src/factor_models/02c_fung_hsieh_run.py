...
# ================================================================
        ...
        def _write_fh_side(path, region, tbl_label, caption_suffix):
            ...
            with open(path, 'w', encoding='utf-8') as f:
                ...
                f.write("{\\footnotesize\\noindent This table reports estimates of the regression "
                        "$r_{i,t}=\\alpha_i+\\beta_1\\,\\textit{S\\&P}_t+\\beta_2\\,\\textit{SC--LC}_t"
                        "+\\beta_3\\,\\textit{BdOpt}_t+\\beta_4\\,\\textit{FXOpt}_t"
                        "+\\beta_5\\,\\textit{ComOpt}_t+\\beta_6\\,\\textit{10Y}_t"
                        "+\\beta_7\\,\\textit{CredSpr}_t+\\varepsilon_{i,t}$, "
                        "where $r_{i,t}$ is the monthly excess return of strategy $i$ and the regressors are the seven "
        ...
        with open(vif_article_path, 'w', encoding='utf-8') as f:
            ...
            f.write("\\begin{minipage}{\\textwidth}\n")
            f.write("{\\footnotesize\\noindent Variance inflation factors of the seven Fung \\& Hsieh (2004) ")
        ...
        vif_json = {s: vif_storage[s]['EUR'].set_index('Factor')['VIF'].to_dict()
                    for s in article_strategies
                    if s in vif_storage and 'EUR' in vif_storage[s]}
        with open(TABLES_DIR / f"FungHsieh_VIF_{REGRESSION_FREQ}.json", 'w', encoding='utf-8') as jf:
            json.dump(vif_json, jf, indent=2)
        print(f"💾 VIF JSON: FungHsieh_VIF_{REGRESSION_FREQ}.json")
