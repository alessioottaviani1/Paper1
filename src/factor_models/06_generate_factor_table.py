...
FACTOR_INFO = {
    ...
    "5Y5Y_INFL": ("F", "Forward inflation factor (euro area). Mark-to-market excess return on a long-inflation position in a 5y--5y forward zero-coupon inflation swap (annuity-weighted change in the forward breakeven).", r"\citet{haubrich2012inflation}"),
    "5Y5Y_INFL_US": ("F", "Forward inflation factor (United States), expressed in euro. Mark-to-market excess return on a long-inflation position in a 5y--5y forward zero-coupon inflation swap.", r"\citet{haubrich2012inflation}"),
    ...
    "R2_EU": ("F", "Excess return on the 2-year German government bond (Schatz).", r"\citet{duarte2007risk}"),
    "R5_EU": ("F", "Excess return on the 5-year German government bond (Bobl).", r"\citet{duarte2007risk}"),
    "R10_EU": ("F", "Excess return on the 10-year German Bund.", r"\citet{duarte2007risk}"),
...
def main():
    ...
    # FACTOR_INFO is keyed on the parquet's own column names, which for the Fung-Hsieh
    # trend-following straddles are the literature names PTFSBD / PTFSCOM / PTFSFX
    # (see 00_build_all_factors.py). BdOpt / ComOpt / FXOpt are display labels used only
    # in the Fung-Hsieh regression table (02c), so no alias is applied here: aliasing
    # broke the FACTOR_INFO lookup and sent the three factors to a spurious "Panel Z".
    DISPLAY_ALIAS = {}
    missing_info = [f for f in factor_names if DISPLAY_ALIAS.get(f, f) not in FACTOR_INFO]
    if missing_info:
    ...
    for fac in factor_names:
        key = DISPLAY_ALIAS.get(fac, fac)
        if key in FACTOR_INFO:
            panel, desc, ref = FACTOR_INFO[key]
        else:
            panel = "Z"  # Unknown panel, will sort last
            desc = "Description TBD."
            ref = "---"
        panels.setdefault(panel, []).append((key, desc, ref))
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        f.write(r"\label{tab:factor_list} \\[4pt]" + "\n")
        f.write(r"\multicolumn{3}{@{}p{\textwidth}@{}}{\footnotesize " +
