...
with open(latex_path, 'w', encoding='utf-8') as f:
    ...
    f.write("\\item This table reports performance metrics for buy-and-hold and ")
    ...
    if INCLUDE_ROBUSTNESS_TABLE and all_robustness:
        ...
        f.write("\\begin{tablenotes}\n")
        f.write("\\footnotesize\n")
        f.write("\\item This table reports robustness tests with leverage constraints ")
...
# ============================================================================
SLIDE_STRATEGIES_MM = ['CDS_Bond_Basis', 'iTraxx_Combined', 'BTP_Italia']  # paper order
SLIDE_LABELS_MM_LATEX = {
    'CDS_Bond_Basis':  'CDS\\textit{-}Bond Basis',
    'iTraxx_Combined': 'iTraxx \\textit{combined}',
    'BTP_Italia':      'BTP\\textit{-}Italia',
}

slide_path_mm = TABLES_DIR / "MoreiraMuir_Presentation_Slide_monthly.tex"
slide_rows_mm = []
for s in SLIDE_STRATEGIES_MM:
    s_data = results_df[results_df['strategy'] == s]
    if s_data.empty:
        print(f"   ⚠️  {s} not found in results_df, skipping in slide")
        continue
    row = s_data.iloc[0].to_dict()
    row['Strategy_label'] = SLIDE_LABELS_MM_LATEX[s]
    slide_rows_mm.append(row)

slide_df_mm = pd.DataFrame(slide_rows_mm)

with open(slide_path_mm, 'w', encoding='utf-8') as f:
    f.write("% Moreira-Muir Presentation Slide -- 3 paper strategies\n")
    ...
    f.write("Strategy & \\multicolumn{3}{c}{Buy-and-Hold} & "
            "\\multicolumn{3}{c}{Volatility-Managed} & "
            "\\multicolumn{3}{c}{Regression} & N \\\\\n")
    f.write("\\cmidrule(lr){2-4} \\cmidrule(lr){5-7} \\cmidrule(lr){8-10}\n")
    f.write(" & Return & Vol & Sharpe & Return & Vol & Sharpe & "
            "Alpha & Beta & $R^2$ & \\\\\n")
    f.write(" & (\\% p.a.) & (\\% p.a.) & & (\\% p.a.) & (\\% p.a.) & & "
            "(\\% p.a.) & & & \\\\\n")
    f.write("\\midrule\n")
    for _, row in slide_df_mm.iterrows():
        f.write(f"{row['Strategy_label']} & "
                f"{row['bh_return_pct']:.2f} & {row['bh_vol_pct']:.2f} & {row['bh_sharpe']:.2f} & "
                f"{row['vm_return_pct']:.2f} & {row['vm_vol_pct']:.2f} & {row['vm_sharpe']:.2f} & "
                f"{row['alpha_annual_pct']:+.2f} & {row['beta']:.2f} & {row['r_squared']:.3f} & "
                f"{int(row['n_obs'])} \\\\\n")
    f.write("\\bottomrule\n")
    f.write("\\end{tabular}\n")
    f.write("\\begin{tablenotes}\n")
    f.write("\\tiny\n")
    f.write("\\item Volatility-managed portfolios scale exposure inversely with realized "
            "variance so that unconditional volatility matches buy-and-hold. Regression "
            "alpha measures the volatility-timing premium from "
            "$r^{VM}_{t+1} = \\alpha + \\beta r_{t+1} + \\varepsilon_{t+1}$.\n")
    f.write("\\end{tablenotes}\n")
    f.write("\\end{threeparttable}\n")

print(f"💾 Slide MM salvato: {slide_path_mm.name}")
