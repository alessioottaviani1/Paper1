...
def generate_tex_thesis(all_subperiod, all_threshold, framework_key):
    ...
    cap = {'Duarte': r'Duarte et al.\ (2007)', 'ActiveFI': r'Brooks et al.\ (2020)',
           'FungHsieh': r'Fung \& Hsieh (2004)'}[framework_key]
    tex.append(rf"\caption{{Sub-Period and Regime Analysis: {cap}}}")
    ...
    tex.append(rf"{{\footnotesize\noindent Annualized alpha (\% p.a.) of the three strategies under the "
               ...
               r"Panel A: OLS with Newey--West HAC standard errors (4 lags) on time sub-samples. "
...
# Etichette tipografiche dei fattori: le stesse delle tabelle di regressione
# Duarte / Fung-Hsieh / Brooks, cosi' la VIF non espone i nomi di pipeline.
VIF_FACTOR_LABELS = {
...
def write_combined_vif_table():
    ...
    tex.append(r"{\footnotesize\noindent Variance inflation factors of the EUR factors of the three "
               ...
               r"estimation sample; conventional caution thresholds are 5 and 10. "
               r"Elevated VIF for $R_2$, $R_5$, $R_{10}$ (Panel A) reflects the "
               r"inherent correlation of maturity-sorted government bond returns; elevated VIF for "
               r"GlobTerm and GlobAgg (Panel C) reflects the strong correlation "
...
def main():
    ...
    with open(synthesis_path, 'w', encoding='utf-8') as f:
        ...
        f.write("with Newey--West HAC standard errors (4 lags) using EUR factors. ")
        ...
        # ── Panel C: Out-of-sample alpha (resolved at COMPILE time) ──
        # \input pulls the latest panel_d_oos_alpha.tex (written by 07_oos_alpha.py)
        # when LaTeX runs, so the execution order of 03 and 07 is irrelevant.
        f.write("\\input{\\tablepath/panel_d_oos_alpha}\n")
        f.write("\\end{table}\n")
