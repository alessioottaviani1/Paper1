...
def analysis_ferson_schadt(strategy, framework, region, freq, stress_monthly):
    ...
    return {
        ...
        'alpha1_pval': alpha1_p,          # block-bootstrap p (HEADLINE)
        'alpha1_pval_hac': alpha1_p_hac,  # HAC p at NW1994 lag (robustness only)
...
def main():
    ...
    with open(synthesis_path, 'w', encoding='utf-8') as f:
        ...
        f.write("\\small\n\n")
        f.write("\\makeatletter\\@ifundefined{panelCbox}{\\newsavebox{\\panelCbox}\\newlength{\\panelCwd}}{}\\makeatother\n\n")
        ...
        f.write("\\vspace{0.4cm}\n")
        f.write("\\begin{center}\\textit{Panel C: Christopherson, Ferson, and Glassman (1998) conditional alpha (\\% p.a.)}\\end{center}\n")
        f.write("\\vspace{0.1cm}\n\n")
        f.write("\\sbox{\\panelCbox}{%\n")
        f.write("\\begin{tabular}{l" + " r r r" * n_fw + "}\n")
        f.write("\\toprule\n")
        ...
        f.write("\\bottomrule\n")
        f.write("\\end{tabular}}%\n")
        f.write("\\setlength{\\panelCwd}{\\wd\\panelCbox}%\n")
        f.write("\\noindent\\makebox[\\linewidth][c]{\\usebox{\\panelCbox}}\n\n")

        # ── Panel D: Out-of-sample alpha (inlined from 07_oos_alpha.py output) ──
        ...
        # ── Notes (width = Panel C) ──
        f.write("\\noindent\\makebox[\\linewidth][c]{\\begin{minipage}{\\panelCwd}\n")
        ...
        f.write("Panel C reports the intercept $\\alpha_0$ and the stress-loading ")
        f.write("coefficient $\\alpha_1$ from the \\citet{christopherson1998conditioning} ")
        f.write("full conditional model ")
        f.write("$r_t = \\alpha_0 + \\alpha_1 z_t + (\\beta + \\delta z_t)' X_t + \\varepsilon_t$, ")
        f.write("in which both the alpha and the factor loadings are conditioned on $z_t$, ")
        f.write("the standardized iTraxx Main 5Y spread; $\\bar{R}^2$ is the adjusted $R^2$. ")
        f.write("Significance of $\\alpha_1$ is assessed by a moving-block bootstrap ")
        f.write("($B=9{,}999$, $H_0{:}\\,\\alpha_1=0$ imposed, block length~9); Newey--West HAC ")
        f.write("$t$-statistics are in parentheses, with HAC-based $p$-values yielding identical conclusions. ")
        f.write("Panel D reports out-of-sample alpha: loadings estimated on an ")
        f.write("expanding past-only window (60-month burn-in), with the realized ")
        f.write("abnormal return tested by Newey--West HAC and $N$ the number of ")
        f.write("out-of-sample months; the rolling-window counterpart is in Appendix~A.7. ")
    ...
    with open(presentation_path, 'w', encoding='utf-8') as f:
        ...
        # ---- RIGHT: Conditional Alpha (Ferson-Schadt) ----
        f.write("\\begin{column}{0.5\\textwidth}\n")
        f.write("\\centering\n\\scriptsize\n\\setlength{\\tabcolsep}{4pt}\n\n")
        ...
        f.write(f" {{\\scriptsize\\textit{{(HIGH: iTraxx Main 5Y $>$ {DEFAULT_THRESHOLD} bps)}}}}")
        f.write("\\\\[4pt]\n")
        n_fw = len(FRAMEWORKS)
        f.write("\\begin{tabular}{l" + "@{\\hspace{8pt}}cc" * n_fw + "}\n")
        f.write("\\toprule\n")
        ...
        for strategy in slide_strategies:
            ...
            for fw_key in FRAMEWORKS:
                ...
                if rn:
                    ...
                    f.write(" & --")
                # HIGH cell (highlighted in red)
                if rh:
                    stars_h = significance_stars(rh['alpha_pval'])
                    if stars_h:
                        f.write(f" & \\hlt{{${rh['alpha_ann']:+.2f}^{{{stars_h}}}$}}")
                    else:
                        f.write(f" & \\hlt{{{rh['alpha_ann']:+.2f}}}")
                else:
                    f.write(" & --")
            f.write(" \\\\\n")
        f.write("\\bottomrule\n")
