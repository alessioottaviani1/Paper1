...
def generate_tex_thesis(all_subperiod, all_threshold, framework_key):
    ...
    tex.append(rf"\footnotesize\textit{{Notes:}} "
               ...
               r"Panel A: OLS with HAC (Newey--West) standard errors on time sub-samples. "
               ...
               r"$N$ = months per regime; HAC (Newey--West) $t$-statistics. "
...
def main():
    ...
    with open(synthesis_path, 'w', encoding='utf-8') as f:
            ...
            for fw_key in FRAMEWORKS:
                ...
                for reg in ('LOW', 'MEDIUM', 'HIGH'):
                    ...
                    if r is None or r.get('skip'):
                        ...
                        st = significance_stars(r['alpha_pval'])
                        if st:
                            f.write(f" & ${r['alpha_ann']:+.2f}^{{{st}}}$")
                        else:
                            f.write(f" & {r['alpha_ann']:+.2f}")
            f.write(" \\\\\n")
        ...
        f.write("Panel B reports regime alphas from common-beta dummy intercepts, ")
        f.write("with LOW / MEDIUM / HIGH defined on the 5-year iTraxx Europe Main level ")
        f.write(f"(LOW $<{LOW_CUT}$, MEDIUM $[{LOW_CUT},{HIGH_CUT})$, HIGH $\\geq {HIGH_CUT}$ bps). ")
        ...
        f.write("$t$-statistics in parentheses are Newey--West HAC. ")
