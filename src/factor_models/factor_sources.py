...
def load_martin(filename, **kwargs):
    ...
    # Il CSV ha una colonna numero-riga ("Unnamed: 0") PRIMA della vera 'date';
    # _index_by_date usa columns[0] e indicizzerebbe su quegli interi (letti come
    # nanosecondi dal 1970), rompendo silenziosamente ogni allineamento mensile a valle.
    # Scarto le colonne unnamed e indicizzo esplicitamente su 'date'.
    df = df.loc[:, ~df.columns.astype(str).str.match(r"^Unnamed")]
    return _index_by_date(df, date_col=("date" if "date" in df.columns else None))
