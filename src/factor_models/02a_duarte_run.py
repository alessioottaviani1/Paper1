...
if all_results_us or all_results_eur:
    ...
    if combined_results:
        ...
        # ---- Helper: one single-currency Duarte table (landscape) ----
        def _write_duarte_side(path, region, tbl_label, caption_suffix):
            ...
            with open(path, 'w', encoding='utf-8') as f:
                ...
                f.write(f"\\caption{{Duarte Factor Model Regressions{caption_suffix}}}\n")
                f.write(f"\\label{{{tbl_label}}}\n")
                f.write("\\begin{minipage}{\\textwidth}\n")
                f.write("{\\footnotesize\\noindent This table reports estimates of the regression "
                        "$r_{i,t}=\\alpha_i+\\beta_1\\,\\textit{Mkt}_t+\\beta_2\\,\\textit{SMB}_t"
                        "+\\beta_3\\,\\textit{HML}_t+\\beta_4\\,\\textit{UMD}_t"
                        "+\\beta_5\\,R_{S,t}+\\beta_6\\,R_{I,t}+\\beta_7\\,R_{B,t}"
                        "+\\beta_8\\,R_{2,t}+\\beta_9\\,R_{5,t}+\\beta_{10}\\,R_{10,t}"
                        "+\\varepsilon_{i,t}$, "
                        "where $r_{i,t}$ is the monthly excess return of strategy $i$ and the regressors are the ten "
                        f"Duarte et al.\\ (2007) {factors_desc} equity and bond risk factors. ")
                f.write("$\\alpha$ is annualized (\\% p.a.). $t$-statistics in parentheses "
                        "(Newey--West HAC, $\\lfloor 4(T/100)^{2/9}\\rfloor = 4$ lags). ")
                if region == "EUR":
                    f.write("$Mkt$: European value-weighted market excess return over the 1-month German bill; ")
                    f.write("$SMB$, $HML$, $UMD$: the Fama--French European size, value, and momentum factors. ")
                    f.write("$R_S$: excess return on the EuroStoxx Banks index. ")
                    f.write("$R_I$, $R_B$: excess returns on European A-rated industrial and European bank bond indices. ")
                    f.write("$R_2$, $R_5$, $R_{10}$: excess returns on European 2-, 5-, and 10-year government bond portfolios. ")
                else:
                    f.write("$Mkt$, $SMB$, $HML$, $UMD$: the Fama--French market, size, value, and momentum factors. ")
                    f.write("$R_S$: U.S. bank stock excess return. ")
                    f.write("$R_I$, $R_B$: A/BAA-rated U.S. industrial and bank bond excess returns. ")
                    f.write("$R_2$, $R_5$, $R_{10}$: maturity-sorted U.S. Treasury portfolio excess returns. ")
                ...
                f.write("\\begin{tabular*}{\\textwidth}{@{\\extracolsep{\\fill}}lcccccccccccc}\n\\toprule\n")
                f.write(" & $\\alpha$ (\\% p.a.) & $\\beta_{Mkt}$ & $\\beta_{SMB}$ & $\\beta_{HML}$ & $\\beta_{UMD}$"
                        " & $\\beta_{R_S}$ & $\\beta_{R_I}$ & $\\beta_{R_B}$ & $\\beta_{R_2}$ & $\\beta_{R_5}$"
                        " & $\\beta_{R_{10}}$ & $\\bar{R}^2$ \\\\\n\\midrule\n")
        ...
        _write_duarte_side(TABLES_DIR / f"Duarte_article_{REGRESSION_FREQ}.tex", "EUR", "tab:duarte", "")
        _write_duarte_side(TABLES_DIR / f"Duarte_US_article_{REGRESSION_FREQ}.tex", "US", "tab:duarte_us", " (US Factors)")
        print("💾 Duarte: EUR (main) + US (appendice)")
        ...
        with open(vif_article_path, 'w', encoding='utf-8') as f:
            ...
            f.write("\\begin{minipage}{\\textwidth}\n")
            f.write("{\\footnotesize\\noindent Variance inflation factors of the ten Duarte et al.\\ (2007) ")
            ...
            f.write("Elevated VIF for $R_2$, $R_5$, $R_{10}$ reflects the inherent correlation of ")
            f.write("maturity-sorted government bond returns and does not affect inference on alpha.}\n")
            f.write("\\end{minipage}\n\\par\\vspace{6pt}\n")
        ...
        vif_json = {s: vif_storage[s]['EUR'].set_index('Factor')['VIF'].to_dict()
                    for s in article_strategies
                    if s in vif_storage and 'EUR' in vif_storage[s]}
        with open(TABLES_DIR / f"Duarte_VIF_{REGRESSION_FREQ}.json", 'w', encoding='utf-8') as jf:
            json.dump(vif_json, jf, indent=2)
        print(f"💾 VIF JSON: Duarte_VIF_{REGRESSION_FREQ}.json")
