...
# Risk-free: 1-month Euribor from bbg.xlsx (sheet rates_fx, ticker EUR001M Index)
...
# --- Bloomberg loader for the 1-month Euribor risk-free ---
_FM_DIR = next(
...
_rates = load_bloomberg(EURIBOR_SHEET)
if EURIBOR_TICKER not in _rates.columns:
    raise KeyError(f"'{EURIBOR_TICKER}' not found in sheet '{EURIBOR_SHEET}' of bbg.xlsx")
euribor_df = _rates[[EURIBOR_TICKER]].dropna()
euribor_df.columns = ['Euribor1M']
euribor_df.index.name = 'Date'
...
with open(latex_path, 'w', encoding='utf-8') as f:
    ...
    f.write("\\bottomrule\n\\end{tabular}\n\\begin{tablenotes}\n\\footnotesize\n")
    f.write("\\item This table reports the Manipulation-Proof Performance Measure (MPPM) ")
    ...
    if INCLUDE_ALPHA_COMPARISON:
        ...
        if alpha_cols:
            ...
            f.write("\\bottomrule\n\\end{tabular}\n\\begin{tablenotes}\n\\footnotesize\n")
            f.write("\\item This table compares MPPM with alphas from factor model regressions. ")
...
# ============================================================================
# PRESENTATION SLIDE VERSION (3 paper strategies, identical layout to main.tex)
# ============================================================================
SLIDE_STRATEGIES = ['CDS_Bond_Basis', 'iTraxx_Combined', 'BTP_Italia']  # paper order
SLIDE_LABELS_LATEX = {
    'CDS_Bond_Basis':  'CDS\\textit{-}Bond Basis',
    'iTraxx_Combined': 'iTraxx \\textit{combined}',
    'BTP_Italia':      'BTP\\textit{-}Italia',
}

# Build slide_df by filtering results_df on technical strategy names
slide_path = TABLES_DIR / f"MPPM_Presentation_Slide_{freq_label}.tex"
slide_rows = []
for s in SLIDE_STRATEGIES:
    s_data = results_df[results_df['Strategy'] == s]
    if s_data.empty:
        print(f"   ⚠️  {s} not found in results_df, skipping in slide")
        continue
    row = {'Strategy_key': s, 'Strategy_label': SLIDE_LABELS_LATEX[s]}
    for rho in RHO_VALUES:
        rho_data = s_data[s_data['rho'] == rho]
        if rho_data.empty:
            row[f'MPPM_rho{rho}'] = np.nan
        else:
            row[f'MPPM_rho{rho}'] = rho_data['MPPM'].iloc[0]
    rho3_data = s_data[s_data['rho'] == 3].iloc[0]
    row['Avg_Return'] = rho3_data['Avg_Return']
    row['Volatility'] = rho3_data['Volatility']
    row['Sharpe'] = rho3_data['Sharpe']
    row['N_obs'] = rho3_data['N_obs']
...
with open(slide_path, 'w', encoding='utf-8') as f:
    f.write("% MPPM Presentation Slide -- 3 paper strategies\n")
    f.write("% Mirrors the layout of main.tex (threeparttable + tablenotes)\n")
    f.write("% Meant to be \\input{} inside a Beamer frame\n\n")
    f.write("\\begin{threeparttable}\n")
    f.write("\\begin{tabular}{lrrrrrrr}\n")
    f.write("\\toprule\n")
    f.write("Strategy & \\multicolumn{3}{c}{MPPM (\\% p.a.)} & Avg Return & Vol & Sharpe & N \\\\\n")
    ...
    for _, row in slide_df.iterrows():
        f.write(f"{row['Strategy_label']} & "
                f"{row['MPPM_rho2']:.2f} & {row['MPPM_rho3']:.2f} & "
                f"{row['MPPM_rho4']:.2f} & {row['Avg_Return']:.2f} & "
                f"{row['Volatility']:.2f} & {row['Sharpe']:.2f} & "
                f"{int(row['N_obs'])} \\\\\n")
    f.write("\\bottomrule\n")
    f.write("\\end{tabular}\n")
    f.write("\\begin{tablenotes}\n")
    f.write("\\tiny\n")
    f.write("\\item MPPM as in Goetzmann et al.\\ (2007), based on monthly returns "
            "and reported in annualized percentage terms. All moments are annualized.\n")
    f.write("\\end{tablenotes}\n")
    f.write("\\end{threeparttable}\n")

print(f"💾 Slide MPPM salvato: {slide_path.name}")
