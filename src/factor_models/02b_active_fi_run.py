...
FACTOR_NAMES_LATEX = {
    'Term': 'Term',
    'Global_Term': 'GlobTerm',
    'Global_Aggregate': 'GlobAgg',
    'Inflation_Linkers': 'InflLinkers',
    'Corporate_Credit': 'CorpCredit',
    'Emerging_Debt': 'EmergDebt',
    'Emerging_Currency': 'EmergCurr',
    'UST_Volatility': 'USTVol'
}
    ...
    if all_results_us or all_results_eur:
        ...
        if combined_results:
            ...
            with open(latex_presentation_path, 'w', encoding='utf-8') as f:
                ...
                _vol_lbl = "Rates Vol"  # slide: one shared header for both panels (Bund in the EUR panel, 10Y Treasury in the US panel)
                f.write(" & $\\alpha$ (\\% p.a.) & $\\beta_{Term}$ & $\\beta_{Global Term}$ & $\\beta_{Global Agg}$ & $\\beta_{Infl. Linkers}$ & $\\beta_{Corp Credit}$ & $\\beta_{Emerg Debt}$ & $\\beta_{Emerg Curr}$ & $\\beta_{" + _vol_lbl + "}$ & $R^2$ adj & N \\\\\n")
                f.write("\\midrule\n")
...
# ================================================================
            ...
            def _write_afi_side(path, region, tbl_label, caption_suffix):
                ...
                with open(path, 'w', encoding='utf-8') as f:
                    ...
                    f.write("{\\footnotesize\\noindent This table reports estimates of the regression "
                            ...
                            "+\\beta_7\\,\\textit{Emerg Curr}_t+\\beta_8\\,\\textit{" + ("Rates Vol" if region == "EUR" else "UST Vol") + "}_t"
                            "+\\varepsilon_{i,t}$, "
                    ...
                    if region == "EUR":
                        f.write("Global Term: Bloomberg Global Treasury Index (hedged), in excess of cash. ")
                        f.write("Global Agg: Bloomberg Global Aggregate Bond Index (hedged), in excess of cash. ")
                        f.write("Infl.\\ Linkers: global inflation-linked bonds (hedged), in excess of cash. ")
                        f.write("Corp Credit: pan-European high-yield corporate bond index, in excess of cash. ")
                        f.write("Emerg Debt: excess return on hard-currency emerging-market debt (hedged). ")
                        f.write("Emerg Curr: excess return on a basket of emerging-market currencies against the U.S. dollar. ")
                        f.write("Rates Vol: excess return on a one-month variance swap on the ten-year Bund future, in percentage points of annualised variance. ")
                    else:
                        f.write("Global Term: Bloomberg Global Treasury Index (hedged), in excess of cash. ")
                        f.write("Global Agg: Bloomberg Global Aggregate Bond Index, in excess of cash. ")
                        ...
                        f.write("Emerg Curr: excess return on a basket of emerging-market currencies against the U.S. dollar. ")
                        f.write("UST Vol: excess return on a one-month variance swap on the ten-year U.S. Treasury future, in percentage points of annualised variance. ")
                    ...
                    for fac in all_factors:
                        _lbl = ("Rates Vol" if region == "EUR" else "UST Vol") if fac == "UST_Volatility" else FACTOR_NAMES_LATEX.get(fac, fac)
                        f.write(f" & $\\beta_{{\\text{{{_lbl}}}}}$")
                    f.write(" & $\\bar{R}^2$ \\\\\n\\midrule\n")
            ...
            with open(vif_article_path, 'w', encoding='utf-8') as f:
                ...
                f.write("\\midrule\n")
                for factor in ACTIVE_FI_FACTORS:
                    row = r'Rates\_Volatility' if factor == 'UST_Volatility' else factor.replace('_', r'\_')
                    for s in article_strategies:
