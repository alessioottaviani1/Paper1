...
def _cds_rpv01_monthly(target, tenors_spread_d, rpv01_bbg_d, disc_d, months, recovery=0.40):
    ...
    for dt in months:
        ...
        if target in sb and not pd.isna(r):
            try:
                isda[dt] = risky_annuity(dt, sb, target, r, recovery)
            except Exception:
                pass
            # Fallback closed-form (credit triangle) dove il modello ISDA/QuantLib non
            # produce nulla -- es. date pre-Big-Bang (2004-2008) che la convenzione
            # CDS2015 non riesce a prezzare. Il RPV01 e' second-order nella convenzione ed
            # e' standardizzato a valle: abbastanza accurato, e non lascia alcun mese con
            # spread+sconto validi senza annuity.
            if pd.isna(isda.get(dt, np.nan)):
                _T = {"1Y": 1.0, "3Y": 3.0, "5Y": 5.0, "7Y": 7.0, "10Y": 10.0}.get(target)
                _s = sb[target] / 1e4                      # spread bps -> decimale
                if _T is not None and _s > 0:
                    _lam = _s / (1.0 - recovery)           # hazard dal par spread
                    isda[dt] = (1.0 - np.exp(-(_lam + r) * _T)) / (_lam + r)
...
def btp_bund_factor(it_tr, de_tr, it_yld, de_yld, N):
    """BTP-Bund sovereign factor (% per month): duration-matched Italy-minus-Germany government
    total return.  Long the Italian TR index, short the German TR index, the German leg weighted by
    the modified-duration ratio D_IT/D_DE so the position is duration-neutral and isolates the
    sovereign spread.  + = Italian bonds outperform German (spread tightens).  Self-financing
    long-short, so the OLS intercept is a Jensen alpha.

    D is the closed-form (par-bond) modified duration on the benchmark yield, D(y,N) =
    [1 - (1+y)^-N] / y, recomputed each month -- so the weight falls automatically when Italian
    yields spike and Italian duration drops (the regime where the factor is most active); weight set
    ex-ante (t-1).  The analytic ratio matches the empirical (TR-on-dyield) duration to ~1%.
    N = representative bucket maturity (8.5 for 7-10Y, 2.0 for 1-3Y).  it_tr/de_tr: TR index levels;
    it_yld/de_yld: benchmark yields (%).  Ref: pelizzon2016sovereign.
    """
    rit = monthly_return(it_tr) * 100.0
    rde = monthly_return(de_tr) * 100.0
    dmod = lambda y: ((1 - (1 + y / 100.0) ** (-N)) / (y / 100.0)) if (pd.notna(y) and y > 0) else float(N)
    w = (to_monthly_last(it_yld).apply(dmod) / to_monthly_last(de_yld).apply(dmod)).shift(1)
    return rit - w * rde


def variance_swap_payoff(daily_returns, svix2_level, months=1):
    """Recipe 5 -- simple variance-swap excess return (Martin 2017; Carr-Wu 2009):

        r_{t->t+h} = RV_{t->t+h} - SVIX^2_{t,h}      (strike fixed at the START)

    RV : realised SIMPLE variance from daily returns over a trailing `months`-month
    window, ANNUALISED.  svix2_level : Martin's SVIX^2 for the matching horizon
    (ALREADY annualised in his files), taken at the START of the window (month-end
    series lagged by `months`).  Both sides annualised -> identical units.
        months=1 -> 1M (non-overlapping);  months=3 -> 3M (overlapping -> HAC).
    """
    rv_m = (daily_returns.astype(float) ** 2).resample("ME").sum()   # monthly realised variance
    rv   = rv_m.rolling(months).sum() * (12.0 / months)             # trailing window, annualised
    svix2_m = to_monthly_last(svix2_level)
    return rv - svix2_m.shift(months)
...
FACTOR_PLAN = {
 # --- A: Credit -------------------------------------------------------------
 "BTP_BUND":          ("A-Credit",   "keep",      "BCEI9T-BCEG9T TR, duration-matched (benchmark-yield DV01); IK1 too short (2009+)"),
 "BTP_BUND_2Y":       ("A-Credit",   "keep",      "BCEI6T-BCEG6T TR, duration-matched short end (1-3Y, redenomination risk)"),
 ...
 "FX_CARRY":  ("C-Equity", "keep", "AQR Century Currencies Carry (G10; EUR-conv)"),
 "FX_MOM":    ("C-Equity", "keep", "AQR Century Currencies Momentum (G10; EUR-conv)"),
 "COM_CARRY": ("C-Equity", "keep", "AQR Century Commodities Carry (EUR-conv)"),
 "COM_MOM":   ("C-Equity", "keep", "AQR Century Commodities Momentum (EUR-conv)"),
 ...
 # --- D: Volatility ---------------------------------------------------------
 "ATM_IV_CDX":  ("D-Volatility","varswap",  "CDX IG credit varswap: RV(spread) - ATM IV^2, 3M (HAC)"),
 "ATM_IV_ITRX": ("D-Volatility","varswap",  "iTraxx Main credit varswap: RV(spread) - ATM IV^2, 3M (HAC); corr~0.87 w/ ATM_IV_CDX"),
 "SVS_RET_1M":  ("D-Volatility","varswap",  "simple variance-swap return RV(SPX)-SVIX^2 strike, 1M (Martin)"),
 "SVS_RET_3M":  ("D-Volatility","varswap",  "as SVS_RET_1M, 3M horizon (overlapping -> HAC); corr~0.5 w/ 1M"),
 "IV_BUND":     ("D-Volatility","varswap",  "RV(Bund fut) - ATM IV^2, 1M (rates varswap)"),
 "IV_TSY":      ("D-Volatility","varswap",  "RV(UST fut) - ATM IV^2, 1M (rates varswap)"),
 "MOVE":        ("D-Volatility","DROP",     "redundant with IV_TSY (rates vol premium)"),
 ...
 # --- E: Macro --------------------------------------------------------------
 "5Y5Y_INFL":    ("E-Macro","rate_pos", "EUR 5y5y fwd inflation-swap MTM: +annuity*d(rate), annuity=5*DF(10Y); long inflation"),
 "5Y5Y_INFL_US": ("E-Macro","rate_pos", "US 5y5y fwd inflation-swap MTM (as EUR); corr~0.36 w/ EUR -> distinct"),
 # --- F: Rates --------------------------------------------------------------
 "FI_VAL":   ("F-Rates", "keep", "AQR Century Fixed income Value (dev. govt bonds; EUR-conv)"),
 "FI_MOM":   ("F-Rates", "keep", "AQR Century Fixed income Momentum (dev. govt bonds; EUR-conv)"),
 "FI_CARRY": ("F-Rates", "keep", "AQR Century Fixed income Carry (dev. govt bonds; EUR-conv)"),
 "FI_DEF":   ("F-Rates", "keep", "AQR Century Fixed income Defensive (dev. govt bonds; EUR-conv)"),
...
def build():
    ...
    bbg_sw  = src.load_bloomberg("swaps_fx_infl")            # 5y5y inflation swaps + USD 10Y discount
    ...
    # ===== Panel A: Credit =================================================
    # BTP-Bund sovereign factors: duration-matched Italy-minus-Germany government total return
    # (long Italy TR, short Bund TR, German leg weighted to equal modified duration from benchmark
    # yields).  + = Italian bonds outperform German (spread tightens).  TR indices (not IK1-RX1) for
    # full 2004 coverage.  Validated: analytic duration ratio ~= empirical (0.94 vs 0.93).
    F["BTP_BUND"]    = btp_bund_factor(bbg_tr["BCEI9T Index"], bbg_tr["BCEG9T Index"],
                                       bbg_tr["GBTPGR10 Index"], bbg_tr["GDBR10 Index"], 8.5)
    F["BTP_BUND_2Y"] = btp_bund_factor(bbg_tr["BCEI6T Index"], bbg_tr["BCEG6T Index"],
                                       bbg_tr["GBTPGR2 Index"],  bbg_tr["GDBR2 Index"],  2.0)
    ...
    _beta = _rolling_beta(_dX, _dM, window=36, min_periods=12).shift(1)
    ...
    # AQR "Century of Factor Premia" (Ilmanen-Israel-Lee-Moskowitz-Thapar 2021): fattori di stile
    # tradable self-financing long-short per classe di attivo, mensili fino a feb-2026. EUR-converted
    # come GVAL/GMOM. Le sleeve correlano ~0.5 con i compositi all-asset (GVAL/GMOM) -> non ridondanti.
    cofp = src.load_aqr("Century of Factor Premia Monthly.xlsx")
    F["FI_VAL"]    = ghs_longshort(to_monthly_last(cofp["Fixed income Value"])     * 100, r_fx)
    F["FI_MOM"]    = ghs_longshort(to_monthly_last(cofp["Fixed income Momentum"])  * 100, r_fx)
    F["FI_CARRY"]  = ghs_longshort(to_monthly_last(cofp["Fixed income Carry"])     * 100, r_fx)
    F["FI_DEF"]    = ghs_longshort(to_monthly_last(cofp["Fixed income Defensive"]) * 100, r_fx)
    F["FX_CARRY"]  = ghs_longshort(to_monthly_last(cofp["Currencies Carry"])       * 100, r_fx)
    F["FX_MOM"]    = ghs_longshort(to_monthly_last(cofp["Currencies Momentum"])    * 100, r_fx)
    F["COM_CARRY"] = ghs_longshort(to_monthly_last(cofp["Commodities Carry"])      * 100, r_fx)
    F["COM_MOM"]   = ghs_longshort(to_monthly_last(cofp["Commodities Momentum"])   * 100, r_fx)
    ...
    # Simple variance-swap excess return (Martin 2017): TRADABLE counterpart of the SVIX^2
    # LEVEL (a strike/forecast, not a return -> fails Jensen a+b).  RV of the S&P 500 SIMPLE
    # price return minus the SVIX^2 strike fixed at the start of the window.  SPX daily price
    # is in tr_indices ("SPX Index"); SVIX^2 (annualised: 30d->1M, 91d->3M) from Ian Martin.
    spx_daily_ret = bbg_tr["SPX Index"].pct_change()
    martin = src.load_martin("SVIX_till_08292025.csv")
    F["SVS_RET_1M"] = variance_swap_payoff(spx_daily_ret, martin["SVIXsq30"], months=1)
    F["SVS_RET_3M"] = variance_swap_payoff(spx_daily_ret, martin["SVIXsq91"], months=3)

    # --- Credit variance risk premium: ATM_IV_ITRX / ATM_IV_CDX -----------------
    # Delta-hedged ATM straddle (payer+receiver) on the index = credit varswap; computed
    # in variance-swap form  r = RV(spread) - ATM_IV^2  (same machinery as SVS_RET).
    # 3M 100%-moneyness ATM vol (OTR, Barclays) is in the `vol_options` sheet, whose
    # layout load_bloomberg cannot parse -> read directly (col0=Date, col1=iTraxx 100%,
    # col3=CDX.IG 100%).  IV is a LOGNORMAL spread vol in decimals (0.56=56%), so IV^2 is
    # the annualised variance of the spread's log-changes -> match RV with spread LOG returns.
    _civ = pd.read_excel(src.BBG_XLSX, sheet_name="vol_options", header=None, skiprows=8,
                         usecols=[0, 1, 3], names=["Date", "IV_ITRX", "IV_CDX"])
    _civ["Date"] = pd.to_datetime(_civ["Date"], errors="coerce")
    _civ = (_civ.dropna(subset=["Date"]).set_index("Date")
                .apply(pd.to_numeric, errors="coerce").sort_index())

    def _otr_logret(spread_d, series_d):
        # daily log-changes of the on-the-run spread, roll-day move removed (the index roll
        # is a composition change, not a market move).  LOG returns to match the lognormal
        # ATM-vol convention -- NOT the additive _cds_roll_adjust (that is for bp changes).
        r = np.log(spread_d.astype(float)).diff()
        if series_d is not None and not series_d.dropna().empty:
            roll = (series_d.ne(series_d.shift(1)) & series_d.notna() & series_d.shift(1).notna())
            r = r.mask(roll.reindex(r.index, fill_value=False))
        return r

    F["ATM_IV_ITRX"] = variance_swap_payoff(_otr_logret(SPR[Ix["MAIN5"]], _ser(Ix["MAIN5"])),
                                            (_civ["IV_ITRX"]) ** 2, months=3)
    F["ATM_IV_CDX"]  = variance_swap_payoff(_otr_logret(SPR[Ix["CDXIG"]], _ser(Ix["CDXIG"])),
                                            (_civ["IV_CDX"]) ** 2, months=3)
    ...
    # IV_BUND / IV_TSY: rates variance risk premium = the delta-hedged ATM straddle return,
    # which equals the variance-swap payoff RV(future) - implied_variance.  We use the
    # variance-swap representation (same VRP, same machinery as SVS_RET).  The 30-day 100%-
    # moneyness IV for RX1/TY1 sits in the `futures` sheet under a blank BDH ticker, so
    # load_bloomberg cannot expose it -> read directly (cols 45=Dates, 47=RX1 30d, 49=TY1 30d).
    # IV is PRICE vol in %, so (IV/100)^2 is the annualised variance of the future's returns.
    _iv = pd.read_excel(src.BBG_XLSX, sheet_name="futures", header=None, skiprows=6,
                        usecols=[45, 46, 47], names=["Date", "IV_RX_30D", "IV_TY_30D"])
    _iv["Date"] = pd.to_datetime(_iv["Date"], errors="coerce")
    _iv = (_iv.dropna(subset=["Date"]).set_index("Date")
              .apply(pd.to_numeric, errors="coerce").sort_index())
    F["IV_BUND"] = variance_swap_payoff(bbg_fut["RX1 Comdty"].pct_change(),
                                        (_iv["IV_RX_30D"] / 100.0) ** 2, months=1)
    F["IV_TSY"]  = variance_swap_payoff(bbg_fut["TY1 Comdty"].pct_change(),
                                        (_iv["IV_TY_30D"] / 100.0) ** 2, months=1)

    # ===== Panel E: Macro =================================================
    # 5y5y forward inflation-swap MTM excess return (Recipe 4), LONG inflation.  A ZC inflation
    # swap has one cashflow at year 10, so its rate-sensitivity (annuity) ~ 5*DF(10Y) (5 = the
    # 5y-10y accrual window).  DF from the LONGEST nominal 10Y (EUSA10 / USGG10YR, both from
    # 2004) so the factor is not truncated; the discount level is second-order (factor is
    # standardised).  Forward-starting -> no near-term realised-inflation accrual -> clean MTM
    # (no CPI / seasonality).  Rate is in %, so /100 -> decimal (in_bps=False); *100 puts the
    # return back in % (as all return factors).  Leading minus flips the helper's receiver sign
    # to RECEIVE-inflation: the factor rises when the breakeven rises.
    _ann_eur = 5.0 * (1.0 + to_monthly_last(bbg_rates["EUSA10 Curncy"]) / 100.0) ** (-10)
    _ann_us  = 5.0 * (1.0 + to_monthly_last(bbg_sw["USGG10YR Index"])   / 100.0) ** (-10)
    F["5Y5Y_INFL"]    = -rate_change_position_return(bbg_sw["FWISEU55 Index"] / 100.0, _ann_eur, in_bps=False) * 100.0
    F["5Y5Y_INFL_US"] = -rate_change_position_return(bbg_sw["FWISUS55 Index"] / 100.0, _ann_us,  in_bps=False) * 100.0
    ...
    # --- Swap-spread (ASW) factors: realized monthly excess return per unit DV01 (bp), carry +
    #     roll-down + MtM incl. (tradable; intercept = Jensen's alpha).  Long bond future + pay-fixed
    #     constant-maturity EUR swap (vs 6M Euribor), DV01-matched.  + = ASW spread widens.
    #     PRIMARY = exact QuantLib dual-curve full revaluation of the aging swap (single-curve pre-2007,
    #     dual-curve OIS post-2007); swap_spread_closed_form is the convention-free robustness check
    #     (agreement corr 0.973/0.996/0.999 for 2Y/5Y/10Y).  govt long-bond leg = -_dy[stub] (in helper).
    from swap_spread_reval import swap_spread_reval        # QuantLib full reval (importable; QuantLib req.)
    F["SS2Y"]  = swap_spread_reval(bbg_rates, _dy["DU"],  2)
    F["SS5Y"]  = swap_spread_reval(bbg_rates, _dy["OE"],  5)
    F["SS10Y"] = swap_spread_reval(bbg_rates, _dy["RX"], 10)
