...
def _agg_stat(magnitudes):
    """Aggrega un vettore di magnitudini (>=0) con la statistica scelta in config."""
    x = np.asarray(magnitudes, dtype=float)
    x = x[~np.isnan(x)]
    if len(x) == 0:
        return np.nan
    if MISPRICING_AGG_STAT == "mean":
        return float(np.mean(x))
    if MISPRICING_AGG_STAT == "median":
        return float(np.median(x))
    if MISPRICING_AGG_STAT == "trimmed":
        from scipy import stats as _st
        return float(_st.trim_mean(x, TRIM_PCT))
    raise ValueError(f"MISPRICING_AGG_STAT non valido: {MISPRICING_AGG_STAT}")


def _agg_stat_rows(df):
    """Applica MISPRICING_AGG_STAT per riga di un DataFrame, ignorando i NaN.
    Ritorna una Series. Usato per aggregare le sub-skew iTraxx ribilanciate."""
    if MISPRICING_AGG_STAT == "mean":
    ...
    if MISPRICING_AGG_STAT == "trimmed":
        ...
        def _trim_row(row):
            ...
            if len(vals) == 0:
                return np.nan
            return float(_st.trim_mean(vals, TRIM_PCT))
        return df.apply(_trim_row, axis=1)
    raise ValueError(f"MISPRICING_AGG_STAT non valido: {MISPRICING_AGG_STAT}")


def aggregate_daily_mispricing(values, strategy):
    """Da basi dei singoli strumenti (con segno) a mispricing m >= 0 del giorno.

    Usa MISPRICING_AGG_METHOD e MISPRICING_AGG_STAT da config.
    Per 'A'/'B' ritorna 0.0 se nessuno strumento qualifica; np.nan se il giorno è vuoto.
    NB: 'today' è gestito a parte nel flusso storico, NON qui.
    """
    v = pd.Series(values).dropna()
    if len(v) == 0:
        return np.nan

    if MISPRICING_AGG_METHOD == "A":
        if strategy == "cds_bond_basis":
            sub = v[v < 0]                       # solo negative = opportunità della strategia
        else:
            sub = v                              # BTP/iTraxx: tutti gli strumenti
        if len(sub) == 0:
            return 0.0
        return _agg_stat(sub.abs().values)

    if MISPRICING_AGG_METHOD == "B":
        thr = AGG_ENTRY_THRESHOLDS[strategy]
        if strategy == "cds_bond_basis":
            sub = v[v < -thr]
        else:
            sub = v[v.abs() > thr]
    ...
    raise ValueError(f"aggregate_daily_mispricing chiamata con metodo '{MISPRICING_AGG_METHOD}'")
...
basis_daily = {}
mispricing_daily_agg = {}   # popolato solo se MISPRICING_AGG_METHOD in ("A","B")

# --- Carica spread indici iTraxx (solo se servono per ITRAXX_AGG="relative") ---
itraxx_spreads = None
if MISPRICING_AGG_METHOD in ("A", "B") and ITRAXX_AGG == "relative":
    print(f"\n📂 iTraxx index spreads (per ITRAXX_AGG='relative')...")
    _sp = pd.read_excel(
        ITRAXX_SPREADS_FILE, skiprows=ITRAXX_SPREADS_SKIPROWS,
        usecols=ITRAXX_SPREADS_COLS, header=None, names=ITRAXX_SPREADS_NAMES,
    )
    _sp["Date"] = pd.to_datetime(_sp["Date"], errors="coerce")
    _sp = _sp.dropna(subset=["Date"]).set_index("Date").sort_index()
    for c in ["Xover", "Main", "SnrFin", "SubFin"]:
        _sp[c] = pd.to_numeric(_sp[c], errors="coerce")
    itraxx_spreads = _sp
    print(f"   {len(itraxx_spreads)} giorni, "
          f"{itraxx_spreads.index.min().strftime('%Y-%m-%d')} → "
          f"{itraxx_spreads.index.max().strftime('%Y-%m-%d')}")
    print(f"   Spread medi: " + ", ".join(
        f"{c}={itraxx_spreads[c].mean():.0f}" for c in ["Main","SnrFin","SubFin","Xover"]))
...
if BASIS_APPROACH == "market":
    ...
    btp_mispr_agg = pd.Series(index=btp_basis_wide.index, dtype=float)
    ...
    for date in btp_basis_wide.index:
        ...
        if eligible_cols:
            day_vals = btp_basis_wide.loc[date, eligible_cols]
            btp_filtered_mean[date] = day_vals.mean()          # con segno, per il plot
            if MISPRICING_AGG_METHOD in ("A", "B"):
                btp_mispr_agg[date] = aggregate_daily_mispricing(day_vals, "btp_italia")
    ...
    if MISPRICING_AGG_METHOD in ("A", "B"):
        mispricing_daily_agg["btp_italia"] = btp_mispr_agg.dropna()
    ...
    cds_mispr_agg = pd.Series(index=pd.DatetimeIndex(all_dates), dtype=float)
    n_bonds_used = []
    ...
    for date in all_dates:
        ...
        if MISPRICING_AGG_METHOD in ("A", "B"):
            cds_mispr_agg[date] = aggregate_daily_mispricing(day_data, "cds_bond_basis")
    ...
    if MISPRICING_AGG_METHOD in ("A", "B"):
        mispricing_daily_agg["cds_bond_basis"] = cds_mispr_agg.dropna()
    ...
    # ==================================================================
    # iTraxx Skew — media delle 4 on-the-run
    # ==================================================================
    ...
    if itraxx_all_means:
        ...
        itraxx_cs_mean = itraxx_combined_df.mean(axis=1).dropna()   # con segno, per il plot
        basis_daily["itraxx_combined"] = itraxx_cs_mean

        if MISPRICING_AGG_METHOD in ("A", "B"):
            # itraxx_combined_df: colonne = ['Main','SnrFin','SubFin','Xover'] (skew con segno)
            sub = itraxx_combined_df.copy()
            sub_cols = list(sub.columns)
            print(f"\n   iTraxx aggregation method: ITRAXX_AGG='{ITRAXX_AGG}'")

            # ---- STEP 1: matrice ribilanciata per-sub-indice, sulla serie COMPLETA ----
            # rb[c] = contributo ribilanciato (>=0) del sub-indice c. Calcolato su
            # tutta la storia (z-score/relative riflettono la scala tipica del
            # sub-indice, NON solo i giorni attivi). 'sub_g' = skew GREZZO allineato
            # a rb (per la maschera B in bps).
            if ITRAXX_AGG == "raw":
                rb = sub.abs()
                sub_g = sub

            elif ITRAXX_AGG == "zscore":
                rb = pd.DataFrame(index=sub.index, columns=sub_cols, dtype=float)
                for c in sub_cols:
                    s = sub[c]
            ...
            elif ITRAXX_AGG == "relative":
                if itraxx_spreads is None:
                    raise ValueError("ITRAXX_AGG='relative' ma spread non caricati")
                common = sub.index.intersection(itraxx_spreads.index)
                print(f"   relative: overlap skew↔spread = {len(common)} giorni "
                      f"(skew {len(sub)} gg, spread {len(itraxx_spreads)} gg)")
                if len(common) < 0.5 * len(sub):
                    print("   ⚠️ ATTENZIONE: overlap < 50% — verificare allineamento date!")
                sp_a = itraxx_spreads.loc[common, sub_cols]
                sp_safe = sp_a.where(sp_a > 1e-6, np.nan)
                rb = sub.loc[common].abs() / sp_safe
                sub_g = sub.loc[common]          # grezzo allineato (per maschera B)

            elif ITRAXX_AGG == "main_only":
                if "Main" not in sub_cols:
                    raise ValueError("ITRAXX_AGG='main_only' ma colonna 'Main' assente")
                rb = sub[["Main"]].abs()
                sub_g = sub[["Main"]]

            else:
                raise ValueError(f"ITRAXX_AGG non valido: {ITRAXX_AGG}")

            # ---- STEP 2: maschera B (selezione opportunità attive) sul GREZZO in bps ----
            if MISPRICING_AGG_METHOD == "B":
                itrx_thr = AGG_ENTRY_THRESHOLDS["itraxx_combined"]
                if not isinstance(itrx_thr, dict):
                    raise ValueError("Per metodo B, AGG_ENTRY_THRESHOLDS['itraxx_combined'] "
                                     "deve essere un dict per-sub-indice")
                cols_rb = list(rb.columns)
                for c in cols_rb:
                    if c not in itrx_thr:
                        raise ValueError(f"Soglia mancante per sub-indice iTraxx '{c}'")
                mask = pd.DataFrame(index=rb.index, columns=cols_rb, dtype=bool)
                for c in cols_rb:
                    mask[c] = sub_g[c].abs() > itrx_thr[c]
                rb = rb.where(mask, np.nan)
                n_active = rb.notna().sum(axis=1)
                print(f"   [B] sub-skew attive/giorno: media={n_active.mean():.2f}, "
                      f"giorni con 0 attive={int((n_active == 0).sum())}")

            # ---- STEP 3: aggrega per riga; 'tutte NaN/mascherate' → 0 (no opportunità) ----
            row_has_value = rb.notna().any(axis=1)
            itrx_agg = _agg_stat_rows(rb)
            itrx_agg = itrx_agg.where(row_has_value, 0.0).dropna()

            mispricing_daily_agg["itraxx_combined"] = itrx_agg
            print(f"   iTraxx m_agg: {len(itrx_agg)} giorni, "
                  f"media={itrx_agg.mean():.4f}, mediana={itrx_agg.median():.4f}")
...
for name in STRATEGY_NAMES:
    ...
    if MISPRICING_AGG_METHOD in ("A", "B"):
        # m già aggregato in magnitudine (>=0) in STEP 2 — nessuna trasformazione
        m = mispricing_daily_agg[name].copy()
        print(f"\n   {STRATEGY_LABELS[name]}: m_t = {MISPRICING_AGG_STAT} |basis| "
              f"(metodo {MISPRICING_AGG_METHOD})")
    elif transform == "negate_negative_only":
    ...
    # Resample a mensile: mean (media del mese) o last (fine mese), da config
    if BASIS_MONTHLY_AGG == "mean":
        m_monthly = m.resample('ME').mean()
    elif BASIS_MONTHLY_AGG == "last":
        m_monthly = m.resample('ME').last()
    else:
        raise ValueError(f"BASIS_MONTHLY_AGG non valido: {BASIS_MONTHLY_AGG}")
...
# ============================================================================
# STEP 3-bis: WEEKLY VERSION OF MISPRICING (for AR(1) persistence test on P4)
# ============================================================================
# Resample daily mispricing to weekly (Friday close) for finer-grained analysis
# of persistence; useful for AR(1) test (Boyarchenko 2016 / M-P 2012).
print("\n" + "=" * 72)
print("STEP 3-bis: Costruzione mispricing weekly (W-FRI)")
print("=" * 72)
mispricing_weekly = {}
for name in STRATEGY_NAMES:
    m_daily = mispricing_daily[name]
    m_w = m_daily.resample('W-FRI').last()  # Friday close, no average
    m_w = m_w.dropna()
    if BURN_IN_MONTHS > 0:
        # Burn-in: skip first N*4 weeks (~N months)
        m_w = m_w.iloc[BURN_IN_MONTHS * 4:]
    mispricing_weekly[name] = m_w
    print(f"   {STRATEGY_LABELS[name]:25s}: T_w = {len(m_w)} weeks, "
          f"{m_w.index.min().strftime('%Y-%m-%d')} → {m_w.index.max().strftime('%Y-%m-%d')}")

df_mispricing_weekly = pd.DataFrame(mispricing_weekly)
overlap_w = df_mispricing_weekly.notna().all(axis=1)
df_mispricing_weekly_overlap = df_mispricing_weekly.loc[overlap_w].copy()
print(f"\n📊 Weekly overlap: T_w = {len(df_mispricing_weekly_overlap)} weeks "
      f"({df_mispricing_weekly_overlap.index.min().strftime('%Y-%m-%d')} → "
      f"{df_mispricing_weekly_overlap.index.max().strftime('%Y-%m-%d')})")

# Persist for later use (AR(1) in rq3_04)
df_mispricing_weekly_overlap.to_csv(RQ3_DATA_DIR / "mispricing_weekly_overlap.csv")
print(f"💾 mispricing_weekly_overlap.csv")
...
for name in STRATEGY_NAMES:
    ...
    if BASIS_MONTHLY_AGG == "mean":
        ...
        m_monthly = m.resample('ME').last().dropna()
...
# --- iTraxx Main 5Y — unico stress proxy / driver dei regimi — da bbg.xlsx ---
print("\n📂 iTraxx Main 5Y (livelli) da bbg.xlsx...")
...
_cds = load_bloomberg("cds")
stress_proxy_daily["ITRX_MAIN"] = _cds["ITRX EUR CDSI GEN 5Y Corp"].dropna()
print(f"   {len(stress_proxy_daily['ITRX_MAIN'])} obs, "
      f"{stress_proxy_daily['ITRX_MAIN'].index.min().date()} → "
      f"{stress_proxy_daily['ITRX_MAIN'].index.max().date()}")

# --- Resample a mensile ---
stress_proxy_monthly = {}
for pn, series in stress_proxy_daily.items():
    if REGIME_MONTHLY_AGG == "last":
        stress_proxy_monthly[pn] = series.resample('ME').last().dropna()
    elif REGIME_MONTHLY_AGG == "mean":
        stress_proxy_monthly[pn] = series.resample('ME').mean().dropna()
    else:
        raise ValueError(f"REGIME_MONTHLY_AGG non valido: {REGIME_MONTHLY_AGG}")
...
plot_config = {
    ...
    "itraxx_combined": {
        "label": "CDS Index Skew (iTraxx Skew)",
        "color": "#2ca02c",
