...
# --- Input: iTraxx index spreads (livelli, per ITRAXX_AGG="relative") ---
ITRAXX_SPREADS_FILE = DATA_DIR / "raw" / "itraxx.xlsx"
ITRAXX_SPREADS_SKIPROWS = 6          # dati dalla riga 7
ITRAXX_SPREADS_COLS = [0, 1, 2, 3, 4]  # A=Date, B=Xover, C=Main, D=SnrFin, E=SubFin
ITRAXX_SPREADS_NAMES = ["Date", "Xover", "Main", "SnrFin", "SubFin"]
...
# ──────────────────────────────────────────────────────────────────────────
# ⭐ AGGREGAZIONE CROSS-SECTIONAL DEL MISPRICING — due assi indipendenti ⭐
#
# ASSE 1 — MISPRICING_AGG_METHOD: COSA aggreghiamo
#   "today" → comportamento storico (BTP/iTraxx: |media con segno|;
#             CDS: media delle 20 più negative). Ignora ASSE 2.
#   "A"     → magnitudini di TUTTI gli strumenti, senza soglia:
#             BTP/iTraxx: |basis_i| di tutti; CDS: |basis_i| sulle negative.
#   "B"     → magnitudini delle sole OPPORTUNITÀ ATTIVE (oltre soglia di entry):
#             BTP/iTraxx: |basis_i| > soglia; CDS: basis_i < -soglia.
#   In "A"/"B": se nessuno strumento qualifica nel giorno → m = 0.
#
# ASSE 2 — MISPRICING_AGG_STAT: COME aggreghiamo (solo per "A"/"B")
#   "mean"    → media (sensibile, vulnerabile a outlier idiosincratici)
#   "median"  → mediana (robusta a blow-up di singoli emittenti) [consigliato]
#   "trimmed" → media trimmata a TRIM_PCT per coda (via di mezzo)
#
MISPRICING_AGG_METHOD = "A"     # "today" | "A" | "B"
MISPRICING_AGG_STAT   = "mean"    # "mean" | "median" | "trimmed"
TRIM_PCT = 0.10                     # frazione tagliata per coda (solo "trimmed")

# Soglie di entry (bps) usate SOLO dal metodo "B"
# Soglie di entry (bps) usate SOLO dal metodo "B"
# Per iTraxx: soglia per-sub-indice = min(|entry_long|, |entry_short|) dalle
# soglie di trading reali (02b_itraxx_combined.py): Main 5/-10, SnrFin 8/-15,
# SubFin 10/-20, Xover 15/-25 → minima in magnitudine per ciascuno.
# Con metodo "B", ogni sub-skew è filtrata |skew_i| > soglia_i PRIMA del
# ribilanciamento ITRAXX_AGG (filtro in bps, dove le soglie hanno senso).
AGG_ENTRY_THRESHOLDS = {
    "btp_italia":      40,   # |basis| > 40
    "cds_bond_basis":  40,   # basis < -40
    "itraxx_combined": {     # per-sub-indice (min |entry_long|, |entry_short|)
        "Main":   5,
        "SnrFin": 8,
        "SubFin": 10,
        "Xover":  15,
...
# ──────────────────────────────────────────────────────────────────────────
# ⭐ AGGREGAZIONE iTraxx — come trattare l'eterogeneità delle 4 sub-skew ⭐
# Si applica SOLO se MISPRICING_AGG_METHOD in ("A","B"); per "today" è ignorato
# (today resta il baseline storico: |media con segno| delle 4 skew in bp grezzi).
#
#   "raw"       → media delle 4 |skew| in bp grezzi (Xover domina ~50% per scala)
#   "zscore"    → z-score EXPANDING di ogni sub-skew (causale) prima di aggregare
#   "relative"  → skew_i / spread_i (dislocazione relativa, causale) prima di aggregare
#   "main_only" → usa solo Main
#
ITRAXX_AGG = "raw"   # "raw" | "zscore" | "relative" | "main_only"
...
MANUAL_THRESHOLDS = {
    "ITRX_MAIN":  {"low_upper":  60.0, "high_lower": 100.0},
}
...
# --- Aggregazione giornaliero → mensile (last vs mean) ---
# Le basi/mispricing sono prima aggregate cross-section ogni giorno (STEP 2),
# poi collassate a mensile (STEP 3). Questo selettore controlla SOLO il
# collasso temporale (STEP 3): "mean"=media dei giorni del mese (default,
# coerente con la letteratura), "last"=valore dell'ultimo giorno del mese.
BASIS_MONTHLY_AGG = "mean"     # "mean" | "last"

# Selettore per lo stress proxy che definisce il REGIME: "last"=fine mese
# (status quo del resto del paper), "mean"=media mensile (allineato alle basi
# se BASIS_MONTHLY_AGG="mean"; cattura lo stato medio del capitale nel mese).
REGIME_MONTHLY_AGG = "mean"    # "last" | "mean"

DEFAULT_STRESS_PROXY = "ITRX_MAIN"
ALL_STRESS_PROXIES = ["ITRX_MAIN"]
...
# Newey-West (1994) rule-of-thumb truncation lag, floor(4*(T/100)^(2/9)), which
# equals 4 for every monthly sample in the paper (T = 148-243). Kept as a constant
# here because the RQ3 regressions all run on the common monthly panel.
NW_MAX_LAGS = 4
SPANNING_MAX_LAGS = 2
...
#   Channel 1 — Intermediary capital:  HKM (innovazione NON-traded del capital ratio, HKM 2017)
#   Channel 1b — Dealer health:        PB_CDS_5Y_EU (European prime broker CDS)
#   Channel 2 — Funding cost:          LIBOR_OIS (Core) / EURIBOR_OIS (variante in test)
#   Channel 3 — Market illiquidity:    ILLIQ (Amihud, Δ log composite NYU)
PC1_FUNDING_FACTORS = [
    "HKM",             # Intermediary capital — innovazione NON-traded (HKM 2017)
    "LIBOR_OIS",       # Funding cost (global dealer funding in USD)
    "PB_SPR_5Y_EU",    # Dealer health EUR — Δ spread CDS paniere prime broker
    "ILLIQ",           # Market illiquidity — Δ log Amihud composite (NYU)
]

# Varianti del solo canale dealer health, per robustezza (non per selezione)
PC1_FUNDING_ALT = {
    "PB1Y":   ["HKM", "LIBOR_OIS", "PB_SPR_1Y_EU", "ILLIQ"],
    "SNRFIN": ["HKM", "LIBOR_OIS", "SNRFIN_SPR",   "ILLIQ"],
    "TRADED": ["HKM", "LIBOR_OIS", "PB_CDS_5Y_EU", "ILLIQ"],
}


# Panel C: Macroeconomic Placebo Set
# Tests Duffie [2010] / Brunnermeier-Pedersen [2009] prediction P5 against
# the alternative that the common factor PC1 is driven by macro fundamentals.
# If P5 holds, none of these macro proxies should be statistically significant.
PC1_MACRO_PLACEBO = [
    "ΔUM",        # Macroeconomic uncertainty (Ludvigson et al. 2021)
    "ΔUR",        # Real-activity uncertainty (Ludvigson et al. 2021)
    "5Y5Y_INFL",  # Long-term inflation expectations (EUR inflation swap)
    "EPU_EU",     # Economic Policy Uncertainty Europe (Baker et al. 2016)
]
...
def print_config_summary():
    ...
    print(f"  Mispricing agg:        {MISPRICING_AGG_METHOD} / stat={MISPRICING_AGG_STAT}"
          + (f" / trim={TRIM_PCT}" if MISPRICING_AGG_STAT == "trimmed" else ""))
    if MISPRICING_AGG_METHOD in ("A", "B"):
        print(f"  iTraxx agg:            {ITRAXX_AGG}")
