...
# ---------------------------------------------------------------------------
# Test P5 disaggregato: Δm di ciascuna strategia ~ {Core, Macro}.
# Fix incluso: la versione precedente regrediva il PC1 dei RETURNS
# (pc_scores, Sezione E1), mentre testo e Tabella XVIII dichiarano il PC1
# dei Δm. Il PC1(Δm), orientato, resta come colonna di robustezza.
# ---------------------------------------------------------------------------
_dm_std_e1b = StandardScaler().fit_transform(df_dm_clean)
_pc1_dm = pd.Series(PCA(n_components=1).fit_transform(_dm_std_e1b)[:, 0],
                    index=df_dm_clean.index, name="PC1_dm")
if sum(_pc1_dm.corr(df_dm_clean[c]) for c in df_dm_clean.columns) < 0:
    _pc1_dm = -_pc1_dm  # orientamento: PC1 sale quando i mispricing si allargano

p5_dependents = {name: df_dm_full[name].dropna() for name in STRATEGY_NAMES}
p5_dependents["PC1_dm"] = _pc1_dm
...
if pc1_funding_results_all:
    ...
    print(f"\n   💾 T3e2_pc1_funding_regression.csv (per-strategy + PC1_dm)")
