...
# ============================================================================
# SECTION A.0: AR(1) PERSISTENCE OF MISPRICING LEVELS (P4)
# ============================================================================
# Tests P4: persistence of mispricing levels reflects the binding nature of
# intermediary balance-sheet constraints (Boyarchenko 2016, Mitchell-Pulvino 2012).
# Half-life: h* = ln(0.5) / ln(phi) where phi is the AR(1) coefficient on m_t.
# Computed at MONTHLY and WEEKLY frequency for robustness.
# ============================================================================

print("\n" + "=" * 72)
print("SECTION A.0: AR(1) Persistence on Mispricing Levels (P4)")
print("=" * 72)


def _ar1_persistence(series, freq_label, hac_lags):
    """Estimate AR(1) on series; return phi, HAC t-stat, half-life (in periods)."""
    s = pd.Series(series).dropna()
    y = s.iloc[1:].values
    x = sm.add_constant(s.iloc[:-1].values)
    ols = sm.OLS(y, x).fit(cov_type='HAC', cov_kwds={'maxlags': hac_lags})
    phi = float(ols.params[1])
    phi_se = float(ols.bse[1])
    phi_t = float(ols.tvalues[1])
    if 0 < phi < 1:
        ...
        half_life = np.nan
    return phi, phi_se, phi_t, half_life, len(y)


# ── A.0.1: MONTHLY ────────────────────────────────────────────────────────
print("\n--- AR(1) on monthly levels ---")
df_m_clean = df_m.dropna()
ar1_monthly = []
for col in df_m_clean.columns:
    phi, se, t_, hl, n = _ar1_persistence(df_m_clean[col], 'monthly', hac_lags=NW_MAX_LAGS)
    hl_months = hl  # already in months
    ar1_monthly.append({
        'frequency': 'monthly',
        'strategy': STRATEGY_LABELS.get(col, col),
        'phi': phi, 'phi_se': se, 'phi_tstat': t_,
        'half_life_periods': hl,
        'half_life_months': hl_months,
        'N': n,
    })
    print(f"   {STRATEGY_LABELS.get(col, col):25s}: "
          f"φ={phi:.4f} (HAC t={t_:.2f}), "
          f"half-life = {hl_months:.1f} months")

# ── A.0.2: WEEKLY ─────────────────────────────────────────────────────────
print("\n--- AR(1) on weekly levels ---")
weekly_path = RQ3_DATA_DIR / "mispricing_weekly_overlap.csv"
ar1_weekly = []
if weekly_path.exists():
    df_m_weekly = pd.read_csv(weekly_path, index_col=0, parse_dates=True).dropna()
    for col in df_m_weekly.columns:
        phi, se, t_, hl, n = _ar1_persistence(df_m_weekly[col], 'weekly', hac_lags=12)
        hl_months = hl / 4.345 if np.isfinite(hl) else hl  # weeks -> months
        ar1_weekly.append({
            'frequency': 'weekly',
            'strategy': col,  # already labelled in the CSV
            'phi': phi, 'phi_se': se, 'phi_tstat': t_,
            'half_life_periods': hl,
            'half_life_months': hl_months,
            'N': n,
        ...
        print(f"   {col:25s}: "
              f"φ={phi:.4f} (HAC t={t_:.2f}), "
              f"half-life = {hl:.1f} weeks "
              f"≈ {hl_months:.1f} months")
else:
    print(f"   ⚠️ {weekly_path.name} non trovato — esegui prima rq3_01.")

# Save combined
df_ar1 = pd.DataFrame(ar1_monthly + ar1_weekly)
df_ar1.to_csv(RQ3_TABLES_DIR / "T4z_ar1_persistence.csv", index=False)
print(f"\n💾 T4z_ar1_persistence.csv")

# ============================================================================
# SEZIONE A: VAR — Selezione Lag e Stima
# ============================================================================
...
# ─── SLIDE-FRIENDLY VERSION (Beamer 16:9, compact 3×3) ──────────────────
fig_slide, axes_slide = plt.subplots(K, K, figsize=(11, 7.5))
for i, response in enumerate(var_col_names):
    for j, impulse in enumerate(var_col_names):
        ax = axes_slide[i][j]
        periods = np.arange(H + 1)
        ax.plot(periods, girf_values[:, i, j], color='black', linewidth=1.5)
        ax.fill_between(periods, girf_lower[:, i, j], girf_upper[:, i, j],
                        color='steelblue', alpha=0.20)
        ax.axhline(0, color='grey', linewidth=0.6)
        ax.set_title(f"{impulse} $\\to$ {response}", fontsize=10, fontweight='bold')
        ax.grid(True, alpha=0.3)
...
fig_slide.savefig(RQ3_FIGURES_DIR / f"E1_girf_slide.{FIGURE_FORMAT}",
                  dpi=FIGURE_DPI, bbox_inches='tight')
print(f"📊 E1_girf_slide.{FIGURE_FORMAT}  ← SLIDE")
plt.close()
...
# ─── Half-lives from own-response (diagonal) GIRFs ─────────────────────
# Half-life: smallest h s.t. |GIRF_ii(h)| <= 0.5 * |GIRF_ii(0)|
half_life_data = []
for i, strat in enumerate(var_col_names):
    own = girf_values[:, i, i]  # own-response over horizons 0..H
    if abs(own[0]) < 1e-12:
        hl = np.nan
    else:
        threshold = 0.5 * abs(own[0])
        below = np.where(np.abs(own) <= threshold)[0]
        if len(below) > 0:
            hl = float(below[0])  # first horizon at/below threshold
        else:
            # Linear interpolation between last point above and end of horizon
            # (capped at H if shock never decays below 50%)
            hl = float(H)
    half_life_data.append({
        'strategy': strat,
        'half_life': hl,
        'shock_t0': float(own[0]),
        'shock_tH': float(own[-1]),
    })

df_hl = pd.DataFrame(half_life_data).sort_values('half_life')
df_hl.to_csv(RQ3_TABLES_DIR / "T4c_girf_half_lives.csv", index=False)
print(f"💾 T4c_girf_half_lives.csv")
print(f"\n   Half-lives (own-response GIRF):")
for _, r in df_hl.iterrows():
    print(f"      {r['strategy']:25s}: {r['half_life']:.1f} months")
