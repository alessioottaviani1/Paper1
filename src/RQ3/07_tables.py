...
def build_pc1_funding():
    ...
    tex = [r"\begin{table}[H]", r"\centering", r"\singlespacing",
           ...
           r"{\footnotesize\noindent Each column regresses the monthly change in one "
           r"strategy's mispricing, $\Delta m_{i,t}$, on four proxies for the shadow "
           r"cost of intermediary funding: the \citet{he2017intermediary} ratio of "
           r"primary-dealer equity to assets, the LIBOR--OIS spread, the change in "
           r"the five-year CDS spread of a basket of five European prime brokers, and "
           r"the \citet{amihud2015pricing} price-impact measure. The three equations "
           rf"are estimated on the ${_T}$ months for which all three mispricings are "
           r"available, so the coefficients are comparable across columns in sign and "
           r"significance; each mispricing is measured in the units of its own basis, "
           r"so magnitudes are not. Newey--West HAC standard errors. "
           r"$^{***}\,p<0.01$, $^{**}\,p<0.05$, $^{*}\,p<0.10$.}",
