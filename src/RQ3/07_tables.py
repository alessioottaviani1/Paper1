...
def build_spanning_regressions():
    ...
    tex.append(r"{\footnotesize\noindent "
               r"Pairwise regressions $\Delta m_{i,t} = \alpha + \beta\,\Delta m_{j,t} "
               r"+ \varepsilon_t$, where $\Delta m_{i,t}$ is the monthly change in the "
               r"mispricing magnitude of strategy $i$. "
...
def build_ar1_persistence():
    ...
    tex.append(r"{\footnotesize\noindent "
               ...
               r"Newey--West HAC standard errors with six lags. "
               r"$^{***}\,p<0.01$, $^{**}\,p<0.05$, $^{*}\,p<0.10$.}")
    tex.append(r"\end{minipage}")
    tex.append(r"\par\vspace{6pt}")
    tex.append(r"\begin{singlespace}")
...
def build_pc1_funding():
    ...
    tex.append(r"\begin{minipage}{\textwidth}")
    tex.append(r"{\footnotesize\noindent "
               r"OLS regression of the first principal component of the three $\Delta m$ "
               r"series on intermediary stress proxies and on a macro placebo set. "
               r"Panel~A: one proxy per Duffie channel (intermediary capital, funding cost, "
               r"dealer health, market illiquidity). Panel~B: all available funding and "
               r"liquidity proxies. Panel~C: macroeconomic placebo set (macro and real-activity "
               r"uncertainty, long-term inflation expectations, economic policy uncertainty). "
               r"Significant coefficients in Panels~A and~B indicate that the common factor "
               r"driving correlated mispricings is linked to intermediary balance-sheet "
               r"conditions \citep{duffie2010presidential, brunnermeier2009market, "
               r"he2017intermediary}. The non-significant coefficients in Panel~C are consistent "
               r"with the common factor not being driven by macroeconomic fundamentals. "
               r"Newey--West HAC standard errors (4 lags). "
               r"$^{***}\,p<0.01$, $^{**}\,p<0.05$, $^{*}\,p<0.10$.}")
    tex.append(r"\end{minipage}")
    tex.append(r"\par\vspace{6pt}")
...
def build_unified_correlations():
    ...
    tex.append(r"\begin{minipage}{\textwidth}")
    tex.append(r"{\footnotesize\noindent "
               r"$m_{i,t}$ is the market-level mispricing magnitude of strategy $i$ and "
               r"$\Delta m_{i,t}$ its monthly change. Panel~A reports correlations of monthly "
               r"strategy returns; Panel~B, of the $\Delta m$ series. "
               ...
               r"Main 5Y level (LOW $<60$, MEDIUM $[60,100)$, HIGH $\geq 100$ bps); stars from a "
               r"two-sided $t$-test with $n-2$ degrees of freedom. HIGH$^{\mathrm{FR}}$ applies the "
               r"\citet{forbes2002no} adjustment for the higher return variance in the HIGH regime, "
               r"so that the rise in correlation is not a heteroskedasticity artifact. "
               r"$^{***}\,p<0.01$, $^{**}\,p<0.05$, $^{*}\,p<0.10$.}")
    tex.append(r"\end{minipage}")
    tex.append(r"\par\vspace{6pt}")
    tex.append(r"\begin{singlespace}")
    ...
    if df_uncond is not None:
        ...
        if len(df_inst) > 0:
            ...
            p1_evidence = (rf"$\rho_{{\Delta m}} = {row['pearson_r']:+.2f}{_stars(row.get('pearson_p_hac',1))}$ "
                           rf"(CDS--Bond Basis vs iTraxx Combined)")
