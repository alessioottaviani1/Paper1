...
def main():
    ...
    def _fmt(a, t, p, ir): return f"{a:+.2f}{_stars(p)} ({t:.2f}, IR {ir:+.2f})"
    ...
    for sn in STRATEGIES:
        if sn not in exp and sn not in rol:
            continue
        ce = _fmt(exp[sn][1], exp[sn][2], exp[sn][3], exp[sn][4]) if sn in exp else "--"
        cr = _fmt(rol[sn][1], rol[sn][2], rol[sn][3], rol[sn][4]) if sn in rol else "--"
