...
def generate_alpha_regime_synthesis():
    ...
    with open(tex_path, 'w', encoding='utf-8') as f:
        ...
        f.write("\\footnotesize\n")
        f.write("\\item \\textit{Note:} ")
        f.write("Annualized alpha (\\% p.a.) per stress regime, from a single "
                "regression with three intercept dummies and betas common across "
                "regimes (no constant), Newey--West HAC standard errors. "
                "PCA: spanning regression on 8 rolling principal components. "
                "Best-Subset: post-selection OLS on the $\\ell_0$-selected factors. "
