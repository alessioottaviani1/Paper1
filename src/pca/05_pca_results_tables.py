...
import matplotlib
matplotlib.rcParams['pdf.fonttype'] = 42   # TrueType embedding (no Type 3) for print submission
...
def generate_latex_tables(all_results: dict):
    ...
    with open(latex_path, 'w') as f:
        ...
        for panel_idx, timing in enumerate(timings):
            ...
            _disp = ("Predictive ($PC_t \\to R_{t+1}$)" if timing == 'predictive'
                     else "Contemporaneous ($PC_t \\to R_t$)" if timing == 'contemporaneous'
                     else timing_label)
            panel_title = (_disp if len(timings) == 1
                           else f"Panel {chr(65 + panel_idx)}: {_disp}")
...
def generate_variance_timeseries(all_results: dict):
    ...
    ax.set_title(f'Cumulative Variance Explained by First {PCA_N_COMPONENTS} PCs', fontsize=12, fontweight='bold')
...
def generate_top_loadings_table(all_results: dict, n_pcs: int = 3, n_top: int = 6):
    ...
    with open(tex_path, 'w') as f:
        f.write(f"% {'='*76}\n")
        f.write(f"% PCA TOP FACTOR LOADINGS (full-sample eigenvectors)\n")
        f.write(f"% {'='*76}\n\n")
        ...
        f.write(f"{{\\footnotesize\\noindent For each of the first {n_pcs} principal components "
                f"of the standardized candidate factor panel, the {n_top} factors with the largest "
                "absolute loading, with the corresponding eigenvector entry. Loadings are the "
                "full-sample eigenvectors; the sign indicates the direction of the relationship.}\n")
        ...
        for row_idx in range(n_top):
            ...
            for i in range(n_pcs):
                ...
                if row_idx < len(top_loadings[pc_name]):
                    ...
                    cells.append(f"\\texttt{{{factor_clean}}} & {loading:+.2f}")
                else:
                    cells.append("& ")
            f.write(" & ".join(cells) + " \\\\\n")
...
def generate_article_tables(all_results: dict):
    ...
    with open(tex_path, 'w', encoding='utf-8') as f:
        ...
        f.write(f"% Full-sample PCs, K = {n_components}\n")
        f.write("% " + "=" * 74 + "\n\n")
        ...
        f.write("\\begin{minipage}{\\textwidth}\n")
        f.write("{\\footnotesize\\noindent This table reports estimates of the spanning regression "
                "$r_{i,t}=\\alpha_i+\\beta_1\\,PC_{1,t}+\\beta_2\\,PC_{2,t}+\\dots"
                f"+\\beta_{{{n_components}}}\\,PC_{{{n_components},t}}+\\varepsilon_{{i,t}}$, "
                "where $r_{i,t}$ is the monthly excess return of strategy $i$ and the regressors are the "
                f"first {n_components} principal components of the standardized candidate factor panel, "
                "extracted over the full estimation sample "
                "(the number of components is selected by the Bai--Ng $IC_{p2}$ criterion). ")
        f.write("$\\alpha$ is annualized (\\% p.a.). $t$-statistics in parentheses "
                "(Newey--West HAC, $\\lfloor 4(T/100)^{2/9}\\rfloor = 4$ lags). "
                "$\\bar{R}^2$ is the adjusted $R^2$; $N$ is the number of monthly observations. ")
        ...
        f.write(f"\\begin{{tabular*}}{{\\textwidth}}{{@{{\\extracolsep{{\\fill}}}}l{'c' * n_data_cols}}}\n")
        f.write("\\toprule\n")
        ...
        if loadings_path.exists():
            ...
            f.write("\\label{tab:pca_loadings}\n")
            f.write("\\begin{minipage}{\\textwidth}\n")
            f.write(f"{{\\footnotesize\\noindent For each of the first {n_pcs_load} principal "
                    f"components, the {n_top} candidate factors with the largest absolute loading, "
                    "with the corresponding eigenvector entry. Loadings are the full-sample "
                    "eigenvectors of the standardized candidate factor panel; the sign indicates "
...
# ============================================================================
def generate_cross_layer_table():
    """Table XII — Alpha Across Empirical Layers (benchmarks / PCA / best-subset).
    Moved here from machine_learning/07 so that, in the folder order
    fm -> ml -> pca, every input (benchmark summaries, PCA spanning JSONs,
    best-subset ols_results) is fresh when this runs."""
    print_header("GENERATING CROSS-LAYER ALPHA TABLE (Table XII)", "-")
    ...
    def _f1(x):  return f"{x:+.1f}"
    def _f2(x):  return f"{x:.2f}"
    def _sup(p):
        s = "***" if p < 0.01 else "**" if p < 0.05 else "*" if p < 0.10 else ""
        return rf"^{{{s}}}" if s else ""
    ...
    bench_files = {
        "Duarte ($k=10$)":     TABLES_DIR / "Duarte_summary_monthly.json",
        "Brooks ($k=8$)":      TABLES_DIR / "ActiveFI_summary_monthly.json",
        "Fung--Hsieh ($k=7$)": TABLES_DIR / "FungHsieh_summary_monthly.json",
    }
    rows = []
    ...
    for s in strat_order:
        r = _load_json(get_strategy_pca_dir(s) / "spanning_regression_results_contemporaneous.json")
        if not r:
            continue
        try:
            ...
            continue
    if pca_cells:
        rows.append((rf"PCA ($k={pca_K}$)" if pca_K else "PCA", pca_cells))

    bs_cells, bs_ks = {}, []
    for s in strat_order:
        d = _load_json(aen_cfg.get_strategy_aen_dir(s) / "ols_results.json")
        if not d:
            continue
        try:
    ...
    if not rows:
        print("   ⚠️  No inputs found, skipping cross-layer table.")
        return None
    ...
    tex = [
        ...
        r"layer: the three established hedge-fund factor benchmarks of Section~\ref{sec:bench_specs}, "
        ...
        r"\begin{singlespace}",
        r"\small",
        r"\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}} l cc cc cc @{}}",
        r"\toprule",
    ...
    for label, cells in rows:
        ...
        for s in strat_order:
            ...
            line += r" & -- & --" if not c else rf" & ${_f1(c['alpha'])}{_sup(c['p'])}$ & {_f2(c['r2'])}"
        tex.append(line + r" \\")
    tex += [r"\bottomrule", r"\end{tabular*}", r"\end{singlespace}", r"\end{table}"]

    out_path = TABLES_DIR / "AEN_Method_Comparison_article.tex"
    out_path.write_text("\n".join(tex) + "\n", encoding="utf-8")
...
def generate_alpha_regime_synthesis():
    """
    Synthesis table comparing PCA vs best-subset alpha across the 3 stress
    regimes (LOW/MEDIUM/HIGH, common-beta dummies on iTraxx Main 5Y) — stessa
    costruzione di factor_models/03 e dei pannelli e2 per pipeline.

    Columns: PCA (LOW, MEDIUM, HIGH) | Best-Subset (LOW, MEDIUM, HIGH)
    t-stats in parentheses on the second row.

    Reads:
      - PCA: results/<strategy>/pca/conditional_dummy_<timing>.json
      - AEN: results/<strategy>/aen/hqc/conditional_dummy_interaction.json
    """
    ...
    # ── Load PCA regime results (3-state common-beta dummies) ──
    pca_regime_data = {}
    for s in strategies:
        s_dir = get_strategy_pca_dir(s)
        for timing in ['contemporaneous', 'predictive']:
            regime_path = s_dir / f"conditional_dummy_{timing}.json"
            if regime_path.exists():
                with open(regime_path, 'r') as f:
                    pca_regime_data[s] = json.load(f)
                print(f"   ✅ PCA regime data: {s} ({timing})")
                break  # use first available timing

    # ── Load best-subset (AEN dir) regime results ──
    aen_regime_data = {}
    for s in strategies:
        aen_dir = aen_cfg.get_strategy_aen_dir(s)
        cs_path = aen_dir / "conditional_dummy_interaction.json"
        if cs_path.exists():
    ...
    def _cells(regimes_dict):
        """alpha row + t-stat row for LOW/MEDIUM/HIGH; '--' if missing/skipped."""
        a_row, t_row = "", ""
        for reg in REGIMES:
            rd = regimes_dict.get(reg, {})
            if rd and not rd.get('skip'):
                alpha = rd.get('alpha_annualized', np.nan)
                stars = sig_super(rd.get('alpha_pval', np.nan))
                a_row += (f" & ${alpha:+.2f}^{{{stars}}}$" if stars
                          else f" & {alpha:+.2f}")
                t_row += f" & ({rd.get('alpha_tstat', np.nan):.2f})"
            else:
                a_row += " & --"
                t_row += " & "
        return a_row, t_row
    ...
    with open(tex_path, 'w', encoding='utf-8') as f:
        f.write("% " + "=" * 74 + "\n")
        f.write("% ALPHA BY REGIME — PCA + BEST-SUBSET SYNTHESIS (3-state)\n")
        f.write("% " + "=" * 74 + "\n\n")

        f.write("\\begin{table}[H]\n")
        f.write("\\centering\n")
        f.write("\\singlespacing\n")
        f.write("\\caption{Alpha by Stress Regime: PCA and Best-Subset}\n")
        f.write("\\label{tab:alpha_regime_pca_aen}\n")
        f.write("\\begin{minipage}{\\textwidth}\n")
        f.write("{\\footnotesize\\noindent Annualized alpha (\\% p.a.) per stress regime, from a "
                "single regression with three intercept dummies and betas common across regimes "
                "(no constant), "
                "$r_t = \\sum_g \\alpha_g D_{g,t} + \\sum_j \\beta_j X_{jt} + \\varepsilon_t$, "
                "Newey--West HAC standard errors (4 lags). "
                "PCA: spanning regression on the $K=8$ full-sample principal components. "
                "Best-Subset: post-selection OLS on the selected factors "
                "(Table~\\ref{tab:aen_stable_ols}). "
                "Regimes on the 5-year iTraxx Europe Main spread: "
                "LOW $<60$, MEDIUM $\\in[60,100)$, HIGH $\\geq100$ bps. "
        ...
        f.write("\\begin{tabular*}{\\textwidth}{@{\\extracolsep{\\fill}}l r r r r r r}\n")
        f.write("\\toprule\n")
        f.write(" & \\multicolumn{3}{c}{PCA} & \\multicolumn{3}{c}{Best-Subset} \\\\\n")
        f.write("\\cmidrule(lr){2-4} \\cmidrule(lr){5-7}\n")
        f.write("Strategy & LOW & MEDIUM & HIGH & LOW & MEDIUM & HIGH \\\\\n")
        f.write("\\midrule\n")

        for s in strategies:
            display = strategy_map.get(s, s)
            pca = pca_regime_data.get(s, {}).get('regimes', {})
            aen = aen_regime_data.get(s, {}).get('regimes', {})

            pca_a, pca_t = _cells(pca)
            aen_a, aen_t = _cells(aen)

            f.write(f"\\textit{{{display}}}{pca_a}{aen_a} \\\\\n")
            f.write(f" {pca_t}{aen_t} \\\\\n")
            f.write("\\addlinespace\n")
...
# STEP 3e: Cross-layer alpha table (Table XII) — needs fm + ml + pca inputs
    try:
        generate_cross_layer_table()
    except Exception as e:
        print(f"   ⚠️ Cross-layer table error: {e}")
