...
HAC_LAGS = 4   # Newey-West (1994) rule floor(4*(T/100)^(2/9)) = 4 for T = 157-243
...
def load_itrx_main_monthly():
    ...
    import factor_sources as src  # type: ignore  # noqa: E402  (path added at runtime above)
...
def auto_nw_lags(T):
    """Newey-West (1994) automatic rule: floor(4*(T/100)^(2/9))."""
    return int(np.floor(4.0 * (T / 100.0) ** (2.0 / 9.0)))


def run_regression(y, X, hac_lags=None):
    """
...
def compute_rolling_alpha(y, X, window, hac_lags=None):
    """
...
def write_subperiod_article(all_results, timings_to_process, pc_names,
    ...
    with open(out_path, 'w', encoding='utf-8') as f:
        ...
        for timing in timings_to_process:
            ...
            f.write("\\clearpage\n")
            # A longtable is NOT a float: it is typeset in the main text flow, so any
            # font/spacing declaration made before it leaks into everything that follows
            # (it was shrinking the appendix text after this table). Scope it in a group.
            f.write("\\begingroup\n")
            ...
            # Caption WITH the note inside it: a \multicolumn note wider than the
            # natural column widths dumps the excess into the last spanned column,
            # which is what wrecked the header alignment. The caption spans the full
            # width without touching the columns.
            f.write(f"\\caption[PCA Subperiod Analysis ({timing_label})]"
                    f"{{PCA Subperiod Analysis ({timing_label})\\\\[4pt]\n")
            f.write("{\\footnotesize\\normalfont\\noindent "
                    "First/Second Half: temporal split at sample midpoint; stress regimes "
                    "(LOW/MEDIUM/HIGH) are reported in Table~\\ref{tab:alpha_regime_pca_aen}. "
                    "$\\alpha$ is annualized (\\% p.a.). "
                    "$t$-statistics in parentheses (Newey--West HAC, 4 lags). "
...
def write_regime_summary_article(all_results, timings_to_process,
    ...
    with open(out_path, 'w', encoding='utf-8') as f:
        ...
        f.write("$t$-statistics in parentheses (Newey--West HAC, 4 lags). ")
...
def main():
    ...
    for t_idx, timing in enumerate(timings_to_process):
        ...
        for s_idx, (strategy_name, rolling_df) in enumerate(rolling_results[timing].items()):
            ...
            ax.set_title(strategy_display, fontsize=12, fontweight='bold')
