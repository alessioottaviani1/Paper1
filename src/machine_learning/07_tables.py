...
def build_main_spanning(all_ols: dict, all_boot: dict) -> str:
    ...
    tex = [
        r"\begin{table}[H]",
        r"\centering",
        r"\caption{Spanning regressions of strategy excess returns on the selected risk factors}",
        r"\label{tab:ml_spanning}",
    ...
    tex += [
        ...
        r"\footnotesize",
        r"\item \textit{Note:} The table reports OLS estimates of "
        ...
        r"Factor definitions are in Table~\ref{tab:ml_factor_list}.",
        r"\end{tablenotes}",
        r"\end{singlespace}",
...
def build_cross_layer(all_ols: dict, pca_timing: str = "contemporaneous",
    ...
    tex += [
        ...
        r"\begin{tablenotes}[para,flushleft]",
        r"\footnotesize",
        r"\item \textit{Note:} Each cell is the annualized alpha (\% p.a.) and adjusted $R^2$ from a "
        r"spanning regression of the strategy excess return on the factor set of the corresponding "
        r"layer: the three established hedge-fund factor benchmarks, the principal-component factors, "
        r"and the best-subset selection. Alphas are annualized; stars denote significance from each "
        r"layer's own inference. $^{***}\,p<0.01$, $^{**}\,p<0.05$, $^{*}\,p<0.10$.",
        r"\end{tablenotes}",
        r"\end{singlespace}",
        r"\end{threeparttable}",
        r"\end{table}",
...
def build_selector_overlap(all_subset: dict, all_aen: dict, all_pds: dict) -> str:
    ...
    tex = [
        r"\begin{table}[H]",
        r"\centering",
        r"\caption{Selected factors under alternative selection methods}",
        r"\label{tab:ml_overlap}",
        r"\begin{threeparttable}",
        r"\begin{singlespace}",
        r"\small",
    ...
    tex += [
        ...
        r"\begin{tablenotes}[para,flushleft]",
        r"\footnotesize",
        r"\item \textit{Note:} The selected factor set for each strategy under " + method_note + r". "
...
def main():
    ...
    tables = {
        "main_spanning.tex":    build_main_spanning(all_ols, all_boot),
        ...
        "selector_overlap.tex": build_selector_overlap(all_subset, all_aen, all_pds),
        "factor_list.tex":      build_factor_list(used),
    }
