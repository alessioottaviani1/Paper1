...
def build_k_sensitivity(all_sens: dict) -> str:
    ...
    tex += [
        ...
        r"Table~\ref{tab:ml_factor_list}.",
        r"\end{tablenotes}",
        r"\end{singlespace}",
...
def build_selector_overlap(all_subset: dict, all_aen: dict, all_pds: dict) -> str:
    ...
    tex = [
        ...
        r"\label{tab:aen_selection_matrix}",
        r"\begin{minipage}{\textwidth}",
...
def build_aen_stable_ols(all_ols: dict, all_boot: dict) -> str:
    """Main-text: post-selection OLS on the stability-selected factors."""
    strats = [s for s in STRATEGIES if s in all_ols]
    if not strats:
        return ""
    tex = [r"\begin{table}[H]", r"\centering", r"\singlespacing",
           r"\caption{Post-Selection OLS on the Stability-Selected Factors}",
           r"\label{tab:aen_stable_ols}",
           r"\begin{minipage}{\textwidth}",
           r"{\footnotesize\noindent This table reports estimates of the post-selection OLS "
           r"regression $r_{i,t}=\alpha_i+\boldsymbol{\beta}_i'\,\mathbf{f}_{\hat S_i,t}"
           r"+\varepsilon_{i,t}$, where $r_{i,t}$ is the monthly excess return of strategy $i$ "
           r"and $\hat S_i$ is the factor set retained by the Adaptive Elastic Net with "
           r"stability selection (selection frequency $\geq 80\%$ over the bootstrap resamples); "
           r"coefficients are unpenalised OLS on the original factors. $\alpha$ is annualized "
           r"(\% p.a.), with the 95\% bootstrap confidence interval below it; $t$-statistics in "
           r"parentheses (Newey--West HAC). $\bar{R}^2$ is the adjusted $R^2$; $k$ the number of "
           r"selected factors; $T$ the number of monthly observations. "
           r"$^{***}\,p<0.01$, $^{**}\,p<0.05$, $^{*}\,p<0.10$.}",
           r"\end{minipage}", r"\par\vspace{6pt}",
           r"\begin{singlespace}", r"\small",
           r"\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}l r r}",
           r"\toprule",
           r"Factor & Coefficient & $t$ \\",
           r"\midrule"]
    for idx, s in enumerate(strats):
        d, b = all_ols[s], all_boot.get(s, {})
        if idx > 0:
            tex += [r"\addlinespace", r"\midrule"]
        tex.append(rf"\multicolumn{{3}}{{l}}{{\textit{{{TITLE_MAP.get(s, s)}}}}} \\")
        tex.append(r"\addlinespace")
        a = d.get("alpha", {})
        tex.append(rf"$\alpha$ (\% p.a.) & ${_f1(a.get('annualized_pct'))}{_sup(a.get('p_value'))}$ "
                   rf"& {_f2(a.get('t_statistic'))} \\")
        ci = (b.get("alpha", {}) or {}).get("ci95_annualized_pct")
        if isinstance(ci, (list, tuple)) and len(ci) == 2:
            tex.append(rf"\quad 95\% CI & \multicolumn{{2}}{{r}}{{$[{_f1(ci[0])},\ {_f1(ci[1])}]$}} \\")
        tex.append(r"\addlinespace")
        fac = d.get("factors", {})
        for f in _selected(d):
            fd = fac.get(f, {})
            tex.append(rf"{_pretty(f)} & ${_f2(fd.get('coefficient'))}{_sup(fd.get('p_value'))}$ "
                       rf"& {_f2(fd.get('t_statistic'))} \\")
        tex.append(r"\addlinespace")
        tex.append(rf"\multicolumn{{3}}{{l}}{{\footnotesize $k={d.get('n_factors', len(_selected(d)))}$, "
                   rf"$\bar R^2={_f2(d.get('r_squared_adj'))}$, $T={d.get('T','--')}$}} \\")
    tex += [r"\bottomrule", r"\end{tabular*}", r"\end{singlespace}", r"\end{table}"]
    return "\n".join(tex)


def build_aen_selection_names(all_subset, all_aen, all_pds) -> str:
    """Appendix: factor sets retained by each selector, per strategy."""
    strats = [s for s in STRATEGIES if s in all_aen or s in all_subset or s in all_pds]
    if not strats:
        return ""
    cols = [("AEN (stability)", all_aen), ("Best-subset", all_subset),
            ("Double selection", all_pds)]
    cols = [(lab, src) for lab, src in cols if any(s in src for s in strats)]
    tex = [r"\begin{table}[H]", r"\centering", r"\singlespacing",
           r"\caption{Selected Factors Across Selection Methods}",
           r"\label{tab:aen_selection_matrix}",
           r"\begin{minipage}{\textwidth}",
           r"{\footnotesize\noindent For each strategy, the factor sets retained by the "
           r"alternative selectors on the same candidate universe. AEN: stability-selected set "
           r"(frequency $\geq 80\%$). Best-subset: $\ell_0$ search under HQC. Double selection: "
           r"post-double-selection union. Factor sets, not coefficients, are reported; "
           r"post-selection OLS estimates are in Table~\ref{tab:aen_stable_ols}.}",
           r"\end{minipage}", r"\par\vspace{6pt}",
           r"\begin{singlespace}", r"\footnotesize",
           r"\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}l " + "l " * len(cols) + "}",
           r"\toprule",
           "Strategy & " + " & ".join(lab for lab, _ in cols) + r" \\",
           r"\midrule"]
    for s in strats:
        cells = []
        for _, src in cols:
            sel = _selected(src.get(s))
            cells.append(", ".join(_pretty(f) for f in sel) if sel else "--")
        tex.append(rf"\textit{{{TITLE_MAP.get(s, s)}}} & " + " & ".join(cells) + r" \\")
        tex.append(r"\addlinespace")
    tex += [r"\bottomrule", r"\end{tabular*}", r"\end{singlespace}", r"\end{table}"]
    return "\n".join(tex)


def build_aen_method_comparison(all_ols: dict) -> str:
    """Main-text: alpha and dimension across penalised selectors, from method_comparison.json."""
    METHODS = [("LASSO", "LASSO"), ("ElasticNet", "Elastic Net"),
               ("AdaptiveLASSO", "Adaptive LASSO"), ("AEN", "AEN (baseline)"),
               ("ALASSO_Chen", "ALASSO--Chen")]
    data = {}
    for s in STRATEGIES:
        d = _load_json(get_strategy_aen_dir(s) / "method_comparison.json")
        if d:
            data[s] = d
    if not data:
        print("   \u26a0\ufe0f  method_comparison.json non trovato — AEN_Method_Comparison saltata "
              "(conferma nome/schema del json)")
        return ""
    tex = [r"\begin{table}[H]", r"\centering", r"\singlespacing",
           r"\caption{Alpha Across Penalised Selection Methods}",
           r"\label{tab:aen_method_comparison}",
           r"\begin{minipage}{\textwidth}",
           r"{\footnotesize\noindent Post-selection OLS alpha of the spanning regression when "
           r"the factor set is chosen by alternative penalised estimators on the same candidate "
           r"universe. LASSO, Elastic Net, Adaptive LASSO, and AEN use HQC for hyperparameter "
           r"selection; ALASSO--Chen uses AIC. $\alpha$ is annualized (\% p.a.); $t$-statistics "
           r"in parentheses (Newey--West HAC); $k$ is the number of selected factors; "
           r"$\bar{R}^2$ the adjusted $R^2$. "
           r"$^{***}\,p<0.01$, $^{**}\,p<0.05$, $^{*}\,p<0.10$.}",
           r"\end{minipage}", r"\par\vspace{6pt}",
           r"\begin{singlespace}", r"\small",
           r"\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}l l r r r}",
           r"\toprule",
           r"Strategy & Method & $\alpha$ & $t$ & $k$ & $\bar{R}^2$ \\",
           r"\midrule"]
    for s in STRATEGIES:
        if s not in data:
            continue
        for j, (key, lab) in enumerate(METHODS):
            m = data[s].get(key)
            lead = rf"\textit{{{TITLE_MAP.get(s, s)}}}" if j == 0 else " "
            if not m:
                tex.append(rf"{lead} & {lab} & -- & -- & -- & -- \\")
                continue
            a = _g(m, "alpha_ann", "alpha_annualized_pct", "annualized_pct", "alpha")
            p = _g(m, "p", "p_value", "alpha_pval")
            tex.append(rf"{lead} & {lab} & ${_f1(a)}{_sup(p)}$ & {_f2(_g(m,'t','t_statistic'))}"
                       rf" & {_g(m,'k','n_factors',default='--')} & {_f2(_g(m,'r2_adj','r_squared_adj'))} \\")
        tex.append(r"\addlinespace")
    tex += [r"\bottomrule", r"\end{tabular*}", r"\end{singlespace}", r"\end{table}"]
    return "\n".join(tex)
...
def main():
    ...
    tables = {
        ...
        "oos_alpha.tex":        build_oos(oos_exp, oos_roll),
