...
def build_main_spanning(all_ols: dict, all_boot: dict) -> str:
    ...
    boot_note = ""
    if n_reps:
        boot_note = (rf"The 95\% CI is the percentile interval for $\alpha$ "
    ...
    for idx, s in enumerate(strats):
        ...
        panel = chr(65 + idx)
        tex.append(rf"\multicolumn{{4}}{{l}}{{\textit{{Panel {panel}: {title}}}}} \\")

        a = d.get("alpha", {})
        a_ann, a_t, a_p = a.get("annualized_pct"), a.get("t_statistic"), a.get("p_value")
        # The bootstrap CI sits on the alpha row itself: as a separate row it cost
        # three lines and pushed the table past a full page.
        ci = (b.get("alpha", {}) or {}).get("ci95_annualized_pct")
        ci_cell = (rf"$[{_f1(ci[0])},\ {_f1(ci[1])}]$"
                   if isinstance(ci, (list, tuple)) and len(ci) == 2 else "")
        tex.append(rf"$\alpha$ (\% p.a.) & ${_f1(a_ann)}{_sup(a_p)}$ & {_f2(a_t)} & {ci_cell} \\")
        tex.append(r"\addlinespace")
...
def build_k_sensitivity(all_sens: dict) -> str:
    ...
    tex = [
        ...
        r"$t$-statistics; $\bar R^2$ is the in-sample adjusted $R^2$ on the "
        ...
        r"\begin{singlespace}",
        r"\small",
        r"\begin{tabularx}{\textwidth}{c l c c X}",
        r"\toprule",
        r"$k$ & $\alpha$ (\% p.a.) & $t$-stat & $\bar R^2$ & Selected factors \\",
        r"\midrule",
    ]
    for idx, s in enumerate(strats):
        title = TITLE_MAP.get(s, s)
        ...
        for k in ks:
            ...
            acell = "--" if a is None else rf"${a:+.2f}{_stars(p)}$"
            tcell = "--" if t is None else rf"{t:.2f}"
            tex.append(rf"{k} & {acell} & {tcell} & {_f2(cell.get('r2_adj_std'))} & {names} \\")
