...
FACTOR_INFO: dict[str, tuple[str, str]] = {
    ...
    "\u0394UM":         ("Change in macroeconomic uncertainty index.",
                         "Ludvigson et al.\\ (2021)"),
    "\u0394UR":         ("Change in real-activity uncertainty index.",
                         "Ludvigson et al.\\ (2021)"),
    "\u0394V2X":        ("Change in V2X (30-day implied vol on Euro STOXX 50).",
                         "Chung et al.\\ (2019)"),
    "\u0394FAILS_PCT_TSY": ("Change in U.S. Treasury fails-to-deliver / total debt outstanding.",
                         "Fleckenstein et al.\\ (2014)"),
    "LIBOR_OIS":        ("Libor--OIS spread (3M LIBOR $-$ 3M OIS).",
                         "Nyborg and Ostberg (2014)"),
    "CDX_IG":           ("First difference of the CDX Investment Grade index CDS spread.",
                         "Klaus and Rzepkowski (2009)"),
    "EMERG_FX":         ("Equal-weighted basket of emerging market currencies vs USD.",
                         "Brooks et al.\\ (2020)"),
...
def build_article_stable_ols(strategies_data):
    ...
    tex.append(r"\item \textit{Note:} "
               ...
               r"stability selection ($\hat{\Pi}_j \geq 0.80$). "
...
def build_article_stability_freq(all_boot_data):
    ...
    # Get thresholds
    pi_thr = all_boot_data[strats[0]].get('config', {}).get('pi_thr', 0.80)
...
def build_article_selection_matrix_names(all_mc_data):
    ...
    tex.append(r"\textit{Note:} "
               ...
               r"stability selection ($\hat{\pi}_j \geq 80\%$).")
...
# ############################################################################
#               BEAMER SLIDES — CONSOLIDATED + CROSS-METHOD
# ############################################################################

def build_slide_consolidated_aen(all_ols_stable):
    """
    Single slide consolidating AEN results across all 3 strategies.
    Top: summary table (alpha, t-stat, k, R^2 adj per strategy).
    Bottom: stable factor sets (3 columns, one per strategy).
    Layout designed for the 1+1 version of the AEN presentation block.
    """
    if not all_ols_stable:
        return ""

    strats = list(all_ols_stable.keys())
    n_s = len(strats)

    tex = []
    tex.append(r"\begin{frame}[t]{AEN: Stable Factor Set + Post-Selection OLS}")
    tex.append(r"\centering\scriptsize")
    tex.append(r"\renewcommand{\arraystretch}{1.05}")
    tex.append(r"\setlength{\tabcolsep}{6pt}")
    tex.append(r"\vspace{0.05cm}")

    # ── Summary table: alpha + t-stat + k + R^2 ──────────────────────────
    col_spec = "l" + "c" * n_s
    ...
    header = " "
    for s in strats:
        title = TITLE_MAP.get(s, s.replace("_", " "))
        header += rf" & \textit{{{title}}}"
    header += r" \\"
    tex.append(header)
    tex.append(r"\midrule")

    # alpha row
    row_a = r"$\hat{\alpha}$ (\% p.a.)"
    for s in strats:
        d = all_ols_stable[s]
        a = d['alpha']['annualized_pct']
        p = d['alpha']['p_value']
        stars = _stars(p)
        cell = f"{a:+.2f}{stars}"
        # Highlight if significant (red); leave black if n.s.
        if stars:
            row_a += rf" & \hlt{{${cell}$}}"
        else:
            row_a += rf" & ${cell}$ (n.s.)"
    ...
    for s in strats:
        d = all_ols_stable[s]
        t = d['alpha']['t_statistic']
        row_t += rf" & $({t:.2f})$"
    row_t += r" \\"
    tex.append(row_t)

    # k row
    row_k = r"\# factors selected"
    for s in strats:
        d = all_ols_stable[s]
        k = d['n_factors']
        row_k += rf" & {k}"
    ...
    tex.append(r"\vspace{0.30cm}")
    tex.append(r"\textbf{Selected factors --- economically interpretable, strategy-specific:}\\[0.10cm]")

    # ── Bottom: 3 columns of stable factors ──────────────────────────────
    tex.append(r"\begin{columns}[t]")
    col_widths = [0.32, 0.34, 0.32] if n_s == 3 else [1.0 / n_s] * n_s
    for s, w in zip(strats, col_widths):
        d = all_ols_stable[s]
        title = TITLE_MAP.get(s, s.replace("_", " "))
        k = d['n_factors']
        factors = d.get('stable_factors', [])
        tex.append(rf"\column{{{w}\textwidth}}")
        tex.append(rf"\textbf{{{title}}} ($k={k}$)\\[2pt]")
        tex.append(r"\scriptsize")
        for f in factors:
            tex.append(rf"$\bullet$ {_pretty_math(f)} \\")
    tex.append(r"\end{columns}")

    tex.append(r"\vspace{0.20cm}")
    tex.append(r"\centering")
    tex.append(r"{\scriptsize Stable factors via bootstrap selection ($\hat\Pi_j \geq 0.80$).}")

    tex.append(r"\end{frame}")
    return "\n".join(tex)


def build_slide_method_comparison(all_ols_stable,
                                  benchmark_dir,
                                  pca_results_dir,
                                  freq="monthly",
                                  pca_timing="contemporaneous"):
    """
    Cross-method comparison slide: 5 methods (Duarte, Brooks, FH, PCA, AEN)
    x 3 strategies (BTP, CDS-Bond, iTraxx) x 2 metrics (alpha % p.a., R^2 adj).
    
    Reads benchmark JSONs from benchmark_dir and PCA JSONs from
    results/<strategy>/pca/, plus AEN data from all_ols_stable in memory.
    
    Output: a Beamer frame string. Cells with n.s. alpha are left in black,
    significant ones use \\hlt{} (red highlight). The AEN row is highlighted
    in full to draw attention to the key result.
    """
    import json as _json

    # ── Step 1: Load benchmark JSONs ─────────────────────────────────────
    # Each JSON is a list of records; we filter EUR for slide consistency.
    bench_files = {
        "Duarte (k=10)":     benchmark_dir / f"Duarte_summary_{freq}.json",
        "Brooks (k=8)":      benchmark_dir / f"ActiveFI_summary_{freq}.json",
        "Fung--Hsieh (k=7)": benchmark_dir / f"FungHsieh_summary_{freq}.json",
    }

    bench_data = {}  # {method_label: {strategy_key: {alpha, alpha_pval, r2adj}}}
    for label, path in bench_files.items():
        bench_data[label] = {}
        if not path.exists():
            print(f"   ⚠️  {path.name} not found, skipping {label}")
            continue
        with open(path, 'r', encoding='utf-8') as jf:
            records = _json.load(jf)
        for rec in records:
            if rec.get('Region') != 'EUR':
                continue
            strat = rec.get('Strategy')
            try:
                a = float(rec.get('Alpha_annual'))
                t = float(rec.get('t_stat'))
                r2a = float(rec.get('R_squared_adj'))
                # p-value approximation from t (two-sided, large df)
                # Better: use the 'Significance' field if present
                sig_str = rec.get('Significance', '').strip()
                pval = 0.005 if '***' in sig_str else \
                       0.025 if '**' in sig_str else \
                       0.075 if '*' in sig_str else 0.5
            except (TypeError, ValueError):
                continue
            bench_data[label][strat] = {
                'alpha': a, 'pval': pval, 'r2adj': r2a, 'tstat': t
            }

    # ── Step 2: Load PCA JSONs (one per strategy) ────────────────────────
    pca_strat_dirs = {
        'BTP_Italia':       pca_results_dir / 'btp_italia' / 'pca',
        'CDS_Bond_Basis':   pca_results_dir / 'cds_bond_basis' / 'pca',
        'iTraxx_Combined':  pca_results_dir / 'itraxx_combined' / 'pca',
    }
    pca_data = {}  # {strategy_key: {alpha, pval, r2adj}}
    pca_K = None
    for strat_key, pdir in pca_strat_dirs.items():
        pfile = pdir / f"spanning_regression_results_{pca_timing}.json"
        if not pfile.exists():
            print(f"   ⚠️  {pfile.name} not found for {strat_key}")
            continue
        with open(pfile, 'r', encoding='utf-8') as jf:
            r = _json.load(jf)
        try:
            alpha_ann = float(r['alpha']) * 12.0  # decimal monthly → % p.a.
            pval = float(r['alpha_pvalue'])
            r2a = float(r['r_squared_adj'])
            pca_K = r.get('n_components', pca_K)
        except (TypeError, ValueError, KeyError):
            continue
        pca_data[strat_key] = {'alpha': alpha_ann, 'pval': pval, 'r2adj': r2a}

# ── Step 3: AEN row from in-memory data ──────────────────────────────
    # Normalize AEN strategy keys (lowercase) to benchmark-style (capitalized)
    AEN_TO_BENCH = {
        'btp_italia':      'BTP_Italia',
        'cds_bond_basis':  'CDS_Bond_Basis',
        'itraxx_combined': 'iTraxx_Combined',
    }
    aen_data = {}
    aen_ks = []
    for strat_key_raw, d in all_ols_stable.items():
        strat_key = AEN_TO_BENCH.get(strat_key_raw, strat_key_raw)
        try:
            a = float(d['alpha']['annualized_pct'])
            p = float(d['alpha']['p_value'])
            r2a = float(d['r_squared_adj'])
            k = int(d['n_factors'])
        except (TypeError, ValueError, KeyError):
            continue
        aen_data[strat_key] = {'alpha': a, 'pval': p, 'r2adj': r2a, 'k': k}
        aen_ks.append(k)
    aen_label = f"AEN (k={'/'.join(str(x) for x in aen_ks)})" if aen_ks else "AEN"

    # ── Step 4: Build the LaTeX slide ────────────────────────────────────
    ...
    tex.append(r"\renewcommand{\arraystretch}{1.10}")
    tex.append(r"\vspace{0.05cm}")

    # 7 columns: method | (alpha, R2adj) x 3 strategies
    tex.append(r"\begin{tabular}{l cc cc cc}")
    tex.append(r"\toprule")

    # Header row 1: strategy multicolumns
    h1 = "Method"
    for s in strat_order:
        h1 += rf" & \multicolumn{{2}}{{c}}{{\textit{{{strat_disp[s]}}}}}"
    h1 += r" \\"
    tex.append(h1)
    # cmidrules
    cmi = ""
    ...
    # Header row 2
    h2 = " "
    for _ in strat_order:
        h2 += r" & $\alpha$\,(\%) & $\bar R^2$"
    h2 += r" \\"
    tex.append(h2)
    tex.append(r"\midrule")

    # Helper to format alpha cell with stars
    def _fmt_alpha(a, pval):
        stars = '***' if pval < 0.01 else '**' if pval < 0.05 else '*' if pval < 0.10 else ''
        if stars:
            return rf"${a:+.2f}^{{{stars}}}$"
        return rf"${a:+.2f}$"

    # ── Benchmark rows ───────────────────────────────────────────────────
    for label in ["Duarte (k=10)", "Brooks (k=8)", "Fung--Hsieh (k=7)"]:
        row = label
        for s in strat_order:
            d = bench_data.get(label, {}).get(s)
            if d is None:
                row += " & -- & --"
            else:
                row += rf" & {_fmt_alpha(d['alpha'], d['pval'])} & {d['r2adj']:.3f}"
        row += r" \\"
        tex.append(row)


    # ── PCA row ──────────────────────────────────────────────────────────
    pca_label = f"PCA (K={pca_K})" if pca_K else "PCA"
    row = pca_label
    for s in strat_order:
        d = pca_data.get(s)
        if d is None:
    ...
    # ── AEN row (highlighted) ────────────────────────────────────────────
    row = rf"\hlt{{{aen_label}}}"
    for s in strat_order:
        d = aen_data.get(s)
        if d is None:
            row += " & -- & --"
        else:
            stars = '***' if d['pval'] < 0.01 else '**' if d['pval'] < 0.05 else '*' if d['pval'] < 0.10 else ''
            a = d['alpha']
            r2a = d['r2adj']
            if stars:
                a_cell = rf"\hlt{{${a:+.2f}^{{{stars}}}$}}"
            else:
                a_cell = rf"${a:+.2f}$"
            r2_cell = rf"\hlt{{{r2a:.3f}}}"
            row += rf" & {a_cell} & {r2_cell}"
    row += r" \\"
    tex.append(row)
...
def generate_stability_barplot(all_boot_data):
    ...
    for i, s in enumerate(strats):
        ax = axes[0, i]
        freqs = all_boot_data[s].get('factor_frequencies', {})
        pi_thr = all_boot_data[s].get('config', {}).get('pi_thr', 0.80)
...
def main():
    ...
    # ── Generate consolidated AEN slide (for 1+1 version) ─────────────
    consolidated_content = build_slide_consolidated_aen(all_ols_stable)
    if consolidated_content:
        consolidated_fname = "AEN_Consolidated_Presentation_Slide.tex"
        (aen_tables_dir / consolidated_fname).write_text(
            consolidated_content, encoding="utf-8"
        )
        print(f"   ✅ {consolidated_fname}  ← CONSOLIDATED (1+1 version)")

    # ── Generate cross-method comparison slide ────────────────────────
    cross_content = build_slide_method_comparison(
        all_ols_stable,
        benchmark_dir=TABLES_DIR,
        pca_results_dir=RESULTS_DIR,
        freq="monthly",
        pca_timing="contemporaneous",
    )
    if cross_content:
        cross_fname = "Cross_Method_Comparison_Presentation_Slide.tex"
        (aen_tables_dir / cross_fname).write_text(cross_content, encoding="utf-8")
        print(f"   ✅ {cross_fname}  ← CROSS-METHOD (used in both 3+1 and 1+1)")
