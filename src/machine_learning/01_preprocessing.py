...
3. FLAG highly correlated pairs (diagnostic only — no pruning)
4. Apply manual exclusions (FACTORS_TO_EXCLUDE; currently none)
5. Center predictors and response (paper p. 1733)
6. Standardize predictors to L2-norm = 1 (paper p. 1735)
7. Save clean (y, X) datasets

References:
    Zou & Zhang (2009): centering/standardization conventions.
...
def main():
    ...
    # ── STEP 5: Center & Standardize (L2-norm = 1) ────────────────────────
    print_header("STEP 5: Center & Standardize (L2-norm = 1)", "-")
    ...
    print(f"{'=' * 80}")
    print(f"\n   🎯 Next: python src/machine_learning/02s_best_subset.py")
