...
def plot_composite_rolling_alpha(strategies):
    """Composite 3-panel rolling alpha on the frozen best-subset set, one panel
    per strategy, HIGH-stress shading only (paper appendix figure).
    Reads each strategy's rolling_alpha.csv; writes results/figures/aen/aen_rolling_alpha.pdf."""
    fig_dir = PROJECT_ROOT / "results" / "figures" / "aen"
    fig_dir.mkdir(parents=True, exist_ok=True)

    z_raw = load_stress_proxy_monthly("ITRX_MAIN")
    z_monthly = z_raw.resample("ME").last().dropna()
    HIGH = DEFAULT_THRESHOLD

    names = list(strategies)
    fig, axes = plt.subplots(len(names), 1, figsize=(12, 3.2 * len(names)), sharex=False)
    if len(names) == 1:
        axes = [axes]

    for ax, sname in zip(axes, names):
        sdir = get_strategy_aen_dir(sname)
        fp = sdir / "rolling_alpha.csv"
        if not fp.exists():
            ax.set_visible(False)
            print(f"   ⚠ {fp} mancante — pannello {sname} saltato")
            continue
        df = pd.read_csv(fp)
        df["end_date"] = pd.to_datetime(df["end_date"])
        df = df.set_index("end_date")
        dates = df.index
        a = df["alpha_annualized"].values
        lo = df["ci_lower"].values * 12
        hi = df["ci_upper"].values * 12

        ax.plot(dates, a, color="black", linewidth=1.2, label=r"Rolling $\alpha$")
        ax.fill_between(dates, lo, hi, color="grey", alpha=0.20, label="95\\% CI")
        ax.axhline(0, color="grey", linewidth=0.5)

        reg = z_monthly.reindex(dates, method="nearest") >= HIGH
        if reg.any():
            blocks = reg.astype(int).diff().fillna(0)
            starts = list(dates[blocks == 1])
            ends = list(dates[blocks == -1])
            if reg.iloc[0]:
                starts = [dates[0]] + starts
            if reg.iloc[-1]:
                ends = ends + [dates[-1]]
            for s, e in zip(starts, ends):
                ax.axvspan(s, e, alpha=0.20, color="#d62728", zorder=0)

        ax.set_title(TITLE_MAP.get(sname, sname), fontsize=11, fontweight="bold")
    ...
    fig.tight_layout()
    out = fig_dir / "aen_rolling_alpha.pdf"
...
def _build_fs_thesis(all_results):
    ...
    tex.append(r"{\footnotesize\noindent "
               ...
               r"$r_t = \alpha_0 + \alpha_1 z_{t-1} + (\boldsymbol{\beta} + \boldsymbol{\delta} z_{t-1})'"
               r"\mathbf{f}_{\hat S,t} + \varepsilon_t$, with the best-subset-selected factors "
               r"$\hat S$: both the alpha and the factor loadings are conditioned on $z_{t-1}$, "
               r"the one-month-lagged standardized iTraxx Main 5Y spread (mean zero, unit variance, "
               r"a predetermined conditioning variable); $\alpha_1>0$ indicates alpha rises "
               r"with credit stress. Significance of $\alpha_1$ is from a moving-block bootstrap "
               r"($B=9{,}999$, $H_0{:}\,\alpha_1=0$ imposed, block length~9); "
    ...
    for s in strategies:
        ...
        vals.append(f"${c['alpha1'] * 12:+.2f}{_stars_sup(c['alpha1_pval'])}$")
    ...
    tex.append(rf"$\alpha(z{{=}}{{+}}1\sigma)$ (\% p.a.) & {' & '.join(vals_high)} \\")
    tex.append(rf"$\alpha(z{{=}}{{-}}1\sigma)$ (\% p.a.) & {' & '.join(vals_low)} \\")
...
def main():
    ...
    if all_results:
        ...
        # ── Composite rolling-alpha figure (paper appendix) ───────────
        plot_composite_rolling_alpha(STRATEGIES)
