...
def run_ols_for_strategy(strategy_name, strategy_path):
    ...
    selected_factors = subset_results['selected_factors']
    n_selected = subset_results['n_selected']
    selector = subset_results.get('method', 'best-subset (ℓ0)')

    if n_selected == 0:
        print(f"\n   ⚠️  No factors selected. Skipping OLS.")
        return None

    print(f"\n   Best-subset selected {n_selected} factors: {selected_factors}")
...
def main():
    ...
    for strategy_name, strategy_path in STRATEGIES.items():
        ...
        if not (strategy_dir / "subset_results.json").exists():
            print(f"\n   ❌ Best-subset results not found. Run 02s first.")
            continue
