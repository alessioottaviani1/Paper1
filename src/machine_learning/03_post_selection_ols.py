"""
================================================================================
03_post_selection_ols.py - Post-Selection OLS Inference
================================================================================
After best-subset selects factors in Step 02s, this script estimates an
unrestricted OLS regression on the selected factors to:

1. Obtain coefficient estimates and a meaningful intercept (the selection
   step identifies the support only, not the point estimates).
...
L2-normed data from preprocessing.  The standardization was for the selection
step; OLS inference needs original units for interpretable coefficients and a
meaningful intercept (α).

Inputs:
    - subset_results.json from Step 02s (selected factor names)
...
def run_ols_for_strategy(strategy_name, strategy_path, selector_source="best_subset"):
    """Run post-selection OLS for a single strategy.
    selector_source: 'best_subset' (reads subset_results.json) or 'aen' (reads aen_results.json)."""

    strategy_dir = get_strategy_aen_dir(strategy_name)

    # ── Load selected factors from the chosen selector ─────────────────
    if selector_source == "aen":
        src_file = "aen_results.json"
        default_name = "Adaptive Elastic Net"
    else:
        src_file = "subset_results.json"
        default_name = "best-subset (ℓ0)"
    with open(strategy_dir / src_file, 'r') as f:
        sel_results = json.load(f)

    selected_factors = sel_results['selected_factors']
    n_selected = sel_results['n_selected']
    selector = sel_results.get('method', default_name)

    if n_selected == 0:
        print(f"\n   ⚠️  No factors selected. Skipping OLS.")
        return None

    print(f"\n   {selector} selected {n_selected} factors: {selected_factors}")
    ...
    # Persist the exact aligned (y, X) so 04 bootstraps the identical sample
    reg_inputs = X.copy()
    reg_inputs.insert(0, "__y__", y.values)
    reg_inputs.to_parquet(strategy_dir / "regression_inputs.parquet")
    ...
    out_name = "ols_results_AEN.json" if selector_source == "aen" else "ols_results_BESTSUBSET.json"
    with open(strategy_dir / out_name, 'w') as f:
...
def main():
    print_header("POST-SELECTION OLS INFERENCE — AEN vs BEST-SUBSET")
    print(f"   HAC lags (Newey-West): {HAC_LAGS}")
    print(f"   H₀: α = 0 (no risk-adjusted return)")

    all_results = {}          # AEN results (kept as the primary dict downstream)
    compare = {}              # {strategy: {'aen': out, 'best_subset': out}}

    for strategy_name, strategy_path in STRATEGIES.items():
        print_header(f"STRATEGY: {strategy_name}")
        strategy_dir = get_strategy_aen_dir(strategy_name)
        compare[strategy_name] = {}

        for src in ("aen", "best_subset"):
            src_file = "aen_results.json" if src == "aen" else "subset_results.json"
            if not (strategy_dir / src_file).exists():
                print(f"\n   ❌ {src_file} not found. Run {'02' if src=='aen' else '02s'} first.")
                continue
            try:
                result = run_ols_for_strategy(strategy_name, strategy_path, selector_source=src)
                if result is not None:
                    compare[strategy_name][src] = result
                    if src == "aen":
    ...
    print_header("AEN vs BEST-SUBSET — HEAD TO HEAD")
    for strat, d in compare.items():
        print(f"\n{strat}")
        for src in ("aen", "best_subset"):
            r = d.get(src)
            if not r:
                print(f"   {src:<12}: (missing)")
                continue
            a = r['alpha']
            print(f"   {src:<12}: alpha={a['annualized_pct']:+6.2f}%  "
                  f"t={a['t_statistic']:+5.2f}  p={a['p_value']:.3f}  "
                  f"R2adj={r['r_squared_adj']:.3f}  k={r['n_factors']}")
        if d.get("aen") and d.get("best_subset"):
            fa, fb = set(d["aen"]["factors"]), set(d["best_subset"]["factors"])
            print(f"   factors AEN ({len(fa)}): {sorted(fa)}")
            print(f"   factors BS  ({len(fb)}): {sorted(fb)}")
            print(f"   shared ({len(fa&fb)}): {sorted(fa&fb)} | only-AEN: {sorted(fa-fb)} | only-BS: {sorted(fb-fa)}")
    ...
    if all_results:
        ...
        summary = {
            'selector': 'best-subset (ℓ0, primary)',
            'hac_lags': HAC_LAGS,
            'strategies': {name: {
    ...
    print(f"{'=' * 80}")
    print(f"\n   🎯 Next: python src/machine_learning/04_bootstrap.py")
