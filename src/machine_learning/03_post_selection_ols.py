...
def run_ols_for_strategy(strategy_name, strategy_path):
    """Run post-selection OLS for a single strategy on the best-subset factors."""

    strategy_dir = get_strategy_aen_dir(strategy_name)

    # ── Load best-subset results (primary selector; 02s) ───────────────
    with open(strategy_dir / "subset_results.json", 'r') as f:
        sel_results = json.load(f)

    selected_factors = sel_results['selected_factors']
    n_selected = sel_results['n_selected']
    selector = sel_results.get('method', 'best-subset (ℓ0)')
...
def main():
    print_header("POST-SELECTION OLS INFERENCE — BEST-SUBSET")
    print(f"   Selector: best-subset (ℓ0, primary)")
    ...
    for strategy_name, strategy_path in STRATEGIES.items():
        ...
        if not (strategy_dir / "subset_results.json").exists():
            print(f"\n   ❌ subset_results.json not found. Run 02s first.")
            continue
        try:
