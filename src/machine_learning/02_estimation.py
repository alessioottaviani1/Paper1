...
Two-stage AEN from Zou & Zhang (2009), with support for:
    Tuned by Hannan–Quinn IC (HQC). AEN is a selector-robustness check against
the best-subset primary, so alternative ICs / CV are not explored.
...
def compute_ic(y, X, beta):
    """
    Hannan–Quinn IC: IC = T·log(RSS/T) + 2·log(log(T))·df.
    (AEN here is a selector-robustness check vs the best-subset primary, so
    we do not explore alternative ICs.) Returns (ic_value, rss, df).
    """
    T = len(y)
    residuals = y - X @ beta
    ...
    ic = T * np.log(rss / T) + 2 * np.log(np.log(T)) * df
...
def run_stage1(y, X, lambda2_grid, n_lambda1,
    ...
    for lambda2_po in lambda2_grid:
        ...
        for lambda1_po in lambda1_grid:
            ...
            ic_raw, rss_raw, _ = compute_ic(y_np, X_np, beta_raw)
    ...
    print(f"\n   Stage 1 results (HQC on rescaled β̂(enet)):")
    ...
    print(f"      HQC (rescaled) = {best_result['ic_resc']:.4f}"
          f"  |  HQC (raw) = {best_result['ic_raw']:.4f}")
    ...
    if raw_best_idx != resc_best_idx:
        print(f"\n   📊 ROBUSTNESS: Raw-HQC selects different model:"
              f" df={int(raw_best['df'])}")
    else:
        print(f"\n   ✅ ROBUSTNESS: Raw and rescaled HQC agree.")
...
def compute_adaptive_weights(beta_enet, T, gamma=1):
    """
    wⱼ = (|β̂ⱼ| + 1/T)^{-γ}  on the initial estimator β̂ (OLS, p < T).
    """
    epsilon = 1.0 / T
    return (np.abs(beta_enet) + epsilon) ** (-gamma)


def compute_initial_beta(X, y):
    """Initial estimator for the adaptive weights (Zou 2006).

    p = 75 < T for all three strategies (157/197/209), so OLS is the
    appropriate initial estimator (cf. the p < T branch of the γ-sensitivity
    in 05_method_comparison.py). Every factor then gets a finite weight; the
    sparse Stage-1 elastic-net estimate would set wⱼ = (0+1/T)^{-γ} = T for
    every zeroed factor, banning it from Stage 2 and the bootstrap regardless
    of explanatory power. Support is thus decided by the data and the criterion.
    """
    X_np = np.asarray(X, dtype=np.float64)
    y_np = np.asarray(y, dtype=np.float64)
    return np.linalg.lstsq(X_np, y_np, rcond=None)[0]
...
def run_stage2(y, X, weights, lambda2_po_fixed, T,
               n_lambda1, max_iter=10000, tol=1e-7):
    T_obs, p = X.shape
    y_np = np.asarray(y, dtype=np.float64)
    X_np = np.asarray(X, dtype=np.float64)
    ...
    for lambda1_star_po in lambda1_star_grid:
        ...
        ic_raw, rss_raw, _ = compute_ic(y_np, X_np, beta_naive)
...
def run_aen_for_strategy(strategy_name):
    ...
    beta_init = compute_initial_beta(X, y)
    weights = compute_adaptive_weights(beta_init, T, AEN_GAMMA)
    print(f"\n   Weights on β̂(OLS) initial estimator (p = {X.shape[1]} < T, Zou 2006)")
...
def main():
    ...
    print(f"   Criterion: HQC (selector-robustness check vs best-subset)")
    print(f"   AEN γ = {AEN_GAMMA}")
    ...
    print(f"\n{'=' * 80}")
    print(f"✅ AEN ESTIMATION COMPLETE (criterion: HQC)")
    print(f"{'=' * 80}")
