...
def best_subset(Xz, y, k):
    """Exact ell0 best subset of size k via the abess splicing algorithm
    (Zhu, Wen, Zhu, Zhang & Wang 2020, PNAS).

    Returns (sorted index list, engine_label). Raises if abess is unavailable
    or does not honour the requested cardinality: no fallback by design, so
    every reported result is guaranteed to come from the exact ell0 solver.
    """
    try:
        import abess
        ...
        raise RuntimeError(
            "abess is required for the exact ell0 best-subset selection "
            "(pip install abess); no greedy fallback is provided by design."
        ) from e
    ...
    if len(idx) != k:
        raise RuntimeError(
            f"abess returned {len(idx)} factors for k={k}: the ell0 solver "
            f"did not honour the cardinality constraint; aborting instead of "
            f"degrading to a greedy approximation."
        )
    return idx, f"abess(ell0) v{getattr(abess, '__version__', 'unknown')}"
