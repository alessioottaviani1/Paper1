...
HAC_LAGS              = 4                         # Newey-West (1994) rule: 4 for T = 157-243
...
# ── Per-strategy driver ─────────────────────────────────────────────────────────
def alpha_hac(y_raw, X_raw, idx, lags=HAC_LAGS):
    """Annualized alpha (% p.a.), HAC t and p of y_raw on the RAW factors at idx.

    The alpha must be computed on the raw (uncentered) design: with the centered
    X_standardized the OLS intercept collapses to the sample mean of y for EVERY
    subset, so it would carry no information about the selected set. This mirrors
    the raw-data inference of module 03 that produces Table X.
    """
    Xc = sm.add_constant(X_raw[:, idx]) if len(idx) else np.ones((len(y_raw), 1))
    res = sm.OLS(y_raw, Xc).fit(cov_type="HAC", cov_kwds={"maxlags": lags})
    return 12.0 * float(res.params[0]), float(res.tvalues[0]), float(res.pvalues[0])


def load_raw(strategy_name, X_df, y_centered):
    """y_raw and the raw factor matrix aligned to X_standardized (rows and columns)."""
    d = get_strategy_aen_dir(strategy_name)
    y_mean = None
    for jf in sorted(d.glob("*.json")):
        ...
        if isinstance(blob, dict) and "y_mean" in blob:
            y_mean = float(blob["y_mean"])
            break
    if y_mean is None:
        print(f"   ⚠️  {strategy_name}: y_mean not found — alpha column skipped.")
        return None, None
    Xr = pd.read_parquet(FACTORS_PATH).reindex(index=X_df.index)[list(X_df.columns)]
    if Xr.isna().any().any():
        print(f"   ⚠️  {strategy_name}: raw factors misaligned — alpha column skipped.")
        return None, None
    return np.asarray(y_centered, dtype=float) + y_mean, np.asarray(Xr, dtype=float)
...
def run_strategy(strategy_name):
    ...
    y_raw, X_raw = load_raw(strategy_name, X_df, y)

    # k-sensitivity (best-subset at each grid size)
    sensitivity = {}
    print(f"   {'support':>10} {'k':>3} {'R2adj(std)':>11}  {'alpha':>8}  engine")
    for k in K_GRID:
        ...
        if y_raw is not None:
            a, t_, p_ = alpha_hac(y_raw, X_raw, idx)
            cell.update({"alpha": round(a, 3), "alpha_t": round(t_, 2),
                         "alpha_p": round(p_, 4)})
        sensitivity[k] = cell
        _a = f"{cell.get('alpha'):+8.2f}" if "alpha" in cell else "      --"
        print(f"   {'k=' + str(k):>10} {len(idx):>3} {cell['r2_adj_std']:>11.4f} {_a}  {engine}")
