...
def best_subset(Xz, y, k):
    ...
    try:
        ...
        print(f"   ⚠️  abess returned {len(idx)} factors for k={k} (expected {k}); "
              f"using forward-forced — reported engine is NOT ℓ0.")
    except Exception as e:
        print(f"   ⚠️  abess unavailable/failed ({type(e).__name__}: {e}); "
              f"using forward-forced — reported engine is NOT ℓ0.")
