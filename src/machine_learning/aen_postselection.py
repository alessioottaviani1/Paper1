...
# -- factor_sources: z (iTraxx Main 5Y level) dal foglio 'cds' di bbg.xlsx -----
_fs_path = PROJECT_ROOT / "src" / "factor_models" / "factor_sources.py"
_fsspec = importlib.util.spec_from_file_location("factor_sources", _fs_path)
src = importlib.util.module_from_spec(_fsspec)
_fsspec.loader.exec_module(src)
...
def load_z_itrx_main():
    ...
    bbg.xlsx (PX_LAST). Stessa fonte/loader di
    factor_models/03_subperiod_rolling_analysis.py, in sostituzione del
    ritirato Tradable_corporate_bond_factors.xlsx."""
...
def main():
    ...
    for sn, path in STRATEGIES.items():
        ...
        out[sn] = {
            ...
            "alpha0_insample": a0,
            "z_lag":    {"insample": ins_l, "pds": pds_l},
