...
FACTORS_PATH         = aen_config.FACTORS_PATH   # raw factor panel (pre-standardization)
...
def load_inputs(strategy: str):
    ...
    if y_mean is None:
        ...
        y_mean = 0.0
    # Raw (unstandardized) factor matrix, rebuilt from the factor panel that
    # ml/01 standardizes: X_std = (X_raw - mean) / ||X_raw - mean||_2, with the
    # column norms computed on the FULL sample. Using X_std inside frozen_oos
    # would inject look-ahead into an out-of-sample exercise, so the OOS
    # re-estimation runs on the raw factors, exactly as in ml/05.
    Xr = pd.read_parquet(FACTORS_PATH)
    missing = [c for c in X.columns if c not in Xr.columns]
    if missing:
        raise RuntimeError(f"{strategy}: raw factors missing from {FACTORS_PATH}: {missing[:5]}")
    Xr = Xr.reindex(index=X.index)[list(X.columns)].astype(float)
    if Xr.isna().any().any():
        raise RuntimeError(f"{strategy}: NaNs after aligning the raw factor panel to "
                           f"X_standardized (index mismatch).")
    return y.astype(float), X.astype(float), Xr, y_mean, list(X.columns)
...
def frozen_oos(y_raw: np.ndarray, Xz: np.ndarray, idx, burnin=60, lags=HAC_LAGS):
    """NOTE: Xz must be the RAW factor matrix, not the full-sample standardized
    design: standardizing on the full sample injects look-ahead into an
    out-of-sample exercise and breaks comparability with Table IX, Panel B."""
...
def sel_aen_refit(Xz, y, names=None, k=None):
    """Adaptive elastic net (Zou-Zhang 2009) re-fitted on whatever sample it is
    given, so the AEN row also has an honest split-sample alpha instead of '--'.
    Adaptive weights from a ridge pilot (w_j = 1/|b_ridge_j|^gamma); (lambda,
    l1_ratio) by contiguous-block CV, matching the paper's AEN in ml/02."""
    from sklearn.linear_model import Ridge, ElasticNet, enet_path
    n, p = Xz.shape
    # ridge pilot -> adaptive weights (gamma = 1, as in the paper)
    b_pilot = Ridge(alpha=1.0, fit_intercept=False).fit(Xz, y).coef_
    w = 1.0 / np.maximum(np.abs(b_pilot), 1e-6)
    Xw = Xz / w                                   # reweighted design
    folds = np.array_split(np.arange(n), CV_FOLDS)
    l1_ratios = (0.5, 0.7, 0.9, 1.0)
    best = (np.inf, None, None)                   # (cv_mse, l1_ratio, alpha)
    for l1r in l1_ratios:
        alphas, _, _ = enet_path(Xw, y, l1_ratio=l1r, n_alphas=40, eps=1e-3)
        cv = np.zeros(len(alphas))
        for test in folds:
            train = np.setdiff1d(np.arange(n), test)
            _, c_tr, _ = enet_path(Xw[train], y[train], l1_ratio=l1r, alphas=alphas)
            for ai in range(len(alphas)):
                cv[ai] += float(np.mean((y[test] - Xw[test] @ c_tr[:, ai]) ** 2)) / CV_FOLDS
        ai = int(np.argmin(cv))
        if cv[ai] < best[0]:
            best = (float(cv[ai]), l1r, float(alphas[ai]))
    _, l1r, lam = best
    b = ElasticNet(alpha=lam, l1_ratio=l1r, fit_intercept=False,
                   max_iter=50_000).fit(Xw, y).coef_
    S = sorted(np.where(np.abs(b) > 1e-10)[0].tolist())
    return S, f"AEN refit (l1={l1r:g}, lam={lam:.4g}, |S|={len(S)})"
...
def sel_rf(Xz, y, names=None, k=K_DEFAULT):
    """Nonlinear ML screening: random forest -> permutation importance ->
    top-k -> (downstream) post-OLS. The selection criterion is predictive
    and nonlinear, so this is a deliberately MISALIGNED screener for a
    linear Jensen-alpha test: its role is to show the alpha verdict does
    not depend on staying inside the linear selection family."""
    from sklearn.ensemble import RandomForestRegressor
    from sklearn.inspection import permutation_importance
    m = RandomForestRegressor(n_estimators=1000, max_features="sqrt",
                              min_samples_leaf=5, random_state=SEED,
                              n_jobs=-1).fit(Xz, y)
    imp = permutation_importance(m, Xz, y, n_repeats=25,
                                 random_state=SEED, n_jobs=-1)
    S = sorted(np.argsort(-imp.importances_mean)[:k].tolist())
    return S, f"RF permutation importance (top-{k})"
...
#  Benchmark driver
# ══════════════════════════════════════════════════════════════════════════════
def evaluate(name, selector, Xz, y_c, y_raw, names, k, refit_halves=True, Xr=None):
    t0 = time.time()
    ...
    a, t, ir, n_oos = frozen_oos(y_raw, Xz if Xr is None else Xr, S)
...
def run_strategy(strategy: str, k: int, write_pds: bool, groups_path=None):
    print(f"\n{'=' * 78}\n  {strategy.upper()}\n{'=' * 78}")
    y_c, X, X_raw, y_mean, names = load_inputs(strategy)
    Xz = X.values
    Xr = X_raw.values
    yc = y_c.values
    yr = yc + y_mean
    ...
    rows = []
    rows.append(evaluate("ell0",     lambda X_, y_, n_, k_: sel_ell0(X_, y_, n_, k_),    Xz, yc, yr, names, k, Xr=Xr))
    rows.append(evaluate("forward",  lambda X_, y_, n_, k_: sel_forward(X_, y_, n_, k_), Xz, yc, yr, names, k, Xr=Xr))

    aen_idx = load_aen_selection(strategy, names)
    if aen_idx is not None:
        # Re-fit the AEN on whatever sample it is given: this makes the honest
        # split-sample alpha available for the AEN rows too, instead of "--".
        rows.append(evaluate("aen(paper)",
                             lambda X_, y_, n_, k_: (aen_idx, "loaded aen_results.json")
                             if X_.shape[0] == Xz.shape[0] else sel_aen_refit(X_, y_, n_, k_),
                             Xz, yc, yr, names, k, refit_halves=True, Xr=Xr))
    else:
        print("   ⚠️  aen_results.json not found — AEN column skipped.")

    rows.append(evaluate("relaxed",     lambda X_, y_, n_, k_: sel_relaxed_lasso(X_, y_),      Xz, yc, yr, names, k, Xr=Xr))
    rows.append(evaluate("cpss",        lambda X_, y_, n_, k_: sel_cpss(X_, y_),               Xz, yc, yr, names, k, Xr=Xr))
    rows.append(evaluate("bch_pds",     lambda X_, y_, n_, k_: sel_bch_pds(X_, y_),            Xz, yc, yr, names, k, Xr=Xr))
    rows.append(evaluate("group_rep",   lambda X_, y_, n_, k_: sel_group_rep(X_, y_, groups),  Xz, yc, yr, names, k, Xr=Xr))
    rows.append(evaluate("exclusive",   lambda X_, y_, n_, k_: sel_exclusive(X_, y_, groups),  Xz, yc, yr, names, k, Xr=Xr))
    rows.append(evaluate("cluster_rep", lambda X_, y_, n_, k_: sel_cluster_rep(X_, y_, groups),Xz, yc, yr, names, k, Xr=Xr))
    rows.append(evaluate("rf",          lambda X_, y_, n_, k_: sel_rf(X_, y_, n_, k_),          Xz, yc, yr, names, k, Xr=Xr))
    if aen_idx is not None:
        rows.append(evaluate("aen_dedup",
                             lambda X_, y_, n_, k_: sel_aen_dedup(X_, y_, groups, aen_idx),
                             Xz, yc, yr, names, k, refit_halves=False, Xr=Xr))
    rows.append(evaluate("knockoffs",   lambda X_, y_, n_, k_: sel_knockoffs(X_, y_),          Xz, yc, yr, names, k, Xr=Xr))
    ...
    # ── Selection frequency: in how many methods does each factor enter? ──
    ...
    print(f"\n   Selection frequency across {n_sets} non-empty selectors:")
    for name, cnt in top:
        print(f"      {name:<22} {cnt}/{n_sets}")
    df.attrs["selection_frequency"] = {k2: int(v) for k2, v in freq.items()}
    df.attrs["n_selectors"] = n_sets
...
def emit_tex(blob, out_path):
    ...
    DISPLAY = [("ell0",        r"Best subset ($\ell_0$, $k{=}8$)"),
               ("group_rep",   r"Best subset, one factor per risk category"),
               ("aen(paper)",  r"Adaptive elastic net"),
               ("relaxed",     r"Relaxed lasso"),
               ("rf",          r"Random forest screening (top-$8$)"),
               ("knockoffs",   r"Model-X knockoffs (FDR $10\%$)")]
    ...
    L = [r"\begin{table}[H]", r"\centering", r"\singlespacing",
         ...
         r"(valid post-selection inference by sample splitting); on each half the adaptive "
         r"elastic net is re-fitted, so the split-sample alpha is available for every "
         r"selector. $\alpha^{\text{OOS}}$: "
         ...
         r">{\RaggedRight\arraybackslash}p{4.2cm}" + " rrr" * len(STRATS) + "}",
         r"\toprule"]
...
def main():
    ...
    # Always emit the paper table, and write it where the manuscript \inputs it
    # (results/tables/), not only into the benchmark scratch directory.
    tables_dir = get_aen_output_dir().parents[1] / "tables"
    tables_dir.mkdir(parents=True, exist_ok=True)
    tex_path = Path(args.tex_out) if args.tex_out else tables_dir / "selector_robustness_article.tex"
