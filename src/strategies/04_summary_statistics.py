...
with open(tex_path, 'w', encoding='utf-8') as f:
    ...
    f.write("\\begin{minipage}{\\textwidth}\n")
    f.write("{\\footnotesize\\noindent Fee applied as: entry DV01 $\\times$ fee\\_bps at "
            "entry $+$ exit DV01 $\\times$ fee\\_bps at exit. "
            "Exit fee waived for trades held to maturity. "
            f"Regime determined by iTraxx Main 5Y level at trade date: "
            f"LOW ($<${thr_low:.0f} bps), MID ({thr_low:.0f}--{thr_high:.0f} bps), "
            f"HIGH ($>${thr_high:.0f} bps).}}\n")
    f.write("\\end{minipage}\n")
    f.write("\\par\\vspace{6pt}\n")
    f.write("\\begin{singlespace}\n")
    f.write("\\small\n")
