...
# Internal key -> display name. The internal keys stay iTraxx_Combined /
# itraxx_combined (file paths, dict keys); only the printed label changes.
STRATEGY_DISPLAY = {
...
def _fmt(x, nd):
    """Format x to nd decimals, avoiding a spurious '-0.00' on values that round to zero."""
    s = f"{x:.{nd}f}"
    return s if float(s) != 0.0 else f"{0.0:.{nd}f}"
...
def calculate_summary_stats(returns, freq='monthly'):
    ...
    # Newey-West corrected t-statistic for the mean (H0: mean = 0), as in
    # Duarte et al. (2007): "t-statistics for the means are corrected for the
    # serial correlation of excess returns." Scale-invariant, so computed on
    # the raw (monthly) returns.
    if n > 1 and std > 0:
        nw_lags = int(np.floor(4 * (n / 100.0) ** (2.0 / 9.0)))
        _ols = sm.OLS(returns_clean.values, np.ones(n)).fit(
...
if strategies_with_daily:
    frequencies_available = ['monthly']
    print(f"📊 Frequenza: monthly (daily returns compounded to month-end)")
...
for freq in frequencies_available:
    ...
    for strategy_name in STRATEGIES:
        ...
        try:
            ...
            # Compound daily percentage returns to month-end (monthly frequency)
            _daily_dec = daily_returns.dropna() / 100.0
            returns = (((1 + _daily_dec).resample("M").prod() - 1) * 100.0).dropna()
            print(f"   🔄 Compounded to monthly: {len(returns)} observations")
    ...
    # Riordina colonne
    col_order = ['Strategy', 'n', 'mean', 't_stat', 'std', 'minimum', 'maximum',
...
# ========================================================================
    ...
    STRATEGY_LATEX_DISPLAY = STRATEGY_DISPLAY
    ...
    if basis_loaded_from_pickle:
        for strat_name in ["BTP_Italia", "CDS_Bond_Basis", "iTraxx_Combined"]:
            if strat_name not in basis_daily:
                continue
            # Month-end sampling of the basis level (monthly frequency)
            s = basis_daily[strat_name].resample("M").last().dropna()
            if len(s) == 0:
                continue
            stats_b = {
                "Strategy": strat_name,
    ...
    def write_panel_b_body(f):
        ...
        for _, row in summary_df.iterrows():
            strategy_display = STRATEGY_DISPLAY.get(row['Strategy'], row['Strategy'].replace('_', ' '))
    ...
    def write_panel_notes(f):
        ...
        if has_panel_a:
            f.write("Panel~A reports daily basis levels in basis points "
                    "(Sep~2008--May~2025 for CDS--Bond, "
                    "Jun~2004--May~2025 for iTraxx~Skew, "
                    "Mar~2012--May~2025 for BTP~Italia). ")
            ...
            f.write("For iTraxx~Skew, it is the mean of the four on-the-run "
    ...
    def write_returns_only_notes(f):
        f.write("\\begin{tablenotes}[para,flushleft]\n")
        f.write("\\footnotesize\n")
        f.write("\\item This table reports summary statistics "
    ...
    with open(latex_article_path, "w") as f:
        ...
        if has_panel_a:
            # --- Descriptive legend ABOVE the table (JF / EDHEC style) ---
            f.write("\\begin{minipage}{\\textwidth}\n")
            f.write("{\\footnotesize\\noindent This table reports summary statistics for "
                    "the signal-weighted arbitrage strategies, net of transaction costs. "
                    "Panel~A reports monthly basis levels (in basis points), with $P_{10}$ "
                    "and $P_{90}$ the 10th and 90th percentiles; Panel~B reports monthly "
                    "percentage excess returns. The basis for each strategy is constructed "
                    "as described in Section~\\ref{sec:strategy_construction}. In Panel~B, the "
                    "mean and standard deviation are annualized and the Sharpe ratio is their ratio; kurtosis is "
                    "excess kurtosis; the $t$-statistic for the mean is corrected for "
                    "serial correlation (Newey--West, 4 lags); and \\%Neg is the proportion of negative "
                    "monthly excess returns. All series end in May~2025; the samples begin in "
                    "April~2012 (BTP Italia), September~2005 (iTraxx Skew), and "
                    "September~2008 (CDS--Bond Basis).}\n")
            ...
            # ---- Panel A: Basis Levels ----
            f.write("\\begin{tabular*}{\\textwidth}{@{\\extracolsep{\\fill}}lrrrrrrrrrr}\n")
            f.write("\\toprule\n")
            f.write("\\multicolumn{11}{l}{\\textbf{Panel A: Basis Levels (bps)}} \\\\\n")
            f.write("\\midrule\n")
            f.write("Strategy & Mean & Median & Std Dev & Min & Max "
            ...
            f.write("\\midrule\n")
            for _, row in basis_summary_df.iterrows():
                display = STRATEGY_LATEX_DISPLAY.get(row['Strategy'], row['Strategy'])
                f.write(f"\\textit{{{display}}} & "
                        f"{row['mean']:.1f} & {row['median']:.1f} & {row['std']:.1f} & "
                        f"{row['minimum']:.1f} & {row['maximum']:.1f} & "
                        f"{row['skewness']:.2f} & {row['kurtosis']:.2f} & "
                        f"{_fmt(row['serial_corr'], 2)} & {row['p10']:.1f} & {row['p90']:.1f} \\\\\n")
            f.write("\\bottomrule\n")
            f.write("\\end{tabular*}\n\n")
            ...
            f.write("\\toprule\n")
            f.write("\\multicolumn{12}{l}{\\textbf{Panel B: Strategy Returns (\\%)}} \\\\\n")
            f.write("\\midrule\n")
            ...
            for _, row in summary_df.iterrows():
                strategy_display = STRATEGY_DISPLAY.get(row['Strategy'], row['Strategy'].replace('_', ' '))
                f.write(f"\\textit{{{strategy_display}}} & "
                        f"{int(row['n'])} & {row['mean']:.2f} & {row['t_stat']:.2f} & "
                        f"{row['std']:.2f} & {row['minimum']:.2f} & {row['maximum']:.2f} & "
                        f"{row['skewness']:.2f} & {row['kurtosis']:.2f} & "
                        f"{row['ratio_negative']*100:.1f} & {_fmt(row['serial_corr'], 2)} & "
                        f"{row['sharpe']:.2f} \\\\\n")
            f.write("\\bottomrule\n")
            f.write("\\end{tabular*}\n")
    ...
    if basis_loaded_from_pickle:
        ...
        fig.tight_layout()
        fig.savefig(FIGURES_DIR / "basis_time_series.pdf",
                    bbox_inches='tight')
        plt.close()
...
if all_trade_stats:
    ...
    with open(trade_tex_path, "w", encoding="utf-8") as f:
        ...
        f.write("\\begin{minipage}{\\textwidth}\n")
        f.write("{\\footnotesize\\noindent This table reports trade-level statistics for "
                "each arbitrage strategy. P\\&L is the cumulative return per trade in "
                "percentage points. Duration is in calendar days. Exit reasons may not "
...
EQUITY_STRATEGIES = {
    ...
    "iTraxx Skew": RESULTS_DIR / "itraxx_combined" / "index_daily.csv",
...
# ============================================================================
# STEP 4-bis: SLIDE-FRIENDLY EQUITY CURVES (2 rows × 3 cols layout)
# ============================================================================
# Same data as paper figure (single source of truth), but transposed layout
# optimized for a 16:9 presentation slide:
#   Row 1: cumulative return (BTP | CDS-Bond | iTraxx)
#   Row 2: drawdown          (BTP | CDS-Bond | iTraxx)
# Output: results/figures/equity_curves_combined_slide.pdf
# ============================================================================

print_header("SLIDE-FRIENDLY EQUITY CURVES (for presentation)")

# Slide ordering: BTP → CDS-Bond → iTraxx (matches benchmark slides)
EQUITY_STRATEGIES_SLIDE = {
    "BTP Italia":      RESULTS_DIR / "btp_italia"      / "index_daily.csv",
    "CDS-Bond Basis":  RESULTS_DIR / "cds_bond_basis"  / "index_daily.csv",
    "iTraxx Skew": RESULTS_DIR / "itraxx_combined" / "index_daily.csv",
}

fig_s, axes_s = plt.subplots(2, 3, figsize=(14, 6),
                             gridspec_kw={"height_ratios": [1.2, 1],
                                          "hspace": 0.40, "wspace": 0.28})

for col_idx, (name, path) in enumerate(EQUITY_STRATEGIES_SLIDE.items()):
    if not path.exists():
        print(f"⚠️  {name}: index_daily.csv not found")
        continue
    ...
    # --- Top row: cumulative return ---
    ax_eq = axes_s[0, col_idx]
    ax_eq.plot(cum_index.index, cum_index.values, color=color, linewidth=0.9)
    ax_eq.set_title(name, fontsize=10, fontweight="bold")
    ax_eq.set_ylabel("Cumulative Return", fontsize=8)
    ax_eq.grid(True, alpha=0.2)
    ax_eq.xaxis.set_major_formatter(mdates.DateFormatter("%Y"))
    ax_eq.tick_params(axis='both', labelsize=7)

    # Sample period annotation
    start = cum_index.index.min().strftime("%b %Y")
    end = cum_index.index.max().strftime("%b %Y")
    ax_eq.text(0.02, 0.95, f"{start} — {end}",
               transform=ax_eq.transAxes, fontsize=6,
               verticalalignment="top", fontstyle="italic", color="grey")

    # --- Bottom row: drawdown ---
    ax_dd = axes_s[1, col_idx]
    ax_dd.fill_between(drawdown.index, drawdown.values, 0,
                       color=color, alpha=0.3)
    ax_dd.plot(drawdown.index, drawdown.values, color=color,
               linewidth=0.6, alpha=0.7)
    ...
    ax_dd.grid(True, alpha=0.2)
    ax_dd.xaxis.set_major_formatter(mdates.DateFormatter("%Y"))
    ax_dd.tick_params(axis='both', labelsize=7)
...
fig_s.savefig(FIGURES_DIR / "equity_curves_combined_slide.pdf", bbox_inches="tight")
plt.close()

print(f"\n💾 equity_curves_combined_slide.pdf → results/figures/  ← PRESENTATION SLIDE")
...
def _fee(x):                                   # mostra il minimo dei decimali, min 1
...
with open(tex_path, 'w', encoding='utf-8') as f:
    ...
    f.write("\\begin{minipage}{\\textwidth}\n")
    f.write("{\\footnotesize\\noindent The fee is applied as entry DV01 $\\times$ fee (bps) at "
            "entry plus exit DV01 $\\times$ fee (bps) at exit; the exit fee is waived for "
            "trades held to maturity. "
            f"The regime is determined by the iTraxx Main 5Y level at the trade date: "
            f"LOW ($<${thr_low:.0f} bps), MEDIUM ({thr_low:.0f}--{thr_high:.0f} bps), "
            f"HIGH ($\\geq${thr_high:.0f} bps).}}\n")
    f.write("\\end{minipage}\n")
    f.write("\\par\\vspace{6pt}\n")
    f.write("\\begin{singlespace}\n")
    f.write("\\small\n")
...
with open(tex_path, 'w', encoding='utf-8') as f:
    ...
    f.write("\\begin{minipage}{\\textwidth}\n")
    f.write(f"{{\\footnotesize\\noindent All thresholds in bps. CDS--Bond Basis is a "
            f"one-directional negative basis trade (long bond, buy CDS protection). "
