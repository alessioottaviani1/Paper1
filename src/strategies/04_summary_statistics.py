    ...
    with open(latex_article_path, "w") as f:
        ...
        if has_panel_a:
            ...
            f.write("{\\footnotesize\\noindent This table reports summary statistics for "
                    ...
                    "Panel~A reports monthly basis levels (in basis points) and Panel~B "
                    "monthly percentage excess returns; the basis for each strategy is "
                    "constructed as described in Section~\\ref{sec:strategy_construction}. In Panel~B, the "
                    ...
                    "serial correlation (Newey--West); \\%Neg is the proportion of negative "
                    "monthly excess returns; and $P_{10}$, $P_{90}$ are the 10th and 90th "
                    "percentiles. All series end in May~2025; start dates differ across "
                    "strategies.}\n")
            ...
            for _, row in basis_summary_df.iterrows():
                ...
                f.write(f"\\textit{{{display}}} & "
                        ...
                        f"{row['serial_corr']:.3f} & {row['p10']:.1f} & {row['p90']:.1f} \\\\\n")
            f.write("\\bottomrule\n")
            f.write("\\end{tabular*}\n\n")
            ...
            for _, row in summary_df.iterrows():
                ...
                f.write(f"\\textit{{{strategy_display}}} & "
                        ...
                        f"{row['skewness']:.2f} & {row['kurtosis']:.2f} & "
                        f"{row['ratio_negative']*100:.1f} & {row['serial_corr']:.3f} & "
                        f"{row['sharpe']:.2f} \\\\\n")
            f.write("\\bottomrule\n")
            f.write("\\end{tabular*}\n")
