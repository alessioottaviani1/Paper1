...
def write_basic_metrics_latex_article(metrics_by_strategy, out_path):
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        ...
        f.write("{\\footnotesize\\noindent This table reports monthly percentage excess "
                "returns for the equal-weighted arbitrage strategies, net of transaction "
                "costs; it is the equal-weighted counterpart of Panel~B of "
                "Table~\\ref{tab:summary_stats_monthly}. The mean and standard deviation "
...
def write_moreira_muir_performance_latex_article(mm_data, out_path, variant="SW",
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        ...
        f.write("{\\footnotesize\\noindent Following Moreira and Muir (2017). The "
                ...
                "standard errors. $^{*}$, $^{**}$, $^{***}$ denote significance at the "
                "10\\%, 5\\%, and 1\\% levels. Returns are net of transaction costs.}\n")
        f.write("\\end{minipage}\n")
        f.write("\\medskip\n\n")
...
def main():
    ...
    write_moreira_muir_performance_latex_article(
        mm_data, out3_article, variant="SW",
        caption="Moreira-Muir Volatility-Managed Portfolio Performance",
        label="tab:moreira_muir",
    )
    print(f"✅ Saved: {out3_article.name}")
    out3_ew_article = TABLES_DIR / "moreira_muir_performance_monthly_ew_article.tex"
    write_moreira_muir_performance_latex_article(
        mm_data, out3_ew_article, variant="EW",
        caption="Moreira-Muir Volatility-Managed Portfolio Performance (Equal-Weighted)",
        label="tab:moreira_muir_ew",
    )
    print(f"✅ Saved: {out3_ew_article.name}")
