...
\begin{split}
  ...
  &+ \beta_5\,\textit{ComOpt}_t
   + \beta_6\,\Delta\textit{10Y}_t
   + \beta_7\,\Delta\textit{CredSpr}_t + \varepsilon_{i,t},
\end{split}
\end{equation}
...
$\Delta\textit{10Y}$ is the change in the 10-year Treasury yield,
and $\Delta\textit{CredSpr}$ is the change in the Baa--Treasury
credit spread.
...
truncation lag set as $\lfloor T^{1/4} \rfloor$ following
\citet{newey1994automatic}. Variance inflation factors for each specification are reported in
...
report the full regression results. Across all three frameworks, all
three strategies, and both currency denominations, the estimated
intercept is positive and statistically significant at the one-percent
level. The annualised alphas are economically meaningful:
approximately $+5\%$ p.a.\ for the CDS--Bond basis across all
specifications, $+1.6$--$3.5\%$ for iTraxx Combined, and
...
market significant for BTP~Italia but not for iTraxx Combined. These
...
the three frameworks and decomposes them by market regime. Panel~A
...
Panel~B decomposes alpha into two sub-samples defined by the level of
the iTraxx Europe Main 5-year spread, using the same regime
classification introduced in Section~\ref{sec:performance}: HIGH-stress
months are those in which the spread exceeds $100$~basis points, and
NORMAL the remaining observations. In every cell of the table, alpha during HIGH-stress
months is substantially larger than alpha during NORMAL months, in most specifications by a factor of two
to four. The pattern holds across all three benchmark
specifications and across all three strategies, and it does not depend on
the precise threshold used to define the HIGH regime: in
Appendix~\ref{app:subperiod} we report the same exercise at threshold
levels of 80, 100, and 120 basis points, with similar results.
...
documented in Panel~B, and it is also the empirical pattern that
...
\citet{christopherson1998conditioning}, which lets both the alpha and the
factor loadings vary continuously with a publicly observable state variable
rather than across a binary regime split. The specification is
\begin{equation}\label{eq:cfg}
  r_{i,t} \;=\; \alpha_0 \;+\; \alpha_1\, z_{t-1}
  \;+\; \bigl(\boldsymbol{\beta} + \boldsymbol{\delta}\, z_{t-1}\bigr)'\,
  \mathbf{X}_{t} \;+\; \varepsilon_{t},
\end{equation}
where $r_{i,t}$ is the strategy excess return, $\mathbf{X}_t$ collects the factor
returns of the chosen benchmark, and $z_{t-1}$ is the one-period-lagged,
standardised level of the iTraxx Europe Main 5-year spread:
$z_{t-1} = (s_{t-1} - \bar{s})/\hat{\sigma}_s$. Conditioning on the
\emph{predetermined} state $z_{t-1}$ follows the public-information logic of
\citet{christopherson1998conditioning} and avoids the mechanical endogeneity of
a contemporaneous conditioner. The state is mean-zero and unit-variance over the
sample, so $\alpha_0$ retains its interpretation as the unconditional alpha,
$\alpha_1$ measures the additional alpha per one-standard-deviation increase in
lagged funding stress, and $\boldsymbol{\delta}$ captures the state-dependence of
the factor loadings. Inference on $\alpha_1$ is by a moving-block bootstrap with
$H_0{:}\,\alpha_1=0$ imposed, with \citet{newey1994automatic} HAC standard errors
...
The estimates of $\alpha_1$ in Panel~C are positive for all three
...
the stress state. For CDS--Bond Basis and iTraxx Combined, $\alpha_1$
...
is earned during periods of elevated funding stress. Reading Panel~B and Panel~C
together, the unconditional alpha masks a pronounced time variation
...
return that an investor could have earned in real time. Panel~D of
...
Appendix~\ref{app:subperiod} and delivers the same conclusion.
...
but persist throughout, and the elevated-alpha intervals are not
confined to one crisis: they recur across the distinct stress episodes
spanned by the sample. The three benchmark estimates move broadly
...
sub-period splits and threshold robustness for each framework
