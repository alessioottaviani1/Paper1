...
if __name__ == "__main__":
    ...
    ONLY_CCTEU = True   # restringe l'analisi ai CCTeu (arbitraggio replicabile, conversione
                        # di mercato). I CCT-BOT (model-based, non arbitrabili) restano in
                        # basis_daily ma escono dai test finche' non si definisce con RR come
                        # trattarli rigorosamente (DM vs z-spread / proxy di copertura).
    if ONLY_CCTEU:
        n0 = len(B); B = B[B.regime == "CCTeu"].copy()
        print(f"  [ONLY_CCTEU] {n0:,} -> {len(B):,} oss. (soli CCTeu)")
