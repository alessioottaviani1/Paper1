...
if __name__ == "__main__":
    ...
    try:
        ...
        if len(d) > 100:
            d["mon"]=d.date.dt.month.astype(str); d["yr"]=d.date.dt.year.astype(str)
            # STESSE tre specifiche del blocco B: fermarsi alla grezza farebbe fare la
            # figura migliore proprio alla specifica non identificata (confonde la scadenza).
            run(d, "basis_p ~ sd_diff + C(mon) + C(yr)",                   "(C1) ex-crisi, grezza")
            run(d, "basis_p ~ sd_diff + tau + I(tau**2) + C(mon) + C(yr)", "(C2) ex-crisi + tau, tau^2")
            run(d, "basis_p ~ sd_diff + C(cct) + C(mon) + C(yr)",          "(C3) ex-crisi + FE di CCT")
            P("  [il numero che conta e' (C3): il verdetto sul premio di stabilita' si legge")
            P("   nella specifica identificata, dentro e fuori dalle crisi.]")
    except Exception:
        pass
