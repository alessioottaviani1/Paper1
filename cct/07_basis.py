    ...
    for _, pr in PAIRS.iterrows():
        ...
        if b is None or pd.isna(b["coupon"]): continue
                ...
                idx = (PXC[cct].dropna().index.intersection(PXB[btp].dropna().index)
                       .intersection(CRV.index))
        if pr["regime"] == "CCTeu":            # gli IRS servono solo ai CCTeu; per i
            idx = idx.intersection(MKT.index)  # CCT-BOT il K viene dalla curva sovrana:
                                               # intersecare MKT (dal 1999) ucciderebbe il 1995-98
        ...
        for d in idx:
            ...
            tau_b = (pr["BTP_mat"] - d).days / 365.25
                        # sotto ~6 mesi la base e' dominata da rateo e fixing, e per tau<1 il K e'
            # estrapolato flat dal tenor 1y (np.interp): il tratto cortissimo e' rumore.
            if tau_c < 0.50 or tau_b < 0.50: continue
            ...
            if pr["regime"] == "CCTeu":
                ...
                # EUSA = fisso ANNUALE 30/360: spalmarlo in K/2 semestrali sopravvaluta la
                # gamba fissa di ~K^2/4 in decimali (2.2 bp a K=3%, 6.1 a K=5%); lo spread
                # CCTeu matura ACT/360 (+1.46% del suo valore). Entrambe le distorsioni
                # variano col LIVELLO dei tassi: siedono sul regressore del test 12A.
                K_semi = 2.0 * ((1.0 + K / 100.0) ** 0.5 - 1.0) * 100.0
                cpn_syn = s * 365.25 / 360.0 + K_semi
            else:
            ...
            # PRIMA CEDOLA: gia' FISSATA al reset precedente (nota), mentre lo swap par
            # stipulato oggi fissa oggi. Il primo flusso del pacchetto e':
            #   cedola_nota + (K - E_oggi) * stub_residuo
            # Lo status quo usa (s+K)/2, cioe' il forward al posto del fissato: errore
            # = 0.5*(E_fix - E_oggi), mezzo Delta-Euribor realizzato (~1.4 punti nel 2022),
            # per costruzione correlato con Dtassi, il regressore dei test 09/10/12A.
            if len(nxt) and np.isfinite(nxt.coupon_semi_pct.iloc[0]) and len(t_syn):
                cpn_known = float(nxt.coupon_semi_pct.iloc[0])
                stub = float(t_syn[0])
                if pr["regime"] == "CCTeu" and "euribor6m" in MKT.columns \
                        and d in MKT.index and np.isfinite(MKT.loc[d, "euribor6m"]):
                    e_ref = float(MKT.loc[d, "euribor6m"])
                else:                       # CCT-BOT (o euribor assente): proxy 6m dalla curva
                    e_ref = float(nss(np.array([0.5]), p_par)[0])
                a_syn[0] = cpn_known + (K - e_ref) * stub
