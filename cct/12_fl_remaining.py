...
if __name__ == "__main__":
    ...
    try:
        ...
        P("")
        P(f"  {'meccanismo':34}{'poolato':>18}{'con eff. fissi':>20}")
        P(f"  {'stabilita mark-to-market (09)':34}{'-1.60 [-2.78]':>18}{'-0.31 [-1.12]  NO':>20}")
        P(f"  {'quasi-moneta / Nagel (12A)':34}{'+0.13 [+3.29]':>18}{'vedi sopra':>20}")
        P(f"  {'scarsita offerta (12B)':34}{'+0.03 [+2.53]':>18}{'vedi sopra':>20}")
        P(f"  {'stress sovrano (10)':34}{'-0.98 [-4.19]':>18}{'-0.77 [-3.34]  SI':>20}")
        P(f"  {'clientela x dimensione (10c)':34}{'-1.43 [-2.39]':>18}{'-1.79 [-2.35]  SI':>20}")
        P("")
        P("  I due che sopravvivono sono quelli identificati con l'analisi economica, non")
        P("  per ricerca esplorativa. Nel paper questo va presentato come RISULTATO: cinque")
        P("  meccanismi candidati testati con lo stesso criterio, tre respinti, due confermati.")
    except ImportError:
