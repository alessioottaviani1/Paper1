...
# Il proxy dai prezzi copre il tratto 1996-2001 (l'ufficiale 6M parte dal 2002); si
# costruisce SEMPRE, e poi l'ufficiale lo SOVRASCRIVE dove disponibile. Cosi' la
# regola B del 1995-2001 resta popolata dal proxy, il resto dall'ufficiale.
px = PROC/"px_bot.csv"
...
if px.exists():
    ...
    for _, b in sem.iterrows():
        ...
        sp = PX[b["isin"]].dropna()
        sp = sp[sp.index >= b.issue]
        if sp.empty: continue
        p0, d0 = float(sp.iloc[0]), sp.index[0]
        n = (b.maturity - d0).days
        if not (0 < p0 < 120) or n <= 0: continue
        rows[d0] = (100.0 - p0) / p0 * 360.0 / n * 100.0     # in punti percentuali
    proxy = pd.Series(rows).sort_index()
    P(f"[proxy dai prezzi] {len(proxy)} aste ricostruite (copre 1996-2001)")

off = PROC/"bot_auction_6m_official.csv"
if not off.exists():
    P("[STOP] bot_auction_6m_official.csv assente. Questo script DIPENDE dalle aste ufficiali")
    P("       Banca d'Italia: lanciare PRIMA  03c_bankit_auctions.py  poi rilanciare questo.")
    P("       (l'ordine numerico inganna: 03c va eseguito PRIMA di 03 -- e' l'unica eccezione")
    P("        all'ordine numerico nell'intera pipeline.)")
    save_txt("03_bot_auction.txt", L); print("\n".join(L)); raise SystemExit
if off.exists():
    s_off = pd.read_csv(off, index_col=0, parse_dates=True).iloc[:,0].dropna()
    P(f"[fonte UFFICIALE] {off.name}: {len(s_off)} aste, {s_off.index.min().date()} -> {s_off.index.max().date()}")
    # unione: ufficiale prevale, proxy riempie il pre-2002
    s = proxy.combine_first(s_off)
    s.update(s_off)      # dove c'e' l'ufficiale, vince l'ufficiale
    s = s.sort_index()
    P(f"[6M combinata] {len(s)} aste totali, {s.index.min().date()} -> {s.index.max().date()} "
      f"(proxy pre-2002: {int((s.index < '2002-01-01').sum())})")
elif len(proxy):
    s = proxy
else:
    P("[STOP] ne' ufficiale ne' px_bot: lanciare 02_pull_prices.py o fornire il file ufficiale")
    save_txt("03_bot_auction.txt", L); print("\n".join(L)); raise SystemExit
