...
if __name__ == "__main__":
    ...
    ONLY_CCTEU = True   # le predizioni (P1 asimmetria, P3 haircut, P4 concentrazione) si
                        # testano sui CCTeu, l'unico campione con base arbitrabile. I CCT-BOT
                        # restano per l'estensione, dopo la guida RR sulla loro costruzione.
    if ONLY_CCTEU:
        nM, nB = len(M), len(B)
        M = M[M.regime == "CCTeu"].copy()
        B = B[B.regime == "CCTeu"].copy()
        print(f"  [ONLY_CCTEU] mechanism_panel {nM:,}->{len(M):,} | basis {nB:,}->{len(B):,}")
    try:
