...
if __name__ == "__main__":
    ...
    S = build_schedule(C, bot, eur, spr)
    L = []; P = L.append
    ...
    P(f"convenzioni: {S.dc.value_counts().to_dict()}")
        # regola A: finche' non esiste bot_auction_12m.csv il parametro e' un PROXY dai BOT 6M
    # (la norma vuole i BOT ANNUALI): flag esplicito perche' nulla lo usi in silenzio.
    S["param_series"] = np.where(S["rule"].eq("A"), "6m_PROXY", "native")
