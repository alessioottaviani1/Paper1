...
def build_ar1_persistence():
    ...
    tex.append(r"\begin{tablenotes}[para,flushleft]")
    tex.append(r"\item \textit{Note:} "
               r"AR(1) regression of mispricing levels $m_{i,t} = c_i + \phi_i m_{i,t-1} "
               r"+ u_{i,t}$, monthly frequency. "
               r"Half-life $h_i^{*} = \ln(0.5) / \ln(\phi_i)$ measures the "
               r"number of months for the mispricing level to halve following a "
               r"shock. The CDS--Bond basis exhibits the highest persistence, "
               r"consistent with its requirement of continuous repo financing of "
               r"the cash bond \citep{boyarchenko2016trends}; the iTraxx skew, "
               r"a pure derivative basis, exhibits the lowest. Newey--West HAC "
               r"standard errors with six lags. "
               r"$^{***}\,p<0.01$, $^{**}\,p<0.05$, $^{*}\,p<0.10$.")
    tex.append(r"\end{tablenotes}")
    tex.append(r"\end{threeparttable}")
    tex.append(r"\end{table}")
...
def build_pc1_funding():
    ...
    set_labels = {
        ...
        'Macro': 'Panel C: Macroeconomic Placebo',
    }
    ...
    tex.append(r"\item \textit{Note:} "
               ...
               r"$\Delta m$ series on intermediary stress proxies and on a macro "
               r"placebo set. "
               ...
               r"Panel~C: macroeconomic placebo set "
               r"(macro and real-activity uncertainty, long-term inflation "
               r"expectations, economic policy uncertainty). "
               r"Significant coefficients in Panels~A and~B indicate that the "
               r"common factor driving correlated mispricings is linked to "
               r"intermediary balance-sheet conditions "
               r"\citep{duffie2010presidential, brunnermeier2009market, "
               r"he2017intermediary}. "
               r"Non-significant coefficients in Panel~C confirm that the common "
               r"factor is not driven by macroeconomic fundamentals. "
               r"Newey--West HAC standard errors. "
               r"$^{***}\,p<0.01$, $^{**}\,p<0.05$, $^{*}\,p<0.10$.")
    tex.append(r"\end{tablenotes}")
    tex.append(r"\end{singlespace}")
...
def build_slide_p2_interaction():
    ...
    Reads T3d_interaction_regressions.csv and shows the β₂ coefficient
    (the interaction Δm_j × z_t) for each direction, with short labels
    for slide readability.
    ...
    tex.append(r"\toprule")
    tex.append(r"Direction & $\hat\beta_2$ \\")
    tex.append(r"\midrule")
    for _, row in df.iterrows():
...
def build_slide_p3_spanning_granger():
    ...
    tex.append(r"\centering")
    tex.append(r"\textbf{Cross-strategy spanning ($\bar R^2$):}\\[0.10cm]")
    tex.append(r"\renewcommand{\arraystretch}{1.20}")
    tex.append(r"\setlength{\tabcolsep}{6pt}")
    tex.append(r"\begin{tabular}{lc}")
    tex.append(r"\toprule")
    tex.append(r"Dependent: $\Delta m_{i,t}$ & $\bar R^2$ \\")
    tex.append(r"\midrule")
    if df_span is not None:
        # The spanning CSV has one row per dependent strategy, each with R2_adj
        ...
        # Take the strategy-level summary (one row per strategy)
        df_uniq = df_span.drop_duplicates(subset=col_dep, keep='first')
        for _, row in df_uniq.iterrows():
            dep = str(row[col_dep]).replace('_', r'\_')
            r2a = row.get(col_r2, np.nan)
            tex.append(rf"{dep} & {r2a*100:.1f}\% \\")
    tex.append(r"\bottomrule")
    tex.append(r"\end{tabular}")
    ...
    tex.append(r"\begin{tabular}{lc}")
    tex.append(r"\toprule")
    tex.append(r"Direction & $p$-value \\")
    tex.append(r"\midrule")
    if df_grg is not None:
        # Try common column names
        col_from = 'cause' if 'cause' in df_grg.columns else 'from'
        col_to = 'caused' if 'caused' in df_grg.columns else 'to'
        col_p = 'p_value' if 'p_value' in df_grg.columns else 'p'
        for _, row in df_grg.iterrows():
            f_ = str(row.get(col_from, '')).replace('_', r'\_')
            t_ = str(row.get(col_to, '')).replace('_', r'\_')
            pv = row.get(col_p, np.nan)
            stars = _stars(pv)
...
def _build_mapping_from_csvs():
    ...
    """
    # P1: unconditional ρ (best institutional pair, Δm)
    p1_evidence = "See Table~\\ref{tab:rq3_unified_corr}"
    df_uncond = _load_csv("T2a_unconditional_correlations.csv")
    if df_uncond is not None:
        # Take the strongest positive among Δm
        df_dm = df_uncond[df_uncond['series'].astype(str).str.contains("m", case=False)] \
                if 'series' in df_uncond.columns else df_uncond
        if len(df_dm) > 0:
            best = df_dm.loc[df_dm['pearson_r'].idxmax()]
            p1_evidence = (rf"$\rho_{{\Delta m}} = {best['pearson_r']:+.2f}{_stars(best.get('pearson_p_hac',1))}$ "
                           rf"({str(best.get('pair','')).replace('_',r' ')})")
...
# P4 (NEW): persistence reflects funding-dependence (AR(1) on m_t levels)
    ...
    if df_ar is not None:
        ...
        if len(df_m_only) >= 2:
            order = df_m_only.sort_values('half_life_months', ascending=False)
            p4_evidence = " > ".join(
    ...
    for pid, statement, evidence in rows:
        tex.append(rf"{pid} & {statement} & {evidence} & "
                   r"\textcolor{mygreen}{\large\checkmark} \\")
    tex.append(r"\bottomrule")
    tex.append(r"\end{tabular}")
    return "\n".join(tex)
...
def main():
    ...
    body_tables = {
        ...
        "RQ3_PC1_Funding_article.tex":
            build_pc1_funding(),
        "RQ3_Primary_Outcomes_MTC_article.tex":
            build_primary_outcomes_correction(),
    }
