...
if BASIS_APPROACH == "market":
    ...
    if itraxx_all_means:
        ...
        if MISPRICING_AGG_METHOD in ("A", "B"):
            ...
            if ITRAXX_AGG == "main_only":
                if "Main" not in sub_cols:
                    raise ValueError("ITRAXX_AGG='main_only' ma colonna 'Main' assente")
                itrx_agg = sub["Main"].abs().dropna()

            elif ITRAXX_AGG == "raw":
                # media delle magnitudini grezze (no ribilanciamento scala)
                itrx_agg = sub.abs().mean(axis=1).dropna()

            elif ITRAXX_AGG == "zscore":
                # z-score EXPANDING per ogni sub-skew (causale, no look-ahead),
                # poi media delle magnitudini standardizzate
                z = pd.DataFrame(index=sub.index, columns=sub_cols, dtype=float)
                for c in sub_cols:
                    s = sub[c]
                    mu = s.expanding(min_periods=20).mean()
                    sd = s.expanding(min_periods=20).std()
                    z[c] = (s - mu) / sd
                itrx_agg = z.abs().mean(axis=1).dropna()

            elif ITRAXX_AGG == "relative":
                if itraxx_spreads is None:
                    raise ValueError("ITRAXX_AGG='relative' ma spread non caricati")
                # allineamento difensivo skew↔spread per data
                ...
                sub_a = sub.loc[common]
                sp_a = itraxx_spreads.loc[common, sub_cols]   # stesse 4 colonne, stesso ordine
                # dislocazione relativa: |skew_i| / spread_i  (spread>0)
                sp_safe = sp_a.where(sp_a > 1e-6, np.nan)
                rel = sub_a.abs() / sp_safe
                itrx_agg = rel.mean(axis=1).dropna()
