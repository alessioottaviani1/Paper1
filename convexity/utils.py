...
def load_legs_mid():
    """Mid giornalieri delle 12 gambe del fly dal foglio 'swap' (PX_LAST)."""
    raw=load_bbg_sheet(SHEET_SWAP)
    byname={}
    for (t,j),v in raw.items():
        byname.setdefault(t.replace(" Curncy",""), v)
...
def load_halfspreads(index):
    ...
    H=pd.DataFrame({l: float(HS_BASE[l])*mult for l in need})
    return H, f"MOVE-tiered (Paper 1 A.III): x{m0}/x{m1}/x{m2} a MOVE<{t1:.0f}/<{t2:.0f}/>={t2:.0f}"
