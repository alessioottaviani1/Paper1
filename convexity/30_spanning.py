...
def curve_factors(m):
    ...
    Due accortezze. Le gambe si prendono da MK invece di ricostruirle per concatenazione:
    USGG2 non esiste (e' USGG2YR) e con la vecchia versione le quattro governative cadevano
    in silenzio. E mid = load_legs_mid_all() e' un DIZIONARIO di serie, non un DataFrame,
    quindi si indicizza una chiave per volta."""
    if m == "USTgovt":                      # UST: gambe GSW, non nel foglio Bloomberg
        try:
            z = pd.read_csv(PROC/"gsw_nodes_monthly.csv", index_col=0, parse_dates=True)
        except FileNotFoundError:
            return None
        z.columns = ["z2","z10","z30"]
        z = z*100.0                          # GSW e' salvato in decimali: alla scala % delle altre
    else:
        if m not in MK: return None
        legs = MK[m][0]
        if any(l not in mid for l in legs): return None
        z = pd.DataFrame({k: mid[l] for k, l in zip(("z2","z10","z30"), legs)}
                         ).dropna().resample("ME").last()
    return pd.DataFrame({"lev": z.z10, "slo": z.z10-z.z2, "cur": 2*z.z10-z.z2-z.z30})
