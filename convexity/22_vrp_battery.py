...
# ---------------------------------------------------------------- [7] euro vs Bund
P("")
P("[7] IL RESIDUO EURO E' DELLA CURVA SWAP O DELLA VALUTA? (stessa decomposizione sul Bund)")
gov_candidates = ["sigbe_gov_monthly.csv", "sigbe_govt_monthly.csv", "sigbe8_monthly.csv"]
gov = None
for c in gov_candidates:
    try:
        g = pd.read_csv(PROC/c, index_col=0, parse_dates=True)
        gov = g; P(f"    (file governativo: {c}, colonne: {list(g.columns)})"); break
    except Exception: pass
if gov is None:
    de_col = next((c for c in sbe.columns if "DE" in c.upper() or "BUND" in c.upper()), None)
    if de_col is not None: gov = sbe[[de_col]].rename(columns={de_col:"DE"})
if gov is not None:
    de = next((c for c in gov.columns if "DE" in c.upper() or "BUND" in c.upper()), None)
    if de:
        # Bund: RV dal decennale tedesco se disponibile, altrimenti dallo swap euro (proxy)
        try:
            dy_de = (mid["GDBR10"]/100.0)
        except Exception:
            dy_de = daily10("EUR")
            P("    (RV Bund proxy: decennale swap EUR -- GDBR10 non in mid)")
        rvde = rv_from(dy_de, 63).shift(-H_M)
        iv = ivser("EUR")
        al = pd.concat([gov[de].rename("be"), iv.rename("iv"), rvde.rename("rv")], axis=1).dropna()
        if len(al) >= 60:
            v = al["iv"]-al["rv"]; r = al["be"]-al["rv"]
            P(f"    Bund : W={ (al['be']-al['iv']).mean():6.1f}  VRP={v.mean():6.1f}  R={r.mean():6.1f} [t {nw(r):4.1f}]  T={len(al)}")
            alx = decomp("EUR", rv_fwd("EUR"))
            rz = alx["be"]-alx["rv"]
            P(f"    Swap : W={ (alx['be']-alx['iv']).mean():6.1f}  VRP={(alx['iv']-alx['rv']).mean():6.1f}  R={rz.mean():6.1f} [t {nw(rz):4.1f}]  T={len(alx)}")
            P("    Se il residuo e' nello SWAP e non nel Bund, il canale e' collaterale/clearing --")
            P("    specifico dello strumento, non 'dell'Europa'. E' l'identificazione within-currency")
            P("    applicata alla domanda di Rebonato.")
    else:
        P("    colonna Bund non trovata nel file governativo -- pannello saltato")
else:
    P("    serie governative non trovate in processed/ -- pannello saltato (rigenerare da 02 gov)")
