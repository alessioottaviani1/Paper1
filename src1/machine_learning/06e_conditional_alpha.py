...
Five tests, grounded in the conditional performance evaluation literature:
  ...
  (e1) Christopherson–Ferson–Glassman (1998, RFS) conditional alpha + beta:
       r_t = α₀ + α₁·z_t + Σⱼ (βⱼ + δⱼ·z_t)·Xⱼt + εₜ
       - z_t = standardized conditioning variable (continuous)
       - α₁ tests whether the ALPHA varies with stress (CFG's extension of FS)
       - δⱼ tests whether factor exposures are time-varying (the FS conditional betas)
       - Joint F-test on all interaction terms {α₁, δ₁,...,δₖ}
       CFG (1998) extend Ferson–Schadt (1996) by conditioning the alpha, not only the betas.
...
References:
    Christopherson, J. A., Ferson, W. E. and Glassman, D. A. (1998),
        "Conditioning Manager Alphas on Economic Information: Another Look
        at the Persistence of Performance", Review of Financial Studies,
        11(1), 111-142.
...
def ferson_schadt_conditional(strategy_name, strategy_path):
    """
    Christopherson–Ferson–Glassman (1998, RFS) conditional performance model.

    r_t = α₀ + α₁·z_t + Σⱼ βⱼ·Xⱼt + Σⱼ δⱼ·(Xⱼt·z_t) + εₜ

    where z_t is a standardized conditioning variable. CFG condition the ALPHA
    (α₁) on z, extending Ferson–Schadt (1996), who condition only the betas.
    ...
    """
    print_header(f"   (e1) Christopherson–Ferson–Glassman Conditional Model — {strategy_name}", "─")

    y, X, stable_factors = prepare_data(strategy_name, strategy_path)
    T, k = len(y), len(stable_factors)

    # Load primary conditioning variable, LAGGED to z_{t-1} (predetermined).
    # CFG (1998) and Ferson-Schadt (1996) both condition on PUBLIC, lagged info:
    # the alpha/betas at t depend on information known at t-1.  Using z_t
    # contemporaneously would make the conditioning variable endogenous w.r.t.
    # the contemporaneous residual.  Factors X stay contemporaneous.
    z_raw = load_stress_proxy_monthly("ITRX_MAIN")
    z_aligned = z_raw.reindex(y.index, method='nearest').shift(1)   # z_{t-1}
    # Standardize on the post-lag sample (unit sd -> alpha1 per 1 sd of z_{t-1})
    z = (z_aligned - z_aligned.mean()) / z_aligned.std()
    z = z.dropna()

    # Realign after the lag drops the first observation
    common = y.index.intersection(z.index)
    y, X, z = y.loc[common], X.loc[common], z.loc[common]
    T = len(y)

    print(f"\n   Stable factors: {stable_factors}")
    print(f"   Conditioning variable: iTraxx Main 5Y, lagged z_(t-1) (standardized)")
    print(f"   T = {T}, k = {k}")
    ...
    alpha1_p = cfg_alpha1_bootstrap_p(y, X, z)
    ...
    print(f"\n   ── Conditional Model (Christopherson–Ferson–Glassman 1998) ──")
    ...
    result = {
        ...
        'conditional': {
            ...
            'alpha1_pval': round(float(alpha1_p), 4),          # block-bootstrap p (HEADLINE)
            'alpha1_pval_hac': round(float(alpha1_p_hac), 4),  # HAC p (robustness)
...
def multiple_conditioning_variables(strategy_name, strategy_path):
    ...
    for proxy_name, cfg in CONDITIONING_VARIABLES.items():
        ...
        alpha1_p = cfg_alpha1_bootstrap_p(y_sub, X_sub, z_std)  # bootstrap (headline)
        ...
        results[proxy_name] = {
            ...
            'alpha1': round(float(alpha1), 6),
            'alpha1_tstat': round(float(alpha1_t), 4),
            'alpha1_pval': round(float(alpha1_p), 4),          # block-bootstrap p
            'alpha1_pval_hac': round(float(alpha1_p_hac), 4),  # HAC p (robustness)
...
def _build_fs_beamer(strategy_name, fs_result):
    ...
    for f, d in deltas.items():
        tex.append(rf"{_pretty_factor(f)} $\times z_{{t-1}}$ & "
    ...
    tex.append(r"{\tiny Christopherson, Ferson \& Glassman (1998, \textit{RFS}). "
               r"$z_{t-1}$ = standardized iTraxx Main 5Y, lagged one month. "
...
def _build_fs_thesis(all_results):
    ...
    tex.append(r"\item \textit{Note:} "
               ...
               r"both the alpha and the factor loadings are conditioned on $z_{t-1}$, the one-month-lagged standardized "
...
def _build_multiproxy_beamer(all_results):
    ...
    for s in strategies:
        ...
        tex.append(r"\begin{tabular}{l r r r}")
        tex.append(r"\toprule")
        tex.append(r"Conditioning variable ($z_{t-1}$) & "
        ...
        for pn, cfg in CONDITIONING_VARIABLES.items():
            ...
            if r.get('skip'):
                tex.append(rf"{cfg['label']} & "
                           r"\multicolumn{3}{c}{not available} \\")
            else:
                tex.append(
                    rf"{cfg['label']} & "
    ...
    tex.append(r"\vspace{0.15cm}")
    tex.append(r"{\tiny Christopherson, Ferson \& Glassman (1998) model with alternative "
               r"conditioning variables; Patton (2009, \textit{RFS}). "
               r"$z_{t-1}$ standardized to mean zero, unit variance, lagged one month. "
               r"HAC standard errors. "
               r"*** $p{<}1\%$, ** $p{<}5\%$, * $p{<}10\%$.}")
    tex.append(r"\end{frame}")
    return "\n".join(tex)
...
def _build_multiproxy_thesis(all_results):
    ...
    tex.append(r"\footnotesize\textit{Notes:} "
               r"Each column reports $\alpha_1$ from the "
               r"\citet{christopherson1998conditioning} full conditional model "
               r"$r_t = \alpha_0 + \alpha_1 z_{t-1} + "
               r"\sum_j (\beta_j + \delta_j z_{t-1}) X_{jt} + \varepsilon_t$, "
               r"estimated with a different (one-month-lagged) conditioning variable $z_{t-1}$ "
               r"(standardized to zero mean and unit variance). "
               r"The $t$-statistic is Newey--West HAC; the reported $p$-value and "
               r"significance stars are from a moving-block bootstrap "
               r"($B=9{,}999$, $H_0{:}\,\alpha_1=0$ imposed, block~9), finite-sample "
               r"valid \citep{kiefer2005new,lazarus2018har}. "
               r"$\alpha_1 > 0$ in every specification indicates that alpha increases "
               r"with financial stress, robust to the choice of proxy. "
               r"*** $p < 0.01$, ** $p < 0.05$, * $p < 0.10$.")
...
def main():
    ...
    print(f"\n   References:")
    print(f"   - Christopherson, Ferson & Glassman (1998, RFS): conditional alpha + beta")
    ...
    if all_results:
        ...
        # Ferson-Schadt α₁
        print(f"\n   ── Christopherson–Ferson–Glassman α₁ (conditional alpha on iTraxx Main) ──")
