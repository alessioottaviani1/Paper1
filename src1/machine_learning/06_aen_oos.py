# -*- coding: utf-8 -*-
"""
AEN OUT-OF-SAMPLE ALPHA  (alpha_0, the unconditional level)
===========================================================

Implementability test for the AEN unconditional alpha, the direct analogue of
the factor-benchmark Panel D (oos.py).  The in-sample AEN alpha (06e) is a
full-sample OLS intercept on the stable set: the hedge loadings use the whole
period, so the implied hedge is formed with hindsight (Moreira-Muir in-sample-
weights critique; Tessaromatis implementability).  Here we FREEZE the stable
set selected by stability selection and re-estimate ONLY the loadings on a
PAST-ONLY window, evaluating the realised abnormal return on the next month:

        e_mod_t = r_t - beta_hat'_{t-1} F_t          (slopes only)

NO LASSO is run here.  The factors are the AEN stable set (fixed, named series
from bootstrap_stability.json), so there is no per-window re-selection and no
rotation/sign ambiguity (unlike the PCA OOS).  The estimand is the FROZEN-set
loadings-OOS alpha_0 -- the fair counterpart of the benchmark Panel D and the
PCA OOS, computed with the SAME engine (oos.oos_residuals / oos.summarize) so
the three layers sit on identical ground.
  ...
  NOT covered here -- the stability selection is on the unconditional model and
  does not protect alpha_1, which requires valid post-selection inference
  (PDS / re-selection-bootstrap, in the post-selection module).

Author: Alessio Ottaviani, EDHEC Business School.
"""

# ============================================================================
#  TOGGLE  --  switch the OOS scheme by editing this ONE line.
# ============================================================================
OOS_WINDOW = "expanding"          # "expanding"  |  "rolling"
# ============================================================================
OOS_MIN_TRAIN   = 60              # months before the first OOS evaluation
OOS_ROLL_WINDOW = 60              # rolling-window length (used if OOS_WINDOW="rolling")
...
def main():
    mode = OOS_WINDOW.lower()
    if mode not in ("expanding", "rolling"):
        raise ValueError("OOS_WINDOW must be 'expanding' or 'rolling'")

    bar = "=" * 92
    print(bar)
    print(f"AEN OUT-OF-SAMPLE ALPHA  (alpha_0, frozen stable set, past-only beta)"
          f"   window = '{mode.upper()}'")
    print(bar)
    win_note = (f"rolling {OOS_ROLL_WINDOW}m" if mode == "rolling"
                else f"expanding, min_train={OOS_MIN_TRAIN}")
    print(f"  NO LASSO | hedge = stable set (slopes only) | {win_note} | "
          f"HAC = Newey-West(1994)")
    print("-" * 92)
    ...
    print("-" * 60)
    print(f"  Panel D (AEN, {mode}) — annualised alpha, t in ()")
    print("-" * 60)
    ...
    print(f"\nSaved: {out_path}")
    print("Notes:")
    print("  - Switch the window by editing ONE line: OOS_WINDOW = 'rolling'.")
    print("  - Same engine as the factor-benchmark Panel D (oos.oos_residuals):")
    print("    loadings re-estimated on the past window, slopes-only residual,")
    print("    NW(1994) HAC.  NO LASSO and NO PCA -- the stable set is frozen.")
    print("  - This is alpha_0 (the level).  The conditional stress slope alpha_1")
    print("    is handled separately (post-selection inference: PDS / re-sel boot).")
