  ...
  * alpha_1 (the STRESS slope) is covered here by TWO valid procedures that
    AGREE, on z_{t-1} (predetermined, canonical CFG timing):
      ...
      (2) RE-SELECTION BOOTSTRAP -- coherent cross-check that reuses the paper's
          OWN selection cell (bootstrap_stability._aen_boot_cell), NO new LASSO.
          Each block resample (stationary, block=9, the SAME scheme as stability
          selection) re-runs the AEN selection -- adaptive-weight Stage-1 weights,
          lambda2, and lambda1 fixed at their full-sample values exactly as the
          stability-selection bootstrap fixes weights and lambda2 (one CD fit per
          resample); selection still varies because the resampled data vary --
          then fits the CFG and records alpha_1.  (RESEL_RETUNE_LAMBDA1=True
          instead re-tunes lambda1 per resample by HQC; slower, same conclusions.)
          The distribution of alpha_1 across resamples integrates selection AND
          sampling uncertainty (Efron 2014, "bootstrap the whole procedure").
          CAVEAT: an n-out-of-n bootstrap after model selection is NOT uniformly
          consistent near the null (Leeb-Poetscher) -- it can under-cover for
          borderline cases -- which is why it is a cross-check, not the headline.
          For a clearly-separated signal (CDS) it confirms PDS; for the null cases
          (BTP, iTraxx) it agrees they are ns.
...
# -- the paper's OWN AEN machine (bootstrap_stability.py) -------------------
#    Re-used verbatim for the re-selection bootstrap: NO new estimator.
_bs_path = PROJECT_ROOT / "src" / "machine_learning" / "bootstrap_stability.py"
_bspec = importlib.util.spec_from_file_location("bootstrap_stability", _bs_path)
...
aen_boot_cell             = bs._aen_boot_cell          # the paper's exact per-rep AEN cell
...
STABILITY_MAX_Q           = bs.STABILITY_MAX_Q
...
# re-selection bootstrap controls (offline diagnostic)
# DEFAULT (fast): re-select with the paper's exact cell (_aen_boot_cell) at the
#   full-sample-optimal lambda1 -> ONE coordinate-descent fit per resample.
#   Weights/lambda2/lambda1 fixed at full-sample values exactly as the stability-
#   selection bootstrap fixes weights/lambda2; selection still varies because the
#   resampled data vary.  ~tens of seconds for B=499.
# OPT-IN (slower): RESEL_RETUNE_LAMBDA1=True re-tunes lambda1 per resample by HQC
#   over a grid -> RESEL_N_LAMBDA1 fits per resample (minutes).
RESEL_B               = 499     # bootstrap replications (bump to 999 for the final run)
RESEL_RETUNE_LAMBDA1  = False   # True -> re-tune lambda1 per resample (slower, more thorough)
RESEL_N_LAMBDA1       = 30      # lambda1 grid size IF retuning (HQC); ignored when fixed
RESEL_RETUNE_MAXITER  = 3000    # capped CD iterations for the retune path (selection is robust)
RESEL_SETUP_N_LAMBDA1 = 40      # grid for the one-off full-sample weights/lambda2/lambda1
...
def _reselect_aen(y_b, F_b, weights_fs, lambda2_fs, lambda1_grid, max_iter=3000):
    """
    OPT-IN path: re-run the AEN selection on one block resample, re-tuning lambda1
    by HQC over `lambda1_grid` (adaptive weights and lambda2 fixed at full-sample
    values).  Returns selected column indices.  Re-centres y and re-L2-normalises
    X internally, identical to _aen_boot_cell.  CD iterations are capped (selection
    is robust to loose convergence) to keep the per-resample cost bounded.
    """
    y_c = y_b - y_b.mean()
...
def reselection_bootstrap_alpha1(y, F, z, cand, alpha1_point,
    ...
    # one-off: full-sample AEN tuning (adaptive weights, lambda2, lambda1*).
    # This is the silent-but-not-stuck part; print so it is visible.
    print(f"        [setup] full-sample AEN tuning (one-off)...", end="", flush=True)
    t_su = time.time()
    y_c = yv - yv.mean()
    Xc = Fv - Fv.mean(axis=0)
    l2 = np.sqrt((Xc ** 2).sum(axis=0)); l2[l2 < 1e-12] = 1.0
    X_full = Xc / l2
    w_fs, lam2_fs, lam1_opt, _g, lam1_max, _b, fs_sel = compute_weights_and_grid(
        X_full, y_c, lambda2_grid=AEN_LAMBDA2_GRID, n_lambda1=RESEL_SETUP_N_LAMBDA1,
        criterion=AEN_CRITERION, gamma=AEN_GAMMA, gic_alpha=GIC_ALPHA)
    lambda1_grid = build_lambda1_grid(lam1_max, n_lambda1)   # used only if retuning
    fs_sel_names = [cand[j] for j in fs_sel]
    print(f" done ({time.time() - t_su:.0f}s; lambda1*={lam1_opt:.3e}, "
          f"fs-selected={len(fs_sel)})", flush=True)
    ...
    for b in range(B):
        ...
        if RESEL_RETUNE_LAMBDA1:
            sel = _reselect_aen(y_b, F_b, w_fs, lam2_fs, lambda1_grid,
                                max_iter=RESEL_RETUNE_MAXITER)
        else:
            # paper's exact per-rep cell at the full-sample-optimal lambda1: 1 CD fit
            mask = aen_boot_cell(y_b, F_b, lam1_opt, lam2_fs, w_fs, STABILITY_MAX_Q)
            sel = np.where(mask)[0]
    ...
    return {
        ...
        "B": int(B),
        "retune_lambda1": bool(RESEL_RETUNE_LAMBDA1),
...
def main():
    ...
    for sn, path in STRATEGIES.items():
        ...
        _mode = "retune lambda1/HQC" if RESEL_RETUNE_LAMBDA1 else "fixed lambda1* (paper cell)"
        print(f"     re-selection bootstrap [{_mode}], B={RESEL_B}...", flush=True)
