...
FACTOR_INFO = {
    ...
    # ========== PANEL A: CREDIT RISK ==========
    "BTP_BUND": ("A", "First difference of the yield differential between the Italian 10-year BTP and the German Bund.", r"\citet{fausch2018impact}"),
    "CDX_IG": ("A", "First difference of the CDS spread on the CDX Investment Grade index.", r"\citet{klaus2009hedge}"),
    "CREDIT_EU": ("A", "Change in the bond credit risk factor. First difference of the yield spread between euro-area BBB and AAA corporate bond indices.", r"\citet{fama1989business}"),
    "CREDIT_US": ("A", "Change in the bond credit risk factor. First difference of the yield spread between Moody's BAA and AAA corporate bond indices.", r"\citet{fama1989business}"),
    ...
    "PB_EU_CDS_1Y": ("A", "First difference of the average 1-year CDS spread of European prime brokers.", r"\citet{klaus2009hedge}"),
    "PB_EU_CDS_5Y": ("A", "First difference of the average 5-year CDS spread of European prime brokers.", r"\citet{klaus2009hedge}"),
    "PB_US_CDS_1Y": ("A", "First difference of the average 1-year CDS spread of U.S. prime brokers.", r"\citet{klaus2009hedge}"),
    "PB_US_CDS_5Y": ("A", "First difference of the average 5-year CDS spread of U.S. prime brokers.", r"\citet{klaus2009hedge}"),
    ...
    "ATM_IV_CDX": ("D", "First difference of the 3-month at-the-money implied volatility on options written on the CDX Investment Grade index.", r"\citet{novales2019splitting}"),
    "ATM_IV_ITRX": ("D", "First difference of the 3-month at-the-money implied volatility on options written on the iTraxx Europe Main index.", r"\citet{novales2019splitting}"),
    ...
    "IV_BUND": ("D", "First difference of the 3-month at-the-money implied volatility on the 10-year German Bund futures options.", r"\citet{cremers2021treasury}"),
    "IV_TSY": ("D", "First difference of the 3-month at-the-money implied volatility on the 10-year U.S. Treasury futures options.", r"\citet{cremers2021treasury}"),
    "MOVE": ("D", "First difference of the option-implied U.S. rates volatility (yield-curve weighted average of at-the-money implied normal yield volatilities from OTC options on 2-, 5-, 10-, and 30-year constant-maturity interest rate swaps).", r"\citet{bansal2022bond}"),
    "MOVE2": ("D", "Squared MOVE, capturing nonlinear exposure to option-implied U.S. interest rate volatility shocks.", r"\citet{bansal2022bond}"),
    "PTFSBD": ("D", "Return on the bond trend-following factor, constructed from lookback straddles on government bond futures.", r"\citet{fung2004hedge}"),
    "PTFSCOM": ("D", "Return on the commodity trend-following factor, constructed from lookback straddles on commodity futures.", r"\citet{fung2004hedge}"),
    "PTFSFX": ("D", "Return on the currency trend-following factor, constructed from lookback straddles on major currency futures.", r"\citet{fung2004hedge}"),
    ...
    "5Y5Y_INFL": ("E", "First difference of the 5-year-5-year inflation swap rate (forward-starting EUR inflation swap rate capturing long-term inflation expectations).", "Market indicator"),
    "BFCI_EU": ("E", "First difference of the Bloomberg Euro Area Financial Conditions Index (composite Z-score of euro-area money, bond, and equity indicators, standardized to the pre-crisis period).", r"\citet{osina2019global}"),
    "EPU_EU": ("E", "First difference of the Economic Policy Uncertainty index for Europe (newspaper-based index aggregated across European countries).", r"\citet{baker2016measuring}"),
    "EPU_US": ("E", "First difference of the Economic Policy Uncertainty index for the United States (newspaper-based index constructed from normalized frequency of articles referencing economy, uncertainty, and policy).", r"\citet{baker2016measuring}"),
    ...
    "SS2Y": ("F", "First difference of the 2-year EUR swap spread (2-year EUR fixed swap par rate minus 2-year German Schatz yield).", r"\citet{collin2001determinants}"),
    "SS5Y": ("F", "First difference of the 5-year EUR swap spread (5-year EUR fixed swap par rate minus 5-year German Bobl yield).", r"\citet{collin2001determinants}"),
    "SS10Y": ("F", "First difference of the 10-year EUR swap spread (10-year EUR fixed swap par rate minus 10-year German Bund yield).", r"\citet{collin2001determinants}"),
