...
def main():
    ...
    print(f"PCA OUT-OF-SAMPLE ALPHA  (past-only beta, self-contained windows)   "
          f"window = '{OOS_WINDOW.upper()}'")
    print("=" * 92)
    print(f"  K = {N_PC} PCs | min_train = {OOS_MIN_TRAIN}"
          f"{' | L = ' + str(OOS_ROLL_WINDOW) if OOS_WINDOW == 'rolling' else ''}"
    ...
    for sn in STRATEGIES:
        ...
        out[sn] = {"window": OOS_WINDOW, "n_oos": int(len(e)),
                   ...
                   "min_train": OOS_MIN_TRAIN,
                   "roll_window": (OOS_ROLL_WINDOW if OOS_WINDOW == "rolling" else None)}
    ...
    # Panel-D-style block (annualised %, t in parentheses)
    print(f"\n{'-'*60}\n  Panel D (PCA, {OOS_WINDOW}) — annualised alpha, t in ()\n{'-'*60}")
    for sn, a, t, p, n, _ in rows:
        print(f"   {sn:<18} {a:+6.2f}{_stars(p):<3}  ({t:.2f})")

    outdir = get_pca_output_dir()
    fname = f"pca_oos_alpha_{OOS_WINDOW}.json"
    (outdir / fname).write_text(json.dumps(out, indent=2, default=float))
    print(f"\nSaved: {outdir / fname}")
    print("\nNotes:")
