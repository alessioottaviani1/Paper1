...
def generate_alpha_regime_synthesis():
    """
    Synthesis table for PCA and AEN, parallel to the benchmark
    alpha_synthesis_across_models (Panels A/B/C below mirror the benchmark
    Panels B/C/D).

    Reads (expanding-window OOS only, per the paper's main-text choice):
      regime : results/<s>/pca/regime_results_<timing>.json
               results/<s>/aen/hqc/conditional_subsample.json
      CFG    : results/<s>/pca/conditional_fs_<timing>.json
               results/<s>/aen/hqc/conditional_ferson_schadt.json
      OOS    : results/pca/pca_oos_alpha_expanding.json
               results/machine_learning/aen_oos_alpha_expanding.json
    """
    print_header("GENERATING PCA+AEN SYNTHESIS TABLE (regime + CFG + OOS)", "-")
    ...
    def fmt_coef(val, pval, dec=2):
        """'+x.xx' or '$+x.xx^{**}$' if significant; '--' if missing."""
        if val is None or (isinstance(val, float) and np.isnan(val)):
            return "--"
        s = sig_super(pval)
        return (f"${val:+.{dec}f}^{{{s}}}$" if s else f"{val:+.{dec}f}")
    ...
    # ── PCA score timing actually present (prefer contemporaneous) ───────────
    def pca_timing_for(s):
        s_dir = get_strategy_pca_dir(s)
        for timing in ['contemporaneous', 'predictive']:
            if (s_dir / f"regime_results_{timing}.json").exists() or \
               (s_dir / f"conditional_fs_{timing}.json").exists():
                return timing
        return 'contemporaneous'

    # ── REGIME (Panel A) ─────────────────────────────────────────────────────
    pca_regime, aen_regime = {}, {}
    for s in strategies:
        t = pca_timing_for(s)
        rd = _load(get_strategy_pca_dir(s) / f"regime_results_{t}.json")
        if rd: pca_regime[s] = rd
        cs = _load(aen_cfg.get_strategy_aen_dir(s) / "conditional_subsample.json")
        if cs: aen_regime[s] = cs.get('regimes', {})

    # ── CFG (Panel B) ────────────────────────────────────────────────────────
    pca_cfg, aen_cfg_data = {}, {}
    for s in strategies:
        t = pca_timing_for(s)
        pc = _load(get_strategy_pca_dir(s) / f"conditional_fs_{t}.json")
        if pc: pca_cfg[s] = pc.get('conditional', {})
        ac = _load(aen_cfg.get_strategy_aen_dir(s) / "conditional_ferson_schadt.json")
        if ac: aen_cfg_data[s] = ac.get('conditional', {})

    # ── OOS (Panel C), expanding window only ─────────────────────────────────
    pca_oos_all = _load(PROJECT_ROOT / "results" / "pca" / "pca_oos_alpha_expanding.json") or {}
    aen_oos_all = _load(PROJECT_ROOT / "results" / "machine_learning" / "aen_oos_alpha_expanding.json") or {}

    def oos_cell(d):
        """Return (alpha_ann, t, p, N) handling PCA vs AEN key names; None-safe."""
        if not d:
            return (None, None, None, None)
        a = d.get('alpha_oos_annualized', d.get('alpha_oos_ann'))
        t = d.get('t_stat', d.get('t'))
        p = d.get('p_value', d.get('p'))
        n = d.get('n_oos')
        return (a, t, p, n)

    if not (pca_regime or aen_regime or pca_cfg or aen_cfg_data
            or pca_oos_all or aen_oos_all):
        print("   ⚠️  No PCA/AEN data found, skipping synthesis.")
        return None

    tex_path = TABLES_DIR / "alpha_regime_synthesis_pca_aen.tex"
    with open(tex_path, 'w', encoding='utf-8') as f:
        f.write("% " + "=" * 74 + "\n")
        f.write("% ALPHA SYNTHESIS — PCA + AEN  (regime / CFG / out-of-sample)\n")
        f.write("% " + "=" * 74 + "\n\n")

        f.write("\\begin{table}[H]\n")
        f.write("\\centering\n")
        f.write("\\caption{Alpha Synthesis: PCA and AEN}\n")
        ...
        # ────────────────────────── Panel A: regime ──────────────────────────
        f.write("\\begin{center}\\textit{Panel A: Alpha by stress regime (\\% p.a.)}\\end{center}\n")
        f.write("\\vspace{0.05cm}\n")
        ...
        f.write("\\midrule\n")
        for s in strategies:
            f.write(f"\\textit{{{strategy_map.get(s, s)}}}")
            rd = pca_regime.get(s, {})
            for period in ['Stress', 'Normal']:
                p_data = rd.get(period, {})
                f.write(" & " + fmt_coef(p_data.get('alpha_annual'),
                                         p_data.get('alpha_pvalue')))
            ar = aen_regime.get(s, {})
            for reg in ['HIGH', 'NORMAL']:
                rdat = ar.get(reg, {})
                if rdat.get('skip'):
                    f.write(" & --")
                else:
                    f.write(" & " + fmt_coef(rdat.get('alpha_annualized'),
                                             rdat.get('alpha_pval')))
            f.write(" \\\\\n ")
            rd = pca_regime.get(s, {})
            for period in ['Stress', 'Normal']:
                t = rd.get(period, {}).get('alpha_tstat')
                f.write(f" & ({t:.2f})" if t is not None and not (isinstance(t, float) and np.isnan(t)) else " & ")
            ar = aen_regime.get(s, {})
            for reg in ['HIGH', 'NORMAL']:
                rdat = ar.get(reg, {})
                t = rdat.get('alpha_tstat')
                if rdat.get('skip') or t is None or (isinstance(t, float) and np.isnan(t)):
                    f.write(" & ")
                else:
                    f.write(f" & ({t:.2f})")
            f.write(" \\\\\n\\addlinespace\n")
        ...
        # ────────────────────── Panel B: CFG conditional alpha ────────────────
        f.write("\\vspace{0.35cm}\n")
        f.write("\\begin{center}\\textit{Panel B: Christopherson, Ferson, and Glassman (1998) "
                "conditional alpha (\\% p.a.)}\\end{center}\n")
        f.write("\\vspace{0.05cm}\n")
        f.write("\\noindent\\makebox[\\linewidth][c]{%\n")
        f.write("\\begin{tabular}{l r r r r r r}\n")
        f.write("\\toprule\n")
        f.write(" & \\multicolumn{3}{c}{PCA} & \\multicolumn{3}{c}{AEN} \\\\\n")
        f.write("\\cmidrule(lr){2-4} \\cmidrule(lr){5-7}\n")
        f.write(" & $\\alpha_0$ & $\\alpha_1$ & $\\bar{R}^2$"
                " & $\\alpha_0$ & $\\alpha_1$ & $\\bar{R}^2$ \\\\\n")
        f.write("\\midrule\n")
        for s in strategies:
            f.write(f"\\textit{{{strategy_map.get(s, s)}}}")
            for cfg in (pca_cfg.get(s, {}), aen_cfg_data.get(s, {})):
                if cfg:
                    a0 = fmt_coef(cfg.get('alpha0_annualized'), cfg.get('alpha0_pval'))
                    a1v = cfg.get('alpha1')
                    a1_ann = (a1v * 12.0) if isinstance(a1v, (int, float)) else None
                    a1 = fmt_coef(a1_ann, cfg.get('alpha1_pval'))
                    r2 = cfg.get('r2_adj')
            ...
            f.write(" \\\\\n ")
            for cfg in (pca_cfg.get(s, {}), aen_cfg_data.get(s, {})):
                t0 = cfg.get('alpha0_tstat'); t1 = cfg.get('alpha1_tstat')
                c0 = f"({t0:.2f})" if isinstance(t0, (int, float)) else ""
                c1 = f"({t1:.2f})" if isinstance(t1, (int, float)) else ""
                f.write(f" & {c0} & {c1} & ")
            f.write(" \\\\\n\\addlinespace\n")
        f.write("\\bottomrule\n")
        f.write("\\end{tabular}}\n\n")
        ...
        f.write("\\vspace{0.35cm}\n")
        f.write("\\begin{center}\\textit{Panel C: Out-of-sample alpha (\\% p.a.)}\\end{center}\n")
        f.write("\\vspace{0.05cm}\n")
        f.write("\\noindent\\makebox[\\linewidth][c]{%\n")
        f.write("\\begin{tabular}{l r r r r r r}\n")
        f.write("\\toprule\n")
        f.write(" & \\multicolumn{3}{c}{PCA} & \\multicolumn{3}{c}{AEN} \\\\\n")
        f.write("\\cmidrule(lr){2-4} \\cmidrule(lr){5-7}\n")
        f.write(" & $\\alpha$ & $t$ & $N$ & $\\alpha$ & $t$ & $N$ \\\\\n")
        f.write("\\midrule\n")
        for s in strategies:
            f.write(f"\\textit{{{strategy_map.get(s, s)}}}")
            for a, t, p, n in (oos_cell(pca_oos_all.get(s)), oos_cell(aen_oos_all.get(s))):
                if a is None or (isinstance(a, float) and np.isnan(a)):
                    f.write(" & -- & -- & --")
                else:
                    f.write(" & " + fmt_coef(a, p))
                    f.write(f" & {t:.2f}" if t is not None else " & --")
                    f.write(f" & {n}" if n is not None else " & --")
            f.write(" \\\\\n")
        f.write("\\bottomrule\n")
        f.write("\\end{tabular}}\n\n")
        ...
        f.write("\\footnotesize\n")
        f.write("\\item \\textit{Note:} ")
        f.write("Annualized alpha (\\% p.a.) for the PCA and AEN factor specifications. "
                "PCA uses the spanning regression on rolling principal components; "
                "AEN uses the post-selection OLS on the bootstrap-stable factors. "
                "Panel A reports alpha estimated separately in HIGH and NORMAL stress "
                "months, where HIGH denotes months in which the 5-year iTraxx Europe "
                "Main spread exceeds 100~bps. "
                "Panel B reports the intercept $\\alpha_0$ and the stress-loading "
                "coefficient $\\alpha_1$ from the \\citet{christopherson1998conditioning} "
                "full conditional model "
                "$r_t = \\alpha_0 + \\alpha_1 z_{t-1} + (\\beta + \\delta z_{t-1})' X_t "
                "+ \\varepsilon_t$, in which both the alpha and the factor loadings are "
                "conditioned on $z_{t-1}$, the one-period-lagged standardized iTraxx Main "
                "5Y spread; significance of $\\alpha_1$ is from a moving-block bootstrap "
                "($B=9{,}999$, $H_0{:}\\,\\alpha_1=0$ imposed, block length~9). "
                "Panel C reports out-of-sample alpha: factor loadings are re-estimated on "
                "an expanding past-only window (60-month burn-in), the hedge is formed from "
                "the lagged loadings, and the realized abnormal return is tested by "
                "Newey--West HAC; $N$ is the number of out-of-sample months. "
    ...
    print(f"   💾 Saved: {tex_path}  (Panels A regime / B CFG / C OOS)")
    return tex_path
