...
def generate_alpha_regime_synthesis():
    ...
    Reads:
      - PCA: results/<strategy>/pca/conditional_subsample_<timing>.json
    ...
    # ── Load PCA regime results (HIGH/NORMAL sub-sample, mirror of AEN) ──
    pca_regime_data = {}
    for s in strategies:
        s_dir = get_strategy_pca_dir(s)
        for timing in ['contemporaneous', 'predictive']:
            regime_path = s_dir / f"conditional_subsample_{timing}.json"
    ...
    with open(tex_path, 'w', encoding='utf-8') as f:
        ...
        for s in strategies:
            ...
            pca = {}
            for timing in ['contemporaneous', 'predictive']:
                if (s, timing) in pca_regime_data:
                    pca = pca_regime_data[(s, timing)].get('regimes', {})
                    break
            ph = pca.get('HIGH', {})
            pn = pca.get('NORMAL', {})
            if ph and pn and not ph.get('skip') and not pn.get('skip'):
                for regime_data in [ph, pn]:
