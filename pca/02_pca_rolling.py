...
================================================================================
In-sample: PC full-sample per lo spanning contemporaneo (Connor-Korajczyk 1986/1988;
Giglio-Xiu 2021). OOS (vedi 06): PC ri-stimati ricorsivamente su soli dati passati
(meccanica alla Ludvigson-Ng 2009).
...
def main():
    ...
    if rolling_pca.all_loadings:
        # Sign-align eigenvectors across windows before averaging.
        # PCA eigenvectors have arbitrary sign per window; if a PC flips
        # between windows, naive averaging cancels genuine loadings.
        # Standard fix (Hirsa et al., 2023): use the dot product with a
        # reference window; if negative, the PC is flipped, so multiply by -1.
        aligned = [rolling_pca.all_loadings[0].copy()]
        for L_curr in rolling_pca.all_loadings[1:]:
            L_aln = L_curr.copy()
            for k in range(L_aln.shape[0]):  # each PC row
                # Align to the previous (already-aligned) window
                if np.dot(L_aln[k], aligned[-1][k]) < 0:
                    L_aln[k] = -L_aln[k]
            aligned.append(L_aln)
        avg_loadings = np.mean(aligned, axis=0)
        ...
        if n_windows > 1:
            ...
            print("   (full-sample PCA: stima unica, stabilita' loadings tra finestre N/A)")
