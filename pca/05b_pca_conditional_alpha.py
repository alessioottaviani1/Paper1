...
def multiple_conditioning_variables(strategy_name, timing):
    ...
    for proxy_name, cfg in CONDITIONING_VARIABLES.items():
        ...
        a1_p_hac = float(res.pvalues['z_stress'])
        a1_p = cfg_alpha1_bootstrap_p(y_s, X_s, z_std)   # bootstrap (headline)
        ...
        results[proxy_name] = {
            ...
            'alpha1_tstat': round(float(a1_t), 4),
            'alpha1_pval': round(float(a1_p), 4),          # block-bootstrap p
            'alpha1_pval_hac': round(float(a1_p_hac), 4),  # HAC p (robustness)
...
def generate_tex(all_results, timing):
    ...
    tex.append(r"\footnotesize\textit{Notes:} "
               r"Christopherson, Ferson, and Glassman (1998) full conditional model with rolling PCA "
               r"factors: both the alpha and the loadings are conditioned on $z_t$. "
               ...
               r"($B=9{,}999$, $H_0{:}\,\alpha_1=0$ imposed, block~9); Newey--West HAC "
               r"$t$-statistics in parentheses are Newey--West HAC. "
    ...
    tex3.append(r"\footnotesize\textit{Notes:} "
                r"Christopherson, Ferson, and Glassman (1998) with PCA factors and alternative $z_t$. "
                r"HAC standard errors. "
                r"*** $p < 0.01$, ** $p < 0.05$, * $p < 0.10$.")
    ...
    tex_b.append(r"{\tiny Christopherson, Ferson \& Glassman (1998); Patton (2009). "
...
def generate_article_tex(all_results, timing):
    ...
    tex.append(r"\item \textit{Note:} "
               r"Christopherson, Ferson, and Glassman (1998) full conditional model with rolling PCA "
               r"factors (alpha and loadings both conditioned on $z_t$). "
               rf"Timing: {timing_label} ($PC_t \to R_{{t+1}}$ if predictive). "
               r"$z_t$ = standardized iTraxx Main 5Y. "
               r"Significance of $\alpha_1$ from a moving-block bootstrap "
               r"($B=9{,}999$, $H_0{:}\,\alpha_1=0$, block~9); Newey--West HAC $t$-statistics in "
               r"parentheses; the HAC $p$-value is a robustness check. "
               r"$^{***}\,p<0.01$, $^{**}\,p<0.05$, $^{*}\,p<0.10$.")
...
def main():
    ...
    print(f"\n   References:")
    print(f"   - Christopherson, Ferson & Glassman (1998, RFS)")
    print(f"   - Mitchell & Pulvino (2001, JF)")
    print(f"   - Patton (2009, RFS)")
    print(f"   - Connor-Korajczyk (1986,1988); Giglio-Xiu (2021): full-sample PCA spanning")
