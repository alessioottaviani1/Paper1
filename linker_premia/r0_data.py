...
CPI_TICKERS = {"UK": "UKRPI Index", "US": "CPURNSA Index",
               "US_SA": "CPI INDX Index"}   # CPI-U SA (BLS CUSR0000SA0): la sorpresa
                                            # di Maffei (sez. 6.1) usa il SA --
                                            # VERIFICARE il ticker a terminale
...
try:
    ...
    if lpath.exists():
        print(f"  an_liq: {len(pd.read_parquet(lpath))} mesi (proxy: {list(pd.read_parquet(lpath).columns)})")
except ImportError:
