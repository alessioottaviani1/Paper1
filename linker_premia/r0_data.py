...
# US CPI (NSA+SA) e UK RPI ora arrivano da FONTE UFFICIALE (FRED/ONS, sopra): NON sono
# piu' qui. Restano su Bloomberg solo i CPI NAZIONALI IT/FR/DE del test fiscale r2d, per
# cui non abbiamo (ancora) un loader Istat/Insee/Destatis -- e sono fuori dal perimetro
# US/UK che RR chiede di tenere su fonti ufficiali.
CPI_TICKERS = {"IT_NAT": "ITCPIUNR Index",  # Italy CPI FOI ex-tabacco (indice dei BTPei)
...
# ---------------------------------------------------- FONTI UFFICIALI (richiesta RR in call)
# RR: "meglio usare sempre fonti accademiche/ufficiali; Bloomberg solo come alternativa".
# Qui gli indici d'inflazione arrivano PRIMA dalle fonti ufficiali, con Bloomberg come
# fallback se il download fallisce (proxy aziendale, endpoint giu', ecc.):
#   US  CPI-U NSA : BLS CUUR0000SA0 via FRED  (serie CPIAUCNS)   -> an_cpi_US.parquet
#   US  CPI-U SA  : BLS CUSR0000SA0 via FRED  (serie CPIAUCSL)   -> an_cpi_US_SA.parquet
#   UK  RPI       : ONS CHAW, dataset MM23    (Jan 1987 = 100)   -> an_cpi_UK.parquet
# Cleveland era GIA' ufficiale (clevelandfed.org, sotto). Le curve GSW e BoE idem (pipeline).
# Restano su Bloomberg: swap d'inflazione e proxy di liquidita' (nessuna fonte ufficiale
# esiste: sono dati di mercato OTC) e i CPI nazionali IT/FR/DE di r2d (Istat/Insee/Destatis
# possibili in seguito; per ora fuori dal perimetro US/UK di RR).
FRED_IDS = {"US": "CPIAUCNS", "US_SA": "CPIAUCSL"}
ONS_IDS  = {"UK": ("chaw", "mm23")}          # RPI all items

def _fred(series_id: str) -> pd.Series:
    """Serie mensile da FRED (CSV pubblico, nessuna API key)."""
    ...
    if s.empty:
        raise ValueError(f"FRED {series_id}: serie vuota")
    return s

def _ons(series_id: str, dataset: str) -> pd.Series:
    """Serie mensile da ONS (CSV del generator; tiene solo le righe 'YYYY MMM').
    ONS risponde 403 alle richieste senza User-Agent: lo impostiamo esplicitamente."""
    import io as _io, urllib.request as _req
    url = (f"https://www.ons.gov.uk/generator?format=csv"
           f"&uri=/economy/inflationandpriceindices/timeseries/{series_id}/{dataset}")
    r = _req.Request(url, headers={"User-Agent": "Mozilla/5.0 (research; academic use)"})
    with _req.urlopen(r, timeout=30) as resp:
        txt = resp.read().decode("utf-8", errors="replace")
    raw = pd.read_csv(_io.StringIO(txt), header=None, names=["k", "v"], skip_blank_lines=True)
    m = raw["k"].astype(str).str.match(r"^\d{4} [A-Z]{3}$", na=False)
    d = raw[m].copy()
    if d.empty:
        raise ValueError(f"ONS {series_id}: nessuna riga mensile nel CSV")
    d["date"] = pd.to_datetime(d["k"], format="%Y %b")
    s = pd.to_numeric(d.set_index("date")["v"], errors="coerce").dropna()
    return s

def _save_official(mkt: str, s: pd.Series, src: str):
    df_new = s.to_frame(mkt)
    path = CACHE / f"an_cpi_{mkt}.parquet"
    # cross-check con l'eventuale cache Bloomberg preesistente (stesso indice sottostante)
    if path.exists():
        try:
            old = pd.read_parquet(path).iloc[:, 0]
            old.index = pd.to_datetime(old.index).to_period("M")
            new_m = s.copy(); new_m.index = new_m.index.to_period("M")
            both = pd.concat([old.groupby(old.index).last(),
                              new_m.groupby(new_m.index).last()], axis=1).dropna()
            if len(both):
                gap = (both.iloc[:, 0] - both.iloc[:, 1]).abs().max()
                print(f"  an_cpi_{mkt}: cross-check vs cache precedente, max|gap|={gap:.4g}")
        except Exception:
            pass
    df_new.to_parquet(path)
    print(f"  an_cpi_{mkt}: {len(s)} mesi [{src}] {s.index.min().date()} -> {s.index.max().date()}")

# NESSUN fallback: se una fonte ufficiale fallisce, l'eccezione si propaga e blocca il
# run. E' voluto -- un fallback silenzioso su Bloomberg farebbe passare inosservato il
# fatto che la serie NON e' piu' ufficiale (era successo con l'RPI: 403 mascherato).
for _mkt, _sid in FRED_IDS.items():
    _save_official(_mkt, _fred(_sid), f"FONTE UFFICIALE: FRED/BLS {_sid}")
for _mkt, (_sid, _ds) in ONS_IDS.items():
    _save_official(_mkt, _ons(_sid, _ds), f"FONTE UFFICIALE: ONS {_sid.upper()}/{_ds.upper()}")
