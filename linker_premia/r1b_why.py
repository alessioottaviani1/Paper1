...
versione precedente non aveva.
  ...
  (4) LIVELLI, non variazioni -- e non e' una convenzione. L'ADF rigetta la radice
      unitaria sui premi a TUTTE le scadenze (US 5y -3.48, UK 5y -3.15, US 10y -5.14,
      UK 10y -4.00; valore critico 5% -2.88): la dipendente e' I(0). Differenziare una
      serie stazionaria e' SOVRA-DIFFERENZIAZIONE: induce una radice unitaria nella
      componente MA dell'errore, che e' un errore di specificazione, e distrugge proprio
      l'informazione di livello che serve a rispondere a "perche' un premio e' piu' alto".
      Tre regressori risultano I(1) al margine (NOISE -2.55, ONOFF -2.29, HKM ratio
      -2.06): con radici vicine a uno l'ADF ha potenza bassa e sono quasi certamente
      I(0) persistenti. Newey-West assorbe l'autocorrelazione residua, e la conclusione
      non dipende dalla scelta -- NOISE e HKM reggevano in entrambe le specifiche.

  (5) REGRESSIONI SEPARATE, non sul divario. E' il cambio di disegno piu' importante:
...
def _nw_bandwidth_1994(e, n_reg):
    """Numero di lag di Newey-West (1994), 'Automatic Lag Selection in Covariance Matrix
    Estimation', Rev. Econ. Stud. 61: banda scelta dai dati invece che fissata a mano.
    Kernel di Bartlett. Formula: L = floor(c * (T/100)^(2/9)) con c calibrato sui momenti
    dei residui (h_t = somma dei regressori * residuo). Restituisce l'intero >= 0.
    Riferimento pratico: la stessa procedura implementata da statsmodels/sandwich."""
    T = len(e)
    # h_t con pesi unitari sui regressori (w = iota), come nella ricetta NW1994
    h = e.astype(float)
    # stima dei momenti sigma_j = (1/T) sum h_t h_{t-j}
    def sig(j):
        return float(h[j:] @ h[:-j]) / T if j > 0 else float(h @ h) / T
    n = int(4 * (T / 100.0) ** (2.0 / 9.0))          # lag preliminare NW1994 (Bartlett)
    n = max(1, min(n, T - 1))
    s0 = sig(0) + 2.0 * sum(sig(j) for j in range(1, n + 1))
    s1 = 2.0 * sum(j * sig(j) for j in range(1, n + 1))
    if s0 == 0:
        return max(1, n)
    gamma = 1.1447 * (abs(s1 / s0)) ** (2.0 / 3.0)   # costante ottimale Bartlett
    L = int(gamma * T ** (1.0 / 3.0))
    return max(1, min(L, T - 1))


def nw_reg(y, X, lags=None):
    """OLS con errori standard HAC di Newey-West. Se lags=None la banda e' scelta
    automaticamente con la regola di Newey-West (1994): niente numero fisso arbitrario.
    Ritorna (beta, t, lags_usati)."""
    Z = np.column_stack([np.ones(len(X)), X])
    b = np.linalg.lstsq(Z, y, rcond=None)[0]; e = y - Z@b
    if lags is None:
        # banda dai residui scalari (aggregato h_t = somma delle colonne di Z*e)
        h = (Z * e[:, None]).sum(axis=1)
        lags = _nw_bandwidth_1994(h - h.mean(), Z.shape[1])
...
if __name__ == "__main__":
    ...
    hkm_lvl, hkm_ret = hkm_factor()   # ret usato solo nell'ADF, come controllo
    if hkm_lvl is not None:
        P(f"  HKM: {hkm_lvl.name} | {len(hkm_lvl):,} mesi | "
          f"{hkm_lvl.index.min().date()} -> {hkm_lvl.index.max().date()}")
        P("    si usa il capital RATIO (livello): il risk factor e' un RENDIMENTO e non va")
        P("    in una regressione in livelli. I premi sono I(0): v. ADF sotto.")
    else:
    ...
    def blocco():
        """Regressione in LIVELLI: i premi sono I(0) (v. ADF sopra), quindi differenziarli
        sarebbe sovra-differenziazione. E' anche la specifica che risponde alla domanda di
        RR, che e' sui livelli: perche' un premio e' piu' alto dell'altro."""
        P("\n  --- REGRESSIONI (livelli) ---")
        for m in ten:
            X, nomi = [], []
            if liq is not None:
                for c in liq.columns:
                    X.append(clean(liq[c])); nomi.append(f"C) {c}")
            if hkm_lvl is not None:
                X.append(clean(hkm_lvl)); nomi.append("D) HKM capital ratio")
            ...
            Xv = R.iloc[:, 2:].values
            bU, tU, lU = nw_reg(R["US"].values, Xv)      # banda automatica NW(1994)
            bK, tK, lK = nw_reg(R["UK"].values, Xv)
            bG, tG, lG = nw_reg((R["US"]-R["UK"]).values, Xv)
            P(f"\n    scadenza {m:.0f}y  --  n {len(R)}, Newey-West banda automatica "
              f"(NW1994): US {lU}, UK {lK}, diff {lG} lag")
        ...
        # ROBUSTEZZA ANTI-SPURIA: la regressione in livelli e' lecita solo se y e x sono
        # I(0) (o cointegrate). Per i regressori molto persistenti (es. NOISE, ADF al
        # margine) un beta 'significativo' in livelli puo' essere spurio (Granger-Newbold
        # 1974, Ferson-Sarkissian-Simin 2003) e nessun HAC lo corregge. Qui ristimiamo il
        # coefficiente in DIFFERENZE PRIME: se il segno e la significativita' reggono anche
        # li', il risultato non e' un artefatto di persistenza.
        dR = R.diff().dropna()
        if len(dR) > 40:
            dXv = dR.iloc[:, 2:].values
            _, tGd, lGd = nw_reg((dR["US"] - dR["UK"]).values, dXv)
            P(f"      controllo in differenze (t sulla differenza US-UK, banda {lGd}):")
            P("        " + "  ".join(f"{nm.split(')')[0]}){tGd[i+1]:+.1f}"
                                     for i, nm in enumerate(nomi)))
            P("        [se il segno regge in livelli E in differenze la regressione non e'")
            P("         spuria; se un canale e' forte in livelli ma svanisce in differenze,")
            P("         quel coefficiente va letto con cautela.]")
    ...
    for m in ten:
        ...
        if len(pre) < 40 or len(post) < 40:
            ...
            continue
        nomi_ = nk
        bp_, tp_, lp_ = nw_reg(pre.iloc[:,0].values, pre.iloc[:,1:].values)
        bo_, to_, lo_ = nw_reg(post.iloc[:,0].values, post.iloc[:,1:].values)
