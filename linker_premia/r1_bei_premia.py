...
print(">>> r1 US/UK-only v4 -- griglia completa, campionamento su DATE DI RILASCIO <<<")
...
# CAMPIONAMENTO: date di RILASCIO del CPI, non fine mese. Una convenzione sola per tutte
# e tre le sottrazioni (v. rp.lam_gamma_rel per il perche'). Con eom() la sottrazione
# BEI - E[pi] soffriva di look-ahead: il CPI del mese m esce a meta' del mese m+1, quindi
# un'aspettativa datata fine-m usava informazione non ancora pubblica. E BEI - ISR, pur
# non toccato da quel problema, sarebbe rimasto su una griglia diversa dalle altre due,
# rendendo il confronto fra le basi dipendente dal campionamento.
out = {}
for mkt in ("US", "UK"):        # SOLO i due mercati che RR chiede (curve solide)
    b, s, lam = rp.lam_gamma_rel(mkt, MATS)
...
if MATS_UK_EXT:
    _b2, _s2, _lam2 = rp.lam_gamma_rel("UK", MATS_UK_EXT)
...
for _mkt in ("US", "UK"):
    ...
    for _lbl, _pih in _expinf[_mkt].items():
        # out e' indicizzato al MESE DI RIFERIMENTO (v. sample_at), e le aspettative --
        # Cleveland e AR(1) -- sono anch'esse datate al mese di riferimento del CPI su cui
        # sono costruite. L'allineamento e' quindi diretto: nessun ffill, che maschererebbe
        # eventuali buchi riportando in avanti un valore vecchio senza dirlo.
        _pih = _pih.reindex(pd.Index(sorted({float(m) for m in _mm}), name=None), axis=1) \
                   .reindex(out[f"BEI_{_mkt}_{int(_mm[0])}y"].dropna().index)
        for m in _mm:
            k = int(m)
