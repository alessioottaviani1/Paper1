...
for ax, var, ttl in ((axes[0], "lambda", "λ(T) on surprise, net of liquidity  [bp/unit]"),
                     (axes[1], "dy_real", "Δ real yield(T) on surprise  [bp/unit]")):
    for mkt, col, off in (("US", C_US, -0.15), ("UK", C_UK, 0.15)):
        d = r[(r["mkt"] == mkt) & (r["var"] == var)].sort_values("mat")
        bcol = "beta_surp" if "beta_surp" in d.columns and d["beta_surp"].notna().any() and var == "lambda" else "beta_bp"
        tcol = "t_surp" if bcol == "beta_surp" else "t_NW"
        x = np.arange(len(d)) + off
        ax.bar(x, d[bcol], width=0.3, color=col, alpha=0.9, label=mkt)
        for xi, (bb, tt) in zip(x, zip(d[bcol], d[tcol])):
