...
# ---------------------------------------------------------------- fig 2: lambda~InfS e lambda~PCA per scadenza
# specificazione Maffei (r2b_maffei_replica): la figura mostra la DOPPIA segmentazione --
# sorprese (InfS) e liquidita' (PCA) -- e il segno OPPOSTO US/UK sulle sorprese.
r = pd.read_csv(CACHE / "an_maffei_replica.csv")
r = r[r["span"].str.startswith("full")]           # full sample per US e UK (confronto a periodo comune)
fig, axes = plt.subplots(1, 2, figsize=(11, 4.4), sharey=False)
panels = ((axes[0], "lambda~InfS", "λ(T) on inflation surprise  [bp/unit]"),
          (axes[1], "lambda~PCA",  "λ(T) on liquidity (PC1)  [bp/s.d.]"))
for ax, reg, ttl in panels:
    labels = None
    for mkt, col, off in (("US", C_US, -0.16), ("UK", C_UK, 0.16)):
        d = r[(r["mkt"] == mkt) & (r["reg"] == reg)].sort_values("mat")
        if d.empty:
            continue
        x = np.arange(len(d)) + off
        ax.bar(x, d["beta"], width=0.32, color=col, alpha=0.9, label=mkt)
        for xi, bb, tt in zip(x, d["beta"], d["t"]):
            ax.text(xi, bb + (0.35 if bb >= 0 else -0.35), f"{tt:+.1f}", ha="center",
                    va="bottom" if bb >= 0 else "top", fontsize=7,
                    color=col if abs(tt) >= 1.96 else C_GREY)
        labels = [f"{m:g}y" for m in d["mat"]]
    if labels:
    ...
    ax.margins(y=0.15)                            # spazio per le etichette t sopra/sotto le barre
axes[0].legend(frameon=False, loc="upper right", fontsize=10)
fig.suptitle("Maffei specification: surprises (left) vs liquidity (right) — US−UK, full sample. "
             "Numbers are t-NW(6)", y=1.02, fontsize=11)
