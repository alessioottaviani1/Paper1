...
Table~\ref{tab:rq3_fevd} and Figure~\ref{fig:rq3_fevd} report the
forecast error variance decomposition (FEVD) from the weekly VAR.

\input{\tablepath/RQ3_FEVD_article}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../results/rq3_duffie/figures/E3_fevd.pdf}
\caption{Forecast Error Variance Decomposition.
  Share of forecast error variance of $\Delta m_i$ attributable to
  each strategy at horizons 1--12 weeks.}
\label{fig:rq3_fevd}
\end{figure}

\paragraph{Threshold VAR: HIGH vs LOW Regime.}
Figure~\ref{fig:rq3_threshold_var} plots GIRF separately for HIGH and
LOW stress regimes, estimated from a threshold VAR.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../results/rq3_duffie/figures/E4_threshold_var_irf.pdf}
\caption{Threshold VAR --- Generalized IRF: HIGH (red) vs.\ LOW (green).
  Responses of $\Delta m$ to one-standard-deviation shocks, estimated
  separately in HIGH ($>120$ bps) and LOW ($<60$ bps) stress regimes.}
\label{fig:rq3_threshold_var}
\end{figure}
