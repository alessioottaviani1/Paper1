...
For robustness to the weighting scheme, this appendix reports the
equal-weighted (EW, \citealp{duarte2007risk}) counterparts of the
signal-weighted indices used in the main text:
Table~\ref{tab:sw_ew_basic} gives the basic performance metrics,
Table~\ref{tab:mppm_ew} the manipulation-proof performance measure, and
Table~\ref{tab:moreira_muir} the volatility-managed results under both
...
Table~\ref{tab:vif_benchmarks} reports Variance Inflation Factors for
the EUR benchmark specifications of Section~\ref{sec:benchmarks}. VIF is
computed on the factor matrix for each strategy's sample; because the
factors are identical across strategies, differences reflect only the
estimation window.
 
Two groups of regressors are highly collinear. In the Duarte
specification, the maturity-sorted government bond portfolios carry
elevated VIF (R5~$\approx$~52--55, R10~$\approx$~24--27,
R2~$\approx$~14--16), the expected consequence of their common duration
exposure; RI and RB are elevated only in the BTP~Italia sample ($16.1$
and $11.8$). In the Brooks et al.\ specification, the two global bond
benchmarks are near-duplicates (GlobTerm and GlobAgg,
VIF~$\approx$~47--66). Collinearity among controls inflates the
standard errors of those loadings but leaves inference on the
intercept unaffected; every Fung--Hsieh factor sits below the
conventional caution threshold of~5.
...
alpha is significant in both sample halves and increases during
stress.
...
cardinality $k$ varies around the baseline. The $\ell_0$ support is not
nested by construction, and the data show both cases: for BTP~Italia and
the iTraxx skew the smaller supports are contained in the larger ones,
whereas for the CDS--Bond basis correlated candidates substitute for one
another as $k$ grows. The alpha is flat in $k$ in all three cases, so the
conclusions do not hinge on the choice of $k$.
...
Table~\ref{tab:aen_selection_names} reports the factor set selected for
each strategy by every method of Table~\ref{tab:selector_robustness}.
The overlap across selectors is partial, as expected when highly
correlated candidates substitute for one another, but one exposure
recurs in every method that selects at all: the fixed-income defensive
factor for BTP~Italia and the industrial bond return for the
CDS--Bond basis. The two data-driven selectors that choose their own
cardinality retain little or nothing---the post-double-selection
control set is a subset of those recurring exposures, and for the
iTraxx skew neither it nor the knockoffs retain any candidate. The
alpha of Table~\ref{tab:selector_robustness} is unchanged throughout,
so the identified risks are not an artifact of the cardinality
constraint or of any single penalization scheme.
...
This appendix reports a dynamic-correlation robustness check for
Section~\ref{sec:interdependencies}, using the same monthly mispricing
changes $\Delta m_{i,t}$ and regime definitions as the main text.
...
\caption[Dynamic Conditional Correlations (DCC-GARCH)]{Dynamic Conditional Correlations (DCC-GARCH).
  Bivariate DCC(1,1) estimates for each pair of $\Delta m$. Shading
  marks the stress regime of Section~\ref{sec:rq3_construction}, by the
  iTraxx Main 5Y spread: green for LOW ($<60$ bps), yellow for MEDIUM
  ($[60,100)$ bps), red for HIGH ($\geq 100$ bps).}
\label{fig:app_dcc}
\end{figure}
