...
($-0.21$, $t = -4.0$) and quality ($+0.04$, $t = 1.9$). The CDS--Bond
...
$t = 3.1$), the natural exposures of a leveraged long-credit position,
...
betting-against-beta ($-0.03$, $t = -3.6$), fixed-income momentum
($-0.04$, $t = -2.6$), and the one-month variance-swap return
...
both schemes: $+1.42\%$ p.a.\ for BTP~Italia, $+3.90\%$ for
CDS--Bond Basis, and $+2.16\%$ for iTraxx Combined on the expanding
window, against in-sample estimates of $+1.6$, $+4.8$, and $+2.7\%$
in Table~\ref{tab:aen_stable_ols}.

\paragraph{Selector Robustness.}

Table~\ref{tab:selector_robustness} confirms that neither the
$\ell_0$ machinery nor the post-selection inference drives the
result. Six alternative selectors---a one-per-risk-category variant of
the best subset, the adaptive elastic net of \citet{zou2009adaptive}
with and without the category constraint, the relaxed lasso, the
post-lasso with plug-in penalty, and complementary-pairs stability
selection---are run on the same candidate design. For each selector
with a split-stable set, the table also reports an \emph{honest}
alpha: the factors are selected on the first half of the sample only,
and the alpha is estimated on the second half, so the inference is
valid by sample splitting. Every method, under either the split-sample
or the frozen-set out-of-sample design, delivers an alpha significant
at the $1\%$ level, with the honest estimates spanning $+1.7$ to
$+2.0\%$ p.a.\ for BTP~Italia, $+2.5$ to $+3.1\%$ for the CDS--Bond
basis, and $+2.1$ to $+2.5\%$ for iTraxx Combined. The selected sets
themselves overlap only partially across methods
(Appendix~\ref{app:aen_selection_matrix}), as is expected when
correlated candidates substitute for one another, but economically
central exposures recur---the fixed-income defensive factor for
BTP~Italia, emerging-market debt and the industrial bond return for
the CDS--Bond basis---and the alpha verdict is invariant to the
choice.
...
models---while the alpha barely moves: $+1.4$ to $+2.0\%$ p.a.\ for
