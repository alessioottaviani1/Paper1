...
Combined (CI $[1.5, 3.8]$). With eight factors each, the models
...
CDS--Bond Basis, and $+2.50\%$ for iTraxx Combined on the expanding
...
elastic net of \citet{zou2009adaptive}, the relaxed lasso, a
nonlinear machine-learning screen (random forest permutation
importance), and model-X knockoffs \citep{candes2018panning} with the
...
$+2.1$ to $+2.5\%$ for iTraxx Combined; for the primary selector the
...
iTraxx Combined---and the double-selection control set is a strict
...
profile shifts up, from $+1.3\%$ to $+3.8\%$. For iTraxx Combined
...
deviation, $t = 4.31$), while for BTP~Italia and iTraxx Combined the
...
$+2.9\%$ for iTraxx Combined, significant at the $1\%$ level in every
