...
factor literature---\citet{duarte2007risk}, \citet{fung2004hedge},
...
an out-of-sample re-estimation in which the factor hedge is formed
from past-only loadings and the alpha is the realised abnormal return.
...
$\textit{CredSpr}$ is the total return on a portfolio long BAA-rated
corporate bonds and short 10-year government bonds, the tradable
counterpart of the Moody's BAA--Treasury yield spread in the original
specification.
...
\begin{split}
  ...
  + \beta_8\,\textit{RatesVol}_t + \varepsilon_{i,t},
\end{split}
\end{equation}
where $\textit{Term}$ is the excess total return on long-term U.S.\
government bonds over the one-month Treasury bill, $\textit{GlTerm}$
and $\textit{GlAgg}$ are the excess returns on the Global
Treasury and Global Aggregate bond indices, $\textit{InflLink}$ is the
excess return on global inflation-linked bonds, $\textit{CorpCred}$ is
the excess return on a U.S.\ high-yield corporate bond index over
cash, $\textit{EmDebt}$ is the excess return on a hard-currency
emerging-market debt index, $\textit{EmCurr}$ is the return on a
basket of emerging-market currencies against the U.S.\ dollar, and
$\textit{RatesVol}$ is the excess return on a one-month variance swap
on the ten-year U.S.\ Treasury futures.
...
premia, and a variance swap on the ten-year Bund in place of the
Treasury, in Brooks et al.; and the 10-year Bund total-return index and
the total return on a long BBB / short Bund position in Fung and
Hsieh. The Fung and Hsieh trend-following lookback straddle factors are currency-independent by
...
Newey--West HAC standard errors. Variance inflation factors for each
...
$+4.0$ to $+4.2\%$ p.a.\ for the CDS--Bond basis, $+1.7$ to $+2.2\%$
for the iTraxx skew, and $+1.6$ to $+2.1\%$ for BTP~Italia.
...
BTP~Italia, and size, value, and momentum for iTraxx Skew, in both
...
in the iTraxx Main spread adds $1.7$ to $3.2$ percentage points of
annual alpha for the CDS--Bond basis and $0.9$ to $1.5$ for iTraxx
Skew. For BTP~Italia, $\alpha_1$ is significant only under the
